...
export class RealAIService implements AIService {
  ...
  async shouldUpdateDocs(services: Services, commitHash: string): Promise<Result<boolean, string>> {
    try {
      console.log(`🔍 Analyzing commit ${commitHash} for documentation update requirements`);
      ...
      if (commitInfo.author === 'semantic-release-bot') {
        console.log('🤖 Detected automated commit from semantic-release-bot - no docs update needed');
      ...
      if (apiDocsChanged) {
        console.log('📚 Detected changes to autogenerated API docs - docs update required');
        return Result.success(true);
      }

      // Step 5: Use AI to analyze the commit
      console.log('🤖 Using AI to analyze commit for documentation update requirements...');
      ...
      console.error('❌ Error in shouldUpdateDocs:', errorMessage);
