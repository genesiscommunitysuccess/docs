...
The script accepts a series of [arguments](/operations/commands/server-commands/#dictionarybuilder) to establish a connection to the database (e.g. user, password, host, etc) and some specific behaviour (e.g. product name, single dictionary file or composed, etc). Sample below.
