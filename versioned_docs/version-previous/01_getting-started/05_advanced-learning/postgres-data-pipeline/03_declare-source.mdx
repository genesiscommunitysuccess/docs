...
Here are the mappings for each [Field](/database/fields-tables-views/tables/tables-basics/#field-types) of the [Table](/database/fields-tables-views/tables/). There are three ways to define a field mapping:

- when the source property name is the same as the [Field](/database/fields-tables-views/tables/tables-basics/#field-types) name, then there is no need to specify anything
- when the source property name is different from the [Field](/database/fields-tables-views/tables/tables-basics/#field-types) name, however, the type is the same as the [Field](/database/fields-tables-views/tables/tables-basics/#field-types) type or is one that can be converted out of the box. In this case, only the name has to be mapped and this is done by specifying the `property` field
- when the source property name and type are different from the [Field](/database/fields-tables-views/tables/tables-basics/#field-types) name and type, a `transform` function can be used to calculate the mapped value

Looking at the code above, note that `TRADE_ID`, `INSTRUMENT_ID`, `SIDE`, `TRADE_DATETIME`, `TRADE_DATE` and `TRADE_STATUS` are calculated fields and all the rest are mapped by name.
...
Follow the links for more information on [mapping](/server/integration/data-pipeline/basics/#map-functions) and the [`transform`](/server/integration/data-pipeline/basics/#transform-function) function.
