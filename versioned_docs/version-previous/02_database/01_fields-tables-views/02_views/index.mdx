...
Like tables, views are database constructs. To make data from a table or view available to the front end, you need to create a [Request Server](/request-server/introduction/) `requestReply` for snapshot data, or a [Data Server](/data-server/introduction/) `query` for streaming data.
