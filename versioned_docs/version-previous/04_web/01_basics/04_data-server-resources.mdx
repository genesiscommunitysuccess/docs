...
And in this other example, if the ORDER_TYPE field contains the value "GTC,OCO", the following expression will return true:
...
### Object methods
The Java methods corresponding to each one of the field types are also accessible from a criteria building perspective.
...
The methods available for each one of the field types can be found in their correspondent Javadoc documentation. See samples below:
