...
- `POLLING_MAP` is slightly more complex; it is used to accelerate the polling frequency temporarily for a set of queries each time specific events occur. The keys need to be valid `events` on the server and the values are maps themselves containing 3 key-value pairs:
