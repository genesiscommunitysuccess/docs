...
You can delete or modify certain records conditionally without having to map all rows. To do this, use the `where` function  within the `map`:
...
You can use the `where` function within the `map`; this gives you the ability to map rows conditionally before the sink operation:
