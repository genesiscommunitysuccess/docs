...
45min
:::
Create a new component called `OrdersAgGrid` extending `ZeroGridPro`. Apply the same style on the `SIDE` field of the `PositionsAgGrid` in the `OrdersAgGrid` - so, SIDE will be green when ***BUY*** and red when ***SELL***.

:::tip
To do this, create an **order-grid** folder below **client\web\src\components** and create the files needed to create a class, template and styles (e.g. **order-grid.ts**, **order-grid.template.ts** and **order-grid.styles.ts**). Apply the necessary changes, as we saw above. 
