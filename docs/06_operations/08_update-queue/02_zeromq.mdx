...
It works because the GENESIS_CLUSTER process knows (via [clustering](../../../operations/clustering/overview/)) all the nodes within the cluster - so it can broadcast and consume appropriately.

- ZeroMQ is best suited for fixed-sized deployments.
- [MQTT](03_mqtt.mdx) is recommended for set-ups that require dynamic scaling.
...
| `ZeroMQConnectToLocalhostViaLoopback` | Forces ZeroMQ to only bind to the local interface, depending on the OS, this might be usefull for Local development where the workstation might be connected to multiple interfaces to avoid flapping, must not be used in production. | `false` |
