...
:::info
<font size="5">**This version of the documentation describes version 8.4 of the Genesis Platform**</font>.
:::
...
* chore: display toast on crud operations - FUI-2097 
...
This change the Data Server going into an error state when string index fields are truncated or when rows are ignored.
...
You only need to take action if your Data Server has been configured to go into ERROR state when this situation arises. This is not a likely design scenario, but if this is the case for your application, you need to create new code to replicate this behaviour.
:::

### Made rxSources non-functional interfaces, standardised http source syntax and added utilities
The unused RxSink interface not a functional interface anymore; it is now a regular interface. Only applications that use the RxSink interface in the Data Pipeline are affected. 
