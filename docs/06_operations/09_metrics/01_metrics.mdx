...
Genesis uses the [metrics](https://micrometer.io/) library, which is commonly used in Java apps. In all known Genesis applications, the impact is negligible; the benefits far outweigh the very tiny impact on performance. 
...
The `MetricService` distinguishes between two different kinds of metric; process and resource.

- Process metrics are values that will exist once and only once per process, current memory usage, GC stats etc.
- Resource metrics represent data about one or more specified resources hosted within a process. 
Dataservers, event handlers, reqreps and consolidators are all examples of named resources.

### Naming hierarchical metrics
For a consistency and to make things easier for operations and support staff, the Genesis platform enforces a standard convention for the naming of hierarchical metrics.
...
- `$metricName` is what is actually being measured, such as 'latency', 'rate', 'count'
...
Path sanitisation is performed in order to ensure path consistency. Any character in the following set will be replaced with an underscore:
```
*@/\’”;:|[]{}()&^%$,
```
...
All these metric functions are also lookup functions into the metric registry. As such, path sanitisation only occurs if the non-sanitised path does not already have a mapping. This is to avoid string scans and regex evaluation on every lookup to retrieve a metric object. 

In order to maximise performance (for example, when counting messages in a high-volume stream):

- do not use upper-case or forbidden characters in your classifiers or names
- do not store the metric object in a local variable
- do not perform a lookup each time it needs to be used

### Parameters
The parameters for defining a metric are the same for all cases:
- name (of the metric)
- tags (key value pairs)

Tags are key value pairs that add dimension to your metrics. They enable you to filter the data and analyse it in more detail. You can set up tags so that you can analyse the metric by user name or by environment, for example.
The easiest way of doing this is to set up a map and specify this. The map can contain any number of key value pairs, and the platform automatically transforms these into tags.
...
:::info
Timers and Distribution Summaries support data collection for observing percentile distributions; they always publish a count of events, in addition to other measurements.
:::
...
class UserAuthentication {
    ...
    void login(User user) {
        ...
        Timer userLoginTime = metricService.timer(
                "user.processing_latency",
...
Histograms are built into Distribution Summaries. These track the distribution of events in a similar way to timers, but they don't represent units of time. For instance, Distribution Summaries can measure payload sizes of incoming requests.
