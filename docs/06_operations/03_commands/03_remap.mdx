...
The system definition key `RemapEnableAutoConfirm` can be set to `true`, this will cause Remap to skip its prompts (assuming `Y` for all prompts) . This is convenient for local and CI/CD development workflows, use with caution (or avoid) for production deployments.
