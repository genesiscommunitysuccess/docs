...
|          | --compactProcesses <!-- genesisInstall --compactProcesses -->  | no        | When set to `true`, combines compatible services into less JVM processes (depending on the strategy), which overall, reduces the total number of services required to run | none                | none |
|          | --compactProcessesStrategy <!-- genesisInstall --compactProcessesStrategy -->  | no        | The strategy to be used when `compactProcess` is set. Defaults to `DEFAULT`, valid values are: `DEFAULT`, `GROUP_ID`. WARNING: When using the GROUP_ID strategy, processes will be optimized (and sometimes removed) with the goal to provide the optimal developer experience with fewer resources as possible, thus this strategy MUST NOT be used for production deployments. | none                | none |
