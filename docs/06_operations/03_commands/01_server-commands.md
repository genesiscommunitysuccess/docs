...
|          | --compactProcessesStrategy <!-- genesisInstall --compactProcessesStrategy -->  | no        | The strategy to be used when `compactProcess` is set. Defaults to `DEFAULT`, valid values are: `DEFAULT`, `DEV`. WARNING: When using the DEV strategy, processes will be optimized (and sometimes removed) with the goal to provide the optimal developer experience with fewer resources as possible, thus this strategy MUST NOT be used for production deployments. | none                | none |
