...
|          | --compactProcessesStrategy <!-- genesisInstall --compactProcessesStrategy -->  | no        | The strategy to be used when `compactProcesses` is set; defaults to `DEFAULT`; valid values are: `DEFAULT`, `DEV`. WARNING: the DEV strategy optimises (and sometimes removes) processes to provide the optimal developer experience with the fewest possible resources; **DO NOT use this for production deployments** | none                | none |
