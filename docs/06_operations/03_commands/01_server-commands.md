...
|          | --compactProcessesStrategy <!-- genesisInstall --compactProcessesStrategy -->  | no        | The strategy to be used when `compactProcess` is set. Defaults to `DEFAULT`, valid values are: `DEFAULT`, `DEV`. WARNING: When using the DEV strategy, processes will be optimised (and sometimes removed); this provides the optimal developer experience with the fewest possible resources. **DO NOT use this for production deployments**. | none                | none |
