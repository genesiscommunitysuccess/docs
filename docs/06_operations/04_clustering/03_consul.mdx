...
Scenario: This might happen when a resource is moved from one process to another. For example, a `REQ_REP` originally served from `Service A` is now moved to `Service B` and consul still holds a reference that is being served by the former.
