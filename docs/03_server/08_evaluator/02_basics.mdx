...
For example, if you have an Event Handler that inserts a [Notify](../../../components/notifications/notifications-server/notify-configuring-runtime/) email message, you could set up the Event Handler in this way in the _application_**-script-config\src\main\resources\scripts\application-eventhandler.kts** file:
...
```
For the example above to work, you need to set up a [Notify](../../../components/notifications/notifications-server/notify-configuring-runtime/) process together with inserting a GATEWAY and a NOTIFY_ROUTE in the database and add the details of the connection for the SMTP server to the **genesis-system-definition.kts** file.
You can see an example of a dynamic rule using [Notify](../../../components/notifications/notifications-server/email/) email messages being set up in our [tutorial](../../../getting-started/developer-training/training-content-day5/#cron-rules-static-events).
## Static rules
