...
Sync works similarly to [Async](../../../server/api-reference/event-handler-api/#blocking) and [Rx3](../../../server/api-reference/event-handler-api/#rx3), but in this case, there is no `Single<O>` returned and no `suspend` modifier used for Kotlin coroutines. The expected output of the Event Handler logic is just the `O` type.
