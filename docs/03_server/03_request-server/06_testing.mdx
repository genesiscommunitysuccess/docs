...
This is the request server we will use in this example
...
we need to inject a request server client, and define our workflow
...
We need the client to communicate with our request server. 
We need to define our workflow, so that the client knows both the request and the reply type.

### Define a request reply workflow

The syntax for defining the flow is slightly different between Kotlin and Java. 
However, in both instances you will need to provide the input type, the output type and the request name.

In kotlin we can build the flow in a method call

```kotlin 
private val helloWorldFlow = requestReplyWorkflowBuilder<Hello, World>("HELLO_WORLD")
```

However in Java we will need to construct an abstract class;
...
To test dynamic authorisation, we need to amend our request server definition:
...
After we updated that, we can create our new test cases.
Please note that the authorisation for request servers is on the outgoing message.
The request server will filter out any values to which the requesting user has no access to. 
...
As you can see, in our not authorised test, we verify that no results were returned. 
...
At this point we have tested our very simple request server. 
We haven't even had to sue the database!
However, we have covered the baasics of communitation between tests and request servers.

We have covered:
* defining a request flows
* send our request and receive our response
* set the username on the request
