...
Below is an example of a **reqrep.kts** file where the single `requestReply` code block includes a `filter` clause. You can find out more about `filter` clauses on the [Basics](/server/request-server/basics/#filter-block) page.
