...
When you use [Custom Request Servers](#custom-request-servers), permissioning is different; it is similar to permissioning for Event Handlers.
...
- *reply*: Expects a type of `Flow` to be returned. So it can be used with [`getBulk`](../../../database/database-interface/entitydb#getbulk) and [`getRange`](../../../database/database-interface/entitydb#getrange)
- *replySingle*: Expects a return in type of `<output class>` you defined your request server. 
- *replyList*: Expects a return in type of `List<E>`
