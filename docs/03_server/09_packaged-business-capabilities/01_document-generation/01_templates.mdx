...
You can either:
- `generateContent`: generate the raw content of the file (not supported for PDF files), this will be returned alongside any corresponding AssetIds
- `generateAndStore`: generate a file based on the provided template, this will be uploaded to FILE_STORAGE and the corresponding FileStorageID will be returned
...
When an HTML template is provided, Thymeleaf is used to output the processed text.
Thymeleaf has the capability of processing complex data objects, as a result the HTML template can include for loops as shown below. (See [Thymeleaf documentation](https://www.thymeleaf.org/documentation.html) for more details.)
...
On the other hand, when a TXT template is provided, the TXT template engine uses a simple substitution syntax and thus requires each field to be added individually.
It's helpful that the TXT engine gets system definition keys as well as any environment variables as part of it's data context automatically.
