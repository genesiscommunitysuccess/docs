...
- An .html file creates either a .html file from the data retrieved from the database, or, if specified, generates a .pdf file based on a .html file of the data retrieved.

Templates must be pre-uploaded to FILE_STORAGE (see Document Management for more information on uploading/downloading files). If any other assets are required (images, css files etc.), these must also be uploaded to FILE_STORAGE, along with a TEMPLATE_ASSET table that maps the assets to the corresponding template.

## Using the Document Generator

There are two methods for generating documents:

- `generateContent` generates the raw content of the file, which is returned alongside any corresponding AssetIds. This can be used for generating .txt and .html files, but not .pdf files.
...
For example, this component can be part of an HTML template file:
