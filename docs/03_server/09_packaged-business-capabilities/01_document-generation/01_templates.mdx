...
- An .html file creates either a .html file from the data retrieved from the database, or, if specified, generates a .pdf file based on an .html file of the data retrieved.
...
This example injects a Document Generator into an [Event Handler](../../../../server/event-handler/basics) file. It generates and stores a pdf file (so you must have uploaded a template in HTML format) for use with an email. 
