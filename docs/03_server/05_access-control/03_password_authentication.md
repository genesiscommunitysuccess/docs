...
* `refreshTokenExpirationMins` specifies a time out for the refresh token value provided on successful login. One refresh token is associated to one user session in a 1-to-1 relationship basis and its value can be used to create a new user session after the session token has expired. A refresh token can't be used to create a new user session after its expiration. Default: 7200 minutes (i.e. 5 days)
