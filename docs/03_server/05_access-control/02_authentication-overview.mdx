...
Each of these requires its own configuration settings in the application's **auth-preferences.kts** file, which is created by the build process. To change the default **auth-preferences.kts**, create the details in a new **auth-preferences.kts** file and replace the existing file in: 
