...
There are many different ways for your application to perform authentication. Each method requires the implementation of an 'Authenticator'. The authenticators offered by the Genesis Application Platform are:
...
Each of these requires its own configuration settings in the application's `auth-preferences.kts` file. To change copy it to the `scripts` folder of your application.
...
SSO authentication enables users to use a single set of credentials to access a range of applications, including those built on the Genesis application platform. For more information on SSO technology, see the [Single-sign on Wikipedia page](https://en.wikipedia.org/wiki/Single_sign-on).
