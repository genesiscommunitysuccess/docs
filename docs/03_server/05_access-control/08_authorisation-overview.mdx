...
Additionally, it is possible to use the `authKeyWithUserName` block to access the username value associated to the original request. This can be helpful if your AuthCache is modeled using an underlying entity/user mapping table: one unique entry of entity-id and username per record.
