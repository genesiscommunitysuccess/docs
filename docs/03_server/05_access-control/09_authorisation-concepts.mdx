...
The reference `data.counterpartyId` in the auth snippet above refers to the COUNTERPARTY_ID field from the POSITION object that is supplied as a parameter to the resource request. This value is used as a key into a specific AuthCache, identified by entity name. In this example, that is "ENTITY_VISIBILITY"

Each AuthCache will map Entity Id (in our example, COUNTERPARTY_ID from the POSITION object) to a Set of users.

The `mapName` parameter refers to a specific permission entity defined on the server via the [auth-permission.xml](../../../server/access-control/authorisation/#defining-a-permission-rule-using-groovy-and-xml) file. For example:
...
In cases where you do not require the auth map and just want to define a `filter` clause, you can omit it from the definition. The relevant authorisation data (table/view/custom class) will be available as part of the `data` context property. For example:
...
If the username value as part of the `filter` block is required, `filterWithUserName` should be used instead. This will expose the `userName` context property as well as the `data` property.
...
Additionally, you can use the `authKeyWithUserName` block to access the username value associated with the original request. This is helpful where your AuthCache is modelled using an underlying entity/user mapping table: one unique entry of entity-id and username per record.
...
Here is a simple example of a dataserver block:
...
To fully activate Generic permissions, you need to add the following values to your application's [system definition file](/server/configuring-runtime/system-definitions/) (for example **alpha-system-definition.kts`) before you run `genesisInstall`.
