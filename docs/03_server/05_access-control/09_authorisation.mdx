...
:::warning
The back end infra-structure utilizes ChronicleMap to store and serve real time up-to-date dynamic authorizations, that means the application must know ahead how many entries are required to be stored on each Auth Map.
These are defined using the correct combination of `averageEntityChars`, `averageUserNameChars` and `averageUsers` which are referenced in the next sections of this document.
For more technical details, check [Configure Entries](https://github.com/OpenHFT/Chronicle-Map/blob/ea/docs/CM_Tutorial.adoc#configure-entries) from ChronicleMap docs.
:::
