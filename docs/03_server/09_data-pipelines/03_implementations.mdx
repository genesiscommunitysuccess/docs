...
This is a real-time acknowledgement source that creates a JMS consumer and polls a message queue. It returns a flow of Message objects and
the consumer only acknowledges the message has been received once the pipeline confirms it has been processed by the sink successfully.
If the source is unable to connect to the message queue, the process running the pipeline will enter a WARNING state.
The message displayed next to this state will include a monitor name, you have the option to configure this as shown below otherwise a default one will be generated based on the queue name.
If you are providing your own monitor name, ensure this is unique across all JMS Sources + Sinks for that process.
...
- queueName (required): the name of the message queue you would like to consumer from
...
- monitorName (optional): name to be displayed when process state is set to WARNING, default name will be of the format: JMSSourceMonitor-0-{QUEUE_NAME}
...
The type specified when instantiating the sink refer to the type of element that the sink can expect to receive, as part of the configuration you must provide a valid lambda to convert this element type to a valid Message object to send to the message queue.
...
- monitorName (optional): name to be displayed when process state is set to WARNING, default name will be of the format: JMSSinkMonitor-0-{QUEUE_NAME}
