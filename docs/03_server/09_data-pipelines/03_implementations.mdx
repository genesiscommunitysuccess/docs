...
A location or a route builder must be provided. The location must be provided upon instantiation of this source, as shown below.
...
Alternatiely, you can instantiate a camel route builder directly  
```kotlin
    pipeline("TEST_CAMEL_ROUTE_PIPELINE") {
        source(
            camelFileSource {
