...
Passing in the location directly will automatically create a route builder and adds it to the intenral pipeline processors:
```kotlin
...
First off all, lets explain what a route builder is. In this context, it is Apache Camels API for defining where and how to read messages.
...
Here, the processor emits messages directly in to the intenral pipeline processors within the scope of the route builder block:
```kotlin
    pipeline("TEST_CAMEL_ROUTE_PIPELINE") {
        source(
            camelFileSource {
