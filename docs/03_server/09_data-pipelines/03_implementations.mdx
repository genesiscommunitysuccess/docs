...
You can also enable Camel Idempotent Repositories for filtering out duplicate messages.
    ...
    pipeline("TEST_CAMEL_ROUTE_PIPELINE") {
        source(
            camelSource {
                ...
                idempotentConfig = inMemoryIdempotentConfig {
                    cacheSize = 300000
                }
            }
        ).sink(filesSink)
...
```kotlin
    pipeline("TEST_CAMEL_ROUTE_PIPELINE") {
        source(
            camelSource {
                location = "file://src/test/resources/camel-source-test?noop=true&idempotent=true"
                idempotentConfig = fileIdempotentConfig {
                    consumedLocation = ""
                    cacheSize = 300000
                    maxFileStoreSize = 15000000
                }
            }
        ).sink(filesSink)
    }
