...
Also, within the routeBuilder block you can always access the dataPipelineProcessor variable, this is a custom processor we create that will emit the messages from the data pipeline source into your gensis pipeline.
```kotlin
    pipeline("TEST_CAMEL_ROUTE_PIPELINE") {
        source(
            camelFileSource {
