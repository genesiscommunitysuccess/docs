...
This hierarchy enables you to specify multiple environments where your application can be deployed. You can specify separate DEV and UAT environments, for example.
...
If you want to override any of the config settings for a deployment (for example, if you want to use a different database in a specific environment), create a new `system` block in your application's **-system-definition.xls** with the required settings for the environment. 
...
To see the changes, you need to deploy your application. If you are using Genesis Start or the IntelliJ plugin, you can see these files under  **server/**_appname-_**app/build/genesis-home/site-specific**.
