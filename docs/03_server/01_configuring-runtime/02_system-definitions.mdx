...
**WriteClasspathToManifest**: When set to `true`, the classpath for each process will be written to the Manifest of the corresponding Jar file in **generated/classpath** (e.g. **GENESIS_CLUSTER.jar**). By default, the classpaths are written to an environment variable `CLASSPATH`, which has a character limit that can be hit. Defaults to false.
