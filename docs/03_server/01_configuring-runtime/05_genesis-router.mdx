...
`dataserverPollingTimeout` This setting contains the timeout to expire inactive dataserver subscriptions in seconds. Subscription inactivity is determined by the lack of HTTP requests from the client targeting a specific dataserver subscription. Default value is 60 seconds.
