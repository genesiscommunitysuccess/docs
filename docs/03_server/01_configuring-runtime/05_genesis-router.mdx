...
* `CLOSE` - This causes the client connection to be closed if the channel buffer becomes full. This could happen with slow consumers, and can help protect the ROUTER process rather than causing a large backlog of messages, which consumes memory. In the worst case, this could cause an Out Of Memory error for the ROUTER process.

To configure client handling, open a `clientHandler` block. There is an example of this in the main example at the top of the page.
