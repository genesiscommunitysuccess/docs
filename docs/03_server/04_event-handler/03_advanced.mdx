...
The code above assumes you have built your fields and tables correctly. If intelliJ can't find your `Trade`, go back and [build your fields and tables](../../../getting-started/learn-the-basics/data-model/).

:::info
When using custom reply messages, the 'onValidate' codeblock is required.
:::

The response from the server also changes when using custom reply message, so the client-side of your application needs to be aware of that. When using the default eventhandler you would get a response from the server like this in case of an `ack()`:
