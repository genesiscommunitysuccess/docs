...
In an `eventhandler`, you can specify custom reply message types to be returned instead of the default `ack()` or `validationAck()` functions. This gives you more flexibility. 

In the example below, we define a custom message type called `TradeEvent`:
...
The `init` codeblock is used to initialise an object. When this class is called, it validates if `price` and `quantity` are greater than zero. If not, it throws an `IllegalArgumentException`, which can be handled in the [onException code block](#onException).

Here is an example of a custom message reply type called `CustomTradeEventReply`:
...
For the example above, add `CustomTradeEventReply` under _app-name_**-messages** and assemble. Once you have built, add `api(project("{app-name}-messages"))` to your **build.gradle.kts** file under **{app-name}-script-config/build.gradle.kts**.
...
The following code assumes you have built your fields and tables after you created your `TradeEvent` under _app-name_**-config** with a primary key of `tradeId`. If intelliJ can't find your `TradeEvent`, go back and [build your fields and tables](../../../getting-started/learn-the-basics/data-model/).

:::info
When using custom reply messages, the 'onValidate' codeblock is required.
:::
...
Because one of the arguments of the class you created was `tradeId`, and the type of the reply message was: `TradeEventAck`.
