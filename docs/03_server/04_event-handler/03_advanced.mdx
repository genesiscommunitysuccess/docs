...
The `init` codeblock is a special block that is used to initialize an object. So when this class is called, it will validate if `price` and `quantity` are grater than zero.
If not, it will throw an `IllegalArgumentException` which can be handled in the [onException code block](#onException).
...
When using custom reply messages, you are not allowed to use the default `ack()` and `nack()` replies. Because of that, it is important to create a logic that
:::info
When using custom reply messages, the onValidate codeblock becomes required.
:::

The response from the server also change when using custom reply message also change, so you the client-side of your application needs to be aware of that. When using adefault eventhandler
you would get a response from the server like this in case of an `ack()`:
