...
In this document we will cover the basics of testing event handlers. 
We will use a very simple example, and work through the communitation between our test and our event handler. 
Including how to test dynamic authorisation. 
...
This is the event handler we will use in this example: 
...
Here we: 
* Enable **GenesisJunit**
* Set the event handler script that we want to test, using the **ScriptFile** annotation
...
Before we are ready to begin writing tests for our event handler,
we need to inject a event handler client:
...
Let's first write a test to make sure that event handler returns an ack.
...
* EventClientSync - this is the synchronous client, all calls are blockign
...
Use the other clients, if your tests uses other asynchronous operations.
...
The revoke calls are not needed in this case, and just added to demonstrate syntax. 
These calls can be used to revoke previous authorisations in our tests.

### Conclusion

At this point we have tested our very simple event handler. 
We haven't even had to use the database!
However, we have covered the basics of communitation between tests and event handlers.

We have covered: 
* send details to our event
* overwrite the default message type
* set the user on our event
* handle dynamic authorisation

For more details about testing with Genesis, please have a look at our [integration test documentation](/operations/testing/integration-testing)
