...
- For this example, the file defines a single eventHandler of type `<Counterparty>`. It is an object that was created from the COUNTERPARTY table in the schema. The object defines the input that should be expected, such as Counterparty, Name and Counterparty ID.
- The `eventHandler` performs a single action - it inserts a counterparty into the database, using the [entityDb](../../../database/database-interface/entity-db/).
