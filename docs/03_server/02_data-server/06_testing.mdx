...
When testing against previous versions of the framework, please see [here](./#integration-testing-legacy)
...
Create a CSV file called **seed-data.csv** in the root of the resources folder for your tests. Add the data below tot his file.
...
As you can see here, to connect to our Data Server, we need to provide the datasource name, as well as the type or class that the query returns.
...
Let's amend our Data Server and enable authorisation:
...
Once you add the `auth` block in our code, we need to amend the test.
