...
Consolidators support multiple `groupBy` statements in a single consolidation. 
This functionality is useful in case you want to consolidate the same table at multiple levels, 
and it will ensure the consolidation is consistant across multiple level.
If we look at this consolidator for exmaple:
...
We can see that it consolidates both on order year and and on order year and month. 
