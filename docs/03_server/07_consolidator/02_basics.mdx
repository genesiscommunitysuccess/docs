...
Alternatively, you can enable `--coldStart` persistently by adding it to the `<arguments></arguments>` option. More info can be found in [configuring runtime](../configuring-runtime/#arguments-tag)
