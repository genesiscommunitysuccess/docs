...
You can also combine compatible services like (like DataServer, RequestServer, Notify, EventHandler, Streamer) into a single process, called GENESIS_COMPACT_PROCESS, reducing the number of services running
by selecting the option `Enable compact processes` from the plugin, as shown below
