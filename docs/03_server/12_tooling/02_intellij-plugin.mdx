...
:::tip
If the tool window does not appear in a Genesis project, please have a look at the [troubleshooting](#troubleshooting) section below.
:::
...
:::tip
If you have issues starting services, please have a look at the [troubleshooting](#troubleshooting) section below.
:::
...
### Making sure you have opened the correct folder
...
Go the Project Structure from the IntelliJ menu click on **file** and then **project structure**.
...
Open your IntelliJ Settings, and search for **gradle** in the search box.
There is a **Gradle JVM** setting, make sure this is set to **Project SDK**:

![Gradle Settings](/img/intellij-plugin/intellij-settings-gradle.png)

### Process fails to start, shows DOWN, and log exception says address already in use

```log
zeroMQ failed to bind socket to address 'tcp://*:18001', exiting due to critical error.
```

This indicates that 18001 port on your local machine is in use.
Most of the time when this happens, it was due to an error in the process and it not “giving up” the port upon being stopped.
We can stop it happening running:

#### MacOs remedy

1. netstat -vanp tcp | grep **affected port**

This will output anything associated with the port, look at the LISTEN line
