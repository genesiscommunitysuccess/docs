...
A pipeline configuration contains a collection of `sources`, one or many `operation` functions and one or many `sink` functions.
...
This is only relevant to [databases that support Entity DB transactions](../../../02_database/05_database-interface/01_entitydb.mdx#transactions).
