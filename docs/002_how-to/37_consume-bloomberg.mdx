...
Bloomberg B-PIPE is a normalized and consolidated market data feed for financial markets pricing and reference data. The Genesis platform allows you to easily integrate with B-PIPE and consume this data into your application.
...
The example contains a second pipeline that listens to data updates coming from the data subscrptions. Whenever an update is received, it it updates the database, which updates the views which drive the real-time UI grids.
...
To test the set-up of a Data Pipeline for Bloomberg on your app:

- *Please note that you will need access to a real Bloomberg B-PIPE or SAPI endpoint in order for this how-to guide to run correctly.*
