...
1. Go to the file `OTE\client\src\routes\processes\process-history-manager\process-history.ts` file.<br></br> Find the code below:
...
... and replace it with the following, which will make the button specifically call the relevant event in the eventHandler:
...
2. Add the action buttons that enable the user to trigger the processes. Find the line `></entity-management>` and replace the line with the below:
...
When you build and run again, the buttons are now displayed at the top right of the menu for the component. The user can click on a button to trigger the relevant event.
