...
There are a few authentication setups which can be configured for Genesis applications:
...
Larger organizations will typically require [LDAP](#ldap-authentication) or [SSO](#sso-authentication) setups, so that:
...
- `{{TIMEOUT}}` the [`timeoutInMinutes`](#timeoutinminutes) configuration setting.
...
A genesis UI is configured by default to perform a [refresh login](#event_login_refresh) upon expiry to avoid users needing to manually re-authenticate.
...
* [JWT (JSON Web Token) SSO](https://jwt.io/introduction) 
