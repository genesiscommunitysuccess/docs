...
You can add modifiers for fields to cover scenarios such as where:

- the field is derived and should not be modified by the end user, only by the system itself.
- the field should only be set on insert, and never changed on modifications.
...
| `.applicationProvided()` | _any_            | The field must be provided programmatically in the event handler, no user input from the front end |
