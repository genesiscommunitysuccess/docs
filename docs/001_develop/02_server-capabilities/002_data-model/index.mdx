...
The data model is a fundamental part of any Genesis application. To create a data model, you define the [entities (tables)](#tables) that store the application's data. You also define [views](#views), which join related entities together to provide useful views of the data. For example, it makes sense to have separate tables for counterparties, instruments and exchanges, but if you want to look at the trades or the orders in the system, you probably want to see some of these things in a single grid of data for each trade or order you make. For this, you need a view. 

The application's tables and views power most Genesis Server and Client services and components by making them aware of the application's data structure, and by notifying them of relevant changes to the data.
...
A table is made up of one or more fields. Each field must have a unique name (unique within the table definition) and a data type. The following data types are available

###### `String`
String fields can contain any set of characters up to the maximum length of characters specified for the field. 

In the second example belowT, the value in the parenthesis is the maximum number of characters that can be stored in this field for a given record. If you do not specify this value, the default length is 64 characters.
