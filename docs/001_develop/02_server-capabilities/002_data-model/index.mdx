...
The data model is a fundamental part of any Genesis Application. Within the data model we define the [entities (tables)](#tables) relevant to the application which will store the application's data. We also define [views](#views) which join related entities together to represent wholistic views onto the applications dataset. The application's tables and views power most Genesis Server and Client services and components by making them aware of the application's data structure, and by notifying them of relevant changes to the data.

The data model of a Genesis Application is defined in `*-dictionary.kts` files.

## Tables

Tables represent sets of records for each of the entities in an application. 
