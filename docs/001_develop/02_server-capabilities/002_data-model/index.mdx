...
The data model is a fundamental part of any Genesis Application. It defines the entities (tables) relevant to the application, including their associated attributes. These table structures define the data that is to be entered and persisted within the application.

The data model of a Genesis Application is defined in `*-dictionary.kts` files.

## Tables

Tables store records for each entity defined in your application. 
Each table is defined with a set of fields (entity attributes) which make up a record for that particular entity. 

Tables and their fields are defined in  `*-tables-dictionary.kts` files, they need to be defined in the `cfg` folder in your application's server codebase.
...
Below shows an example table definition:
...
tables {
    table(name = "ORDER", id = 11_000, audit = details(id = 11_500, sequence = "OA")) {
        ...
        indices {
            nonUnique("COUNTERPARTY_ID").name("ORDER_BY_COUNTERPARTY_ID")
            nonUnique("INSTRUMENT_ID").name("ORDER_BY_INSTRUMENT_ID")
        }
    }
}
```

### Table and entity terminology

The following table gives an overview of terms for table and entity used in Genesis, in relation to the example above.
...
Views are used to define a single entity representing data across tables.
...
- Root table is called `TRADE`
- The `TRADE` table references both `INSTRUMENT` and `COUNTERPARTY` tables via their respective ID fields `INSTRUMENT_ID` and `COUNTERPARTY_ID`

You could then define a TRADE_VIEW as follows, which pulls in INSTRUMENT_NAME, CTPTY_CODE and CTPTY_NAME fields from those joined tables
...
This single view of data, containing field values across multiple joined tables and derived values, can then be used to power client queries, both [snapshot](/develop/server-capabilities/snapshot-queries-request-server/) and [real-time](/develop/server-capabilities/real-time-queries-data-server/), which in turn can power [grids](/develop/client-capabilities/grids/), [charts](/develop/client-capabilities/charts/) and other data visualisation web client components. 

Views are also be used to power server server capabilities such as [real-time aggregation](/develop/server-capabilities/real-time-aggregation-consolidator/), [real-time triggers](/develop/server-capabilities/real-time-triggers-evaluator/) and outbound integrations. 
...
views {
  ...
  view("TRADE_VIEW", TRADE) {
    ...
    joins {
      joining(INSTRUMEN, backwardsJoin = true) {
        on(TRADE.INSTRUMENT_ID to INSTRUMENT { INSTRUMENT_ID })
      }
    }
...
* The first view is called TRADE_VIEW. It joins the TRADE table to the COUNTERPARTY table and to the INSTRUMENT table. This view also includes a [derived field](#derived fields) called NOTIONAL.
* The second view is called POSITION_VIEW. Its root table is the POSITION table. There are [backwards joins](/database/fields-tables-views/views/views-basics/#backwards-joins) to the ALT_INSTRUMENT_ID and INSTRUMENT_L1_PRICE tables which allow real-time updates from all the fields to trigger updates. Separately, there is a join from the root table to the INSTRUMENT table. This view also includes two [derived fields](/database/fields-tables-views/views/views-advanced/#derived-fields): VALUE and PNL.
