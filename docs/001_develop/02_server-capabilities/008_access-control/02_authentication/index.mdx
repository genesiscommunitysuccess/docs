...
| privateKeyJwt | Contains the details of authenticating client using privet key JWT | Yes, when using certificates for SSO login | No default value | Data class |

Follow the link for more details on [Private Key JWT](#private-key-jwt)
...
| `previousPublicKey` | No | `null` | PEM-encoded previous public key. Set during key rotation to keep the old key available in JWKS |
...
JWKS publication is controlled inside the `privateKeyJwt` block of an OIDC identity provider definition.
...
IDPs cache JWKS responses and may continue issuing tokens signed with an old key for some time after a key change. Setting `previousPublicKey` keeps the old public key in the JWKS response during the transition so those tokens remain verifiable.
...
privateKeyJwt {
    ...
    previousPublicKey = "<old public key>"   // keeps old key in JWKS
}
```

The JWKS endpoint now serves two entries — one for each thumbprint. The IDP can pick up the new key while still accepting tokens signed with the old one.

**Step 3 — wait for the transition window to close**

Remove `previousPublicKey` once all tokens signed with the old key have expired:

```
wait = IDP JWKS cache TTL + IDP token max lifetime
```

Both values come from the **IDP's configuration** (Okta, Azure AD, Keycloak, etc.), not from Genesis:

- **JWKS cache TTL** — how long the IDP caches Genesis's `/oidc/jwks` response before re-fetching it (commonly 1–24 h; check the IDP admin console)
- **Token max lifetime** — the maximum lifetime of access or refresh tokens the IDP issued while the old key was active

A conservative default is **24 hours**. Most IDP admin consoles provide a "force JWKS refresh" option to shorten this.

> **Note:** `tokenLifeInSeconds` in the Genesis OIDC script is unrelated — it controls how long Genesis keeps its internal `SsoToken` row alive after a successful IDP callback, not the lifetime of IDP-issued tokens.

**Step 4 — deploy the clean config**

```kotlin
privateKeyJwt {
    privateKey = "<new private key>"
    publicKey  = "<new public key>"
    // previousPublicKey removed
}
```

The JWKS response shrinks back to one entry.
