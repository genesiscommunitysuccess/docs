...
When the maximum number of sessions is reached, the system will provide host/IP information for each active session to help the user identify which sessions they may want to log out from.
...
### `hideHostsFromMaxActiveSessionsReachedMessage`

Specifies whether to hide host/IP information from the user when they have reached the maximum number of concurrent sessions. When set to `true`, no host or IP information will be sent back to the user once they have reached the `maxSimultaneousUserLogins` number of sessions.

By default, this is set to `false`, which means the host/IP information is sent back to the client. This information can be used by the user to select a specific session to log out from so that a new session can be created.

```kotlin
security {
  hideHostsFromMaxActiveSessionsReachedMessage = true
}
```
...
##### Reading a user's notify-based MFA status

The `ALL_USERS` data server query carries the status as an `MFA_ENABLED` field, alongside the rest of the user record. A user management screen can show a column for it and drive the events above from the selected row, without querying `NOTIFY_BASED_MFA_STATUS` itself.

A user with no `NOTIFY_BASED_MFA_STATUS` record reads as `false`. The field updates on an existing subscription as the record is written or changed, so a grid reflects a toggle without resubscribing.

The field also supports `CRITERIA_MATCH`, so a grid can be filtered down to the users who have it enabled:
...
To enable SAML on the Genesis Router process, change the `GENESIS_ROUTER` process configuration in  `genesis-processes.xml`. If this file is not already in your project's `cfg` directory, follow our guidance on [overriding platform-provided files](/develop/server-capabilities/runtime-configuration/site-specific/#overriding-platform-provided-files)
  ...
  identityProvider("genesis") {
    ...
    onFirstLogin {
      // optional -> should return a User and it UserAttributes from the SamlResponse
        createUser { samlResponse ->
            val userNameResponse = samlResponse.nameIdData["mail"] ?: "defaultUser"
            val user = User {
...
| tokenLifeInSeconds | The life time of the issued SSO_TOKEN. | No | 50 minutes | Int |
...
| secret | The client secret provided by the OIDC Provider when application was registered | Yes, when using client-secret for client authentication | No default value | String |
| privateKeyJwt | Contains the details of authenticating client using [Private Key JWT](#private-key-jwt) | Yes, when using certificates for SSO login | No default value | Data class |
...
`PRIVATE_KEY_JWT` is an OIDC client authentication method defined in [RFC 7523](https://www.rfc-editor.org/rfc/rfc7523). Instead of sending a client secret, Genesis proves its identity to the IDP by presenting a short-lived JWT signed with a private key. The IDP verifies the signature using the matching public key, which it fetches from Genesis's [JWKS endpoint](#jwks-endpoint).

This is stronger than `client_secret` authentication because:
- The private key never leaves Genesis — nothing sensitive is transmitted
- Each assertion is single-use (unique `jti`) with a short TTL
- Compromise of the IDP's side does not expose the client credential

---

At the token exchange step, Genesis constructs and signs a **client assertion JWT** instead of sending a `client_secret`.
The JWT is signed with the configured private key. The `kid` header is included only when `useJwksUrl = true` (which requires `publicKey` to be set), allowing the IDP to look up the correct key in the JWKS when multiple keys are present (e.g. during rotation). If `useJwksUrl = false`, `kid` is omitted from the assertion header regardless of whether `publicKey` is configured.

Each `privateKeyJwt` configuration has the following properties:

| Field | Required | Default | Description |
|-------|----------|---------|-------------|
| `privateKey` | Yes | — | PEM-encoded PKCS#8 private key used to sign client assertions |
| `publicKey` | No | `null` | PEM-encoded public key. Enables JWKS publication and embeds the RFC 7638 thumbprint as `kid` in assertions |
| `incomingPublicKey` | No | `null` | It holds a second PEM-encoded public key. When set, this key is published alongside `publicKey` in the JWKS endpoint (`/oidc/jwks`) so the IDP has both keys available in its cache during a key rotation. |
| `keyAlgorithm` | No | `RSA` | Signing algorithm family — see table below |
| `clientAssertionTtlSeconds` | No | `300` | How long each assertion JWT is valid. Should be short (≤ 300 s) |
| `useJwksUrl` | No | `true` when `publicKey` is set | Controls both JWKS publication and `kid` inclusion. When `false`, neither the JWKS endpoint nor the `kid` header is used even if `publicKey` is set |

Set `useJwksUrl` to `false` when you want to use public key configured on IDP instead of using JWKs endpoint

###### Signing algorithms

| `keyAlgorithm` | JWT `alg` | Key type |
|----------------|-----------|----------|
| `RSA` | RS256 | RSA 2048-bit minimum |
| `PS` | PS256 | RSA 2048-bit minimum (PSS padding) |
| `EC` | ES256 / ES384 / ES512 | EC P-256, P-384, P-521 |
...
When `PRIVATE_KEY_JWT` is the configured grant type for an OIDC identity provider, Genesis exposes a JSON Web Key Set (JWKS) endpoint so the IDP can verify client assertions signed by Genesis.

**URL:** `GET /oidc/jwks`

Authentication is not required. The response is a standard RFC 7517 JWKS document:
...
Each entry in `keys` corresponds to one active public key. During key rotation there will be two entries — the current key and the previous key.

---

###### Configuration

JWKS publication is controlled inside the `privateKeyJwt` block of an OIDC identity provider definition. You need to set public key, when setting the `publicKey` configuration property, the `useJwksUrl` flag will default to true.
...
identityProvider("my-idp") {
    ...
    client {
        id = "my-client-id"
        privateKeyJwt {
            privateKey       = systemDefinition.getItem("PRIVATE_KEY")
            publicKey        = systemDefinition.getItem("PUBLIC_KEY")   // enables JWKS
            keyAlgorithm     = RSA                                       // RSA | EC | PS | EdDSA
            clientAssertionTtlSeconds = 300
            useJwksUrl       = true                                      // `true` by default if publicKey has been set
        }
    }
    // ...
}
...
When rotating the key pair used for `PRIVATE_KEY_JWT` client authentication, Genesis must ensure the IDP can verify client assertions throughout the transition.

Genesis sets `Cache-Control: public, max-age=3600` on the `/oidc/jwks` response, so the worst-case IDP cache TTL is **1 hour**.

---

###### How `incomingPublicKey` works

`PrivateKeyJwtClient` has two public key fields:

| Field | Role |
|---|---|
| `publicKey` | The key currently in use for signing assertions. Its RFC 7638 thumbprint becomes the `kid` in every assertion |
| `incomingPublicKey` | A second public key served in JWKS alongside `publicKey`. Used to pre-load an upcoming key into the IDP's cache before switching the private key |

Both keys appear in the JWKS response. The `kid` in every assertion always matches `publicKey` — `incomingPublicKey` does not affect signing.

---

###### Two-step rotation (recommended)

This procedure pre-loads the new public key into the IDP's JWKS cache **before** switching the private key, eliminating any failure window.

###### Step 1 — Add the new public key to JWKS while still signing with the old private key
...
Deploy with the new public key as `incomingPublicKey`:

```kotlin
privateKeyJwt {
    privateKey       = "CURRENT_PRIVATE_KEY"   // old key — still signing
    publicKey        = "CURRENT_PUBLIC_KEY"   // kid = thumbprint-A
    incomingPublicKey = "NEW_PUBLIC_KEY"      // thumbprint-B pre-loaded into JWKS
}
```

JWKS now serves two entries — thumbprint-A (current) and thumbprint-B (incoming). All assertions still use `kid = thumbprint-A`. The IDP picks up thumbprint-B on its next natural JWKS refresh.

**Wait at least 1 hour** for the IDP's JWKS cache to expire and refresh.

###### Step 2 — Switch to the new private key

```kotlin
privateKeyJwt {
    privateKey = "NEW_PRIVATE_KEY"    // new key — now signing
    publicKey  = "NEW_PUBLIC_KEY"     // kid = thumbprint-B
    // incomingPublicKey removed
}
```

The IDP already has thumbprint-B in its cache from step 1. Assertions now carry `kid = thumbprint-B`, which the IDP can verify immediately with no failure window.

---

###### One-step rotation (IDP re-fetches on unknown `kid`)

If the IDP re-fetches its JWKS cache when it encounters an unknown `kid` (Okta, Azure AD, Auth0 typically do this), you can rotate in a single deployment:

```kotlin
privateKeyJwt {
    privateKey = "NEW_PRIVATE_KEY"
    publicKey  = "NEW_PUBLIC_KEY"
    // no incomingPublicKey needed
}
```

The IDP encounters `kid = thumbprint-B` for the first time, re-fetches `/oidc/jwks`, and finds it. No failure window for compliant IDPs.

If you are unsure whether your IDP re-fetches on unknown `kid`, use the two-step procedure — it is safe for all IDPs.

---

###### Difference from `AUTHORIZATION_CODE`

| | `AUTHORIZATION_CODE` | `PRIVATE_KEY_JWT` |
|---|---|---|
| Client credential | `client_secret` (shared secret) | Signed JWT (asymmetric) |
| Credential transmitted | Yes — secret sent to IDP | No — only a signature |
| Rotation | Coordinate new secret with IDP | Independent key rotation via JWKS |
| IDP config | Secret registered directly | JWKS URI registered |
| Replay protection | None beyond TLS | `jti` claim + short TTL |
...
###### Configuration for private_key_JWT
...
identityProvider("my-idp") {
    ...
    client {
        ...
        privateKeyJwt {
            privateKey                = systemDefinition.getItem("OIDC_PRIVATE_KEY")
            publicKey                 = systemDefinition.getItem("OIDC_PUBLIC_KEY")  // optional, enables JWKS + kid
            keyAlgorithm              = RSA    // RSA | PS | EC | EdDSA
            clientAssertionTtlSeconds = 300
        }
    }
    ...
    config {
        endpoints {
            ...
            authorization = "https://idp.example.com/oauth2/authorize"
        }
    }
...
#### Enabling users for OIDC

To enable users to be able to sign in using OIDC, you must add them to the `USER`, `USER_ATTRIBUTES` and `SSO_USER` tables within your Genesis application.

In the `SSO_USER` table:

* `SSO_METHOD` must be set to OIDC
* `SSO_SOURCE` must be set to the identity provider name defined in the **oidc-config.kts** file

#### Enable SSO on auth-preferences
...
- `autoAuth : true` - When this is set to true, any users who have an active authenticated session will be logged in automatically. This is not mandatory, but is essential if you want sso to function seamlessly. 
- `sso:` 
    - `toggled: true` - This ensures that the `Enable SSO` checkbox displayed on the application's login page is **checked** by default. Where this is set to `false`, the checkbox is **not** checked by default.
    - `identityProvidersPath: 'sso/list'` - This is the endpoint that the UI will call to get a list of identity providers from the server. `sso/list` is the default value for `ssoListEndpoint` configured on the server. Where this value has been changed, it must also match in this configuration.
...
* `maximumLength` specifies the maximum length of password. If null or undefined, this assumes there is no maximum limit. Default: null.
* `minDigits` specifies the minimum number of numeric digits required in a password. If null or undefined, this assumes there is no minimum limit. Default: null.
* `maxRepeatCharacters` specifies the maximum number of the
