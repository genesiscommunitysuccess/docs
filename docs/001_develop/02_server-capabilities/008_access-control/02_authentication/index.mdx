...
- `autoAuth : true` - This is not mandatory, but is typically set when setting up sso. When this is true, the app automatically logs in users with an active authenticated session.
- `sso:` 
    - `toggled: true` - This ensures that the `Enable SSO` checkbox displayed on the application's login page is checked by default. If you set this to `false`, it will not be checked by default.
