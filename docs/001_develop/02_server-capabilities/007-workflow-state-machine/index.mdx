...
Almost every application has workflows where work items move from one state to another, such as *new*, *amended* and *completed*. It is difficult to handle the transitions from one state to another simply using the standard database operations - insert, modify and delete.

The State Machine provides extensions to the Event Handler that make it easy to define and code these transitions and their validations.  
