...
Most applications are focused on trades and orders. And virtually every type of trade and order has its own lifecycle, which tracks it through various states.

The State Machine enables you to specify those states and the workflow from one state to another. 
...
To achieve this, you can create a [table](/develop/server-capabilities/data-model/#tables) called ORDER_STATUS. Each state would have a field of type ENUM in the table.

Once you have the states, there are two main sets of logic that must be defined. The recommended way to do this is to create dedicated `eventHandler` code blocks in your [Event Handler](/develop/server-capabilities/core-business-logic-event-handler/) service:
...
:::info
A [StateMachine API](#statemachine-api) is also available, which can be used if you decide to write your `eventHandler codeblocks using the [Event Handler API](/develop/server-capabilities/core-business-logic-event-handler/#event-handler-api). See further down for examples and docs.
:::
