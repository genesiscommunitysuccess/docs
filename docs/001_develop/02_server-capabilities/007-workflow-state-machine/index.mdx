...
The Genesis server State Machine is used to define workflows for application's table entities. State Machines work in conjunction with [Event Handlers](/develop/server-capabilities/core-business-logic-event-handler/) to ensure records for the entity follow the correct workflow at all times.
...
- [`onEvent`](#onevent) : One exception on the `modifyEvent`, per the [example configuration](#example-configuration) as the object is already in the DB and we know the event uses the table object, both event and the retrieved record are available in `onEvent`
...
- [`excludedFields`](#excludedfields)
- [`onEvent`](#onevent) : One exception on the `transitionEvent`, per the [example configuration](#example-configuration) as the object is already in the DB and we know the event uses the table object, both event and the retrieved record are available in `onEvent`
- [`onValidate`](#onvalidate)
- [`onCommit`](#oncommit)
...
A State Machine is just another layer of an Event Handler, the test configuration is the same as for Event Handlers. You should add further tests to test your transition workflow logic works as expected.
