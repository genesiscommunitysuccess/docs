...
One difference from a regular eventHandler `onValidate` block is that there is no need to return an `ack` or `nack`.
The validation is considered successful unless an `IllegalArgumentException` is thrown, in which case a `nack` will be returned.
