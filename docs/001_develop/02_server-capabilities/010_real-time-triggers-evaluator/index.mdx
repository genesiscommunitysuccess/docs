...
::::warning
Since version 8.13, `CRON_RULE` records with an invalid `CRON_EXPRESSION` will not cause the Evaluator service to crash. These are detected on startup and a warning is written to the Evaluator logs instead. Ensure you monitor logs and correct any misconfigured cron rules.
::::
