...
| IS_TEMPLATE |Relevant when using [notifications and alert component](/develop/business-components/notifications/notifications-introduction/) where users can define rule templates to which users subscribe. Rules marked as `true` are templates, are never triggered (a subscription to a template creates a new dynamic rule) ||false|
...
| RULE_TYPE |Used to specify that this rule belongs to a group. For example, the [notifications and alert component](/develop/business-components/notifications/notifications-introduction/) will set this value to `NOTIFY`|`NOTIFY`|false|
