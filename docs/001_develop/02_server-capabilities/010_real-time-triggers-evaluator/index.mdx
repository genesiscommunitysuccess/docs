...
| RULE_STATUS | The status of the rule. If anything other than `ENABLED`, the rule will not listen or trigger any event | `ENABLED` | true |
| RULE_EXPRESSION | Boolean expression referenced upon the configured RULE_TABLE receiving an update. The expression can reference any of the fields on the updated table or view. The expression must be wrapped in brackets  | `((NOTIONAL > 1000000))` | false |
...
| TABLE_OPERATION | The table operations to trigger the rule on. Valid operations are `INSERT`, `MODIFY` and `DELETE`. Where more than one operation should trigger the rule, these must be pipe-delimited. Where this value is left blank, only `INSERT` table operations will trigger the rule | `INSERT\|MODIFY\|DELETE` | true |
| IS_TEMPLATE |Relevant when using [notifications and alert component](/develop/business-components/notifications/notifications-introduction/) where users can define rule templates to which users subscribe, and will never trigger||false|
...
| RULE_TYPE |Used to specify that this rule belongs to a group. For example, the [notifications and alert component](/develop/business-components/notifications/notifications-introduction/) will set as `NOTIFY`|`NOTIFY`|false|

Rules are managed via the API, which exposes CRUD operations for maintaining the table. Components such as [notifications and alert component](/develop/business-components/notifications/notifications-introduction/) include a user interface that manages dynamic rule set-up and reads application metadata to help users populate table/view selection and expressions.
