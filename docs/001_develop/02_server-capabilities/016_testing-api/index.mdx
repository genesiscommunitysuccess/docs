...
tls:
  ...
  identity:
    ...
    keystore:
      type: "JKS"
      path: "${GENESIS_HOME}/site-specific/cfg/tls/server-keystore.jks"
      storePassword: "changeit"
      keyAlias: "server"
  trust:
    source: CERTIFICATE
    certificate:
      caBundlePath: "${GENESIS_HOME}/site-specific/cfg/tls/ca-cert.pem"
```

The `${...}` placeholders a deployment writes are resolved in a test too. `GENESIS_HOME` resolves to
the home the test declares with [`@GenesisHome`](#genesishome); anything else is read from the
environment.
...
WebSocket and JSON defaults onto the file's own.
...
setting a `tls` mode of `TLS` or `MUTUAL_TLS` gives that test a real TLS listener. The TLS context is
built while the process starts, so a path in the `tls` block that no longer resolves stops the test
rather than being skipped silently. The failure names the mode and the path.
:::
