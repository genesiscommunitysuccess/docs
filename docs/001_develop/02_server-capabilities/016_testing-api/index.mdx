...
2. [`@CsvData`](#csvdata)("my-trades.csv") - our test relies on trade data existing in the database. Here we specify that data. `@CsvData` resolves file names as classpath resources, so `my-trades.csv` must live under `src/test/resources` in your test module (following the standard Gradle/JUnit convention), for example at `src/test/resources/my-trades.csv`.
