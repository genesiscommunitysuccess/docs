...
```kotlin
@SysDefOverwrite("SysDef", "true")
class ScenarioA

@SysDefOverwrite("SysDef", "false")
class ScenarioB
...
```java
@SysDefOverwrite(key = "SysDef", keyValue = "true")
class ScenarioA {}

@SysDefOverwrite(key = "SysDef", keyValue = "false")
class ScenarioB {}
...
Used with scenario testing to execute the same test method across one or more scenarios declared with `@TestScenario`.
Apply at method level only.

<!-- TODO: Review interaction with shared context and per-method annotations. -->
...
<!-- TODO: Add detailed documentation for ProvidedSysDef annotation -->
...
<!-- TODO: Add detailed documentation for TestScenario annotation -->

Annotation used to define and share test scenarios.
This annotation can be applied to classes or methods and is Repeatable.
