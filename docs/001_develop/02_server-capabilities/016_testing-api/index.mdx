...
### Running a process with TLS

A test process reads a `network-config.yaml` or `network-config-PROCESS_NAME.yaml` from its
GENESIS_HOME, exactly as a deployed process does, so a test can run a process with TLS switched on.
See [Service encryption](/develop/server-capabilities/runtime-configuration/service-encryption/) for
...
tls:
  ...
  identity:
    ...
    keystore:
      type: "JKS"
      path: "certs/server-keystore.jks"
      storePassword: "testpass"
      keyAlias: "server"
  trust:
    source: CERTIFICATE
    certificate:
      caBundlePath: "certs/ca-cert.pem"
```

Relative `identity` and `trust` paths resolve against the test GENESIS_HOME. The material itself is
generated at run time by `TestCertificates.install(home)`, which writes a CA, a localhost server
identity and a client identity under `certs/` in the home you give it - so no key material is
committed to your repository. Its keystores use the alias and password shown above. Generate it
before the test context starts, for example from an `@BeforeAll` method.

A `${...}` placeholder is resolved from the environment, as in a deployment, and a test has no way
to set an environment variable - so a test config names its paths outright.

To test a file that does not live under the test GENESIS_HOME - your real application config, or a
variant of it - point [`@NetworkConfig`](#networkconfig) at it. The annotated file is then the
network config, and a file under the test GENESIS_HOME is ignored.

Two things are worth knowing before you add a file.

The whole file applies, not just its `tls` block. A file supplies `messaging`, `netty` and
`zeroMqEncryption` as well, so a file that leaves `messaging` out moves the test off the framework's
WebSocket and JSON defaults onto the file's own. A file also beats the `ChannelType` and
`MessageSerialization` system definitions, exactly as it does for a deployed process; those two
apply only when no file is in play.

Under `SELECTIVE_TLS`, TLS is per service, and a deployment turns it on with `secure="true"` in its
service definition. A test has no service-definitions file, so it says the same thing with
[`@SecureProcess`](#secureprocess). Without it a `SELECTIVE_TLS` process runs plain text, which is
what keeps a legacy `KeyStoreLocation` system definition booting as it always did.

:::note
This applies to **GenesisJunit** only. Tests written against the older
[`AbstractGenesisTestSupport`](/develop/server-capabilities/testing-api/legacy-testing-api/#abstractgenesistestsupport)
are unchanged and cannot run a process with TLS.
:::

:::warning
A network-config file under a test GENESIS_HOME was previously ignored and now takes effect. A file
setting a `tls` mode of `TLS` or `MUTUAL_TLS` gives that test a real TLS listener, and a file beats
the `ChannelType` and `MessageSerialization` system definitions, so a test carrying both follows the
file. The TLS context is built while the process starts, so a path in the `tls` block that no longer
resolves stops the test rather than being skipped silently. The failure names the mode and the path.
:::
...
| [ProcessArg](#processarg)                                   | Class/ Method      |     ❌      |                 ❌                 |
...
Annotation used to provide process arguments, as would be passed on the command line to a real Genesis process.
The arguments are made available to the test process via `ProcessArguments`.
This annotation can be applied to classes or functions and is not Repeatable.
Providing this annotation at the method level will take precedence over the class level annotation -
this is true even if the method level annotation provides no arguments, which clears any arguments
provided at the class level for that test.
This annotation is not supported when using [`@GenesisDatabaseTest`](#genesisdatabasetest), as no process is
bootstrapped in that mode.
...
```kotlin
@GenesisTest
@ProcessArg("--coldStart")
internal class ProcessArgTest {

    @Test
    fun myTest() {
        // the unit test service is started with process argument "--coldStart"
    }

    @Test
    @ProcessArg("--coldStart", "TABLE_NAME")
    fun myOtherTest() {
        // the unit test service is started with process arguments "--coldStart" and "TABLE_NAME",
        // replacing the class-level arguments for this test
    }

    @Test
    @ProcessArg
    fun myThirdTest() {
        // the unit test service is started with no process arguments for this test
    }
}
```
...
Advertises the process as secure, which is what a service definition's `secure="true"` attribute does
in a deployment. You can apply this annotation on both classes and methods.

It is only needed under the `SELECTIVE_TLS` mode, where TLS is decided per service. Under `TLS` and
`MUTUAL_TLS` every process is secure already, and under `PLAIN_TEXT` none is. See
[Running a process with TLS](#running-a-process-with-tls).
...
@SysDefOverwrite("PROCESS_NAME", "TRADE_APP")
@SecureProcess
class SecureTradeAppTest {

    @Inject
    private lateinit var serviceDetailProvider: ServiceDetailProvider

    @Test
    fun `the process is advertised as secure`() {
        assert(serviceDetailProvider.startedServices.single().isSecure)
    }
}
```
...
@GenesisTest
@GenesisHome("/GenesisHome")
@SysDefOverwrite(key = "PROCESS_NAME", keyValue = "TRADE_APP")
@SecureProcess
public class SecureTradeAppTest {

    @Inject
    private ServiceDetailProvider serviceDetailProvider;

    @Test
    public void theProcessIsAdvertisedAsSecure() {
        assertTrue(serviceDetailProvider.getStartedServices().get(0).isSecure());
    }
}
```
</TabItem>
...
Points the test process at a network config file anywhere on the classpath, read exactly as a
`network-config.yaml` under GENESIS_HOME is. The annotated file wins outright: a file under the test
GENESIS_HOME and the `ChannelType` and `MessageSerialization` system definitions are both ignored.
You can apply this annotation on both classes and methods. See
[Running a process with TLS](#running-a-process-with-tls).
...
@GenesisTest
@GenesisHome("/GenesisHome")
@NetworkConfig("/network-configs/tls.yaml")
class TlsTradeAppTest {

    @Inject
    private lateinit var networkConfig: NetworkConfigContainer

    @Test
    fun `the annotated file supplies the network config`() {
        assert(networkConfig.tls.mode == EncryptionMode.TLS)
    }
}
```
...
@GenesisHome("/GenesisHome")
@NetworkConfig("/network-configs/tls.yaml")
public class TlsTradeAppTest {

    @Inject
    private NetworkConfigContainer networkConfig;

    @Test
    public void theAnnotatedFileSuppliesTheNetworkConfig() {
        assertEquals(EncryptionMode.TLS, networkConfig.getTls().getMode());
    }
}
```
</TabItem>
...
Alternatively, you can test using **compiled GPAL definitions** (classes extending `GPalDataserver`, `GPalEventHandler`, and so on). Ensure the definition's package is in the scan list (e.g. by providing an instance of the definition class). In that case no script file and no `@TestScriptFile` are required.
...
### SQL schema selection

When tests run with `DbLayer=SQL`, Genesis test support needs a schema/namespace to use for the database.
The following JVM system properties control how the namespace is chosen:

- `DbNamespace`: if set, this is always used.
- `uniqueTestDbNamespace=true`: if `DbNamespace` is not set, generates a **unique random SQL namespace** for the test JVM (useful for parallel CI runs and avoiding schema collisions). The generated namespace is constrained to **30 characters** to conform with Oracle limits.
- Otherwise (default): if `DbNamespace` is not set and `uniqueTestDbNamespace` is false, the namespace defaults to `ProjectName` (or an empty string if `ProjectName` is not set, which typically uses the database's default schema).

Example (Gradle):

```bash
./gradlew test -DuniqueTestDbNamespace=true
```
