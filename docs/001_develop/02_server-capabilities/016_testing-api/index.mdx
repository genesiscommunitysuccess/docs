...
The `ExpectedProcessStatus` annotations provides a way to assert a certain process end state after a test.
By default GenesisJunit will assert that the process is healthy (`ProcessState.UP`) after a test.
