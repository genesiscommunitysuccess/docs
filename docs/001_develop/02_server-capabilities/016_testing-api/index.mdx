...
GENESIS_HOME, exactly as a deployed process does. See [Service encryption](/develop/server-capabilities/runtime-configuration/service-encryption/)
for what those files contain.
...
This is only needed under the `SELECTIVE_TLS` mode, where TLS is decided per service. Under `TLS` and
