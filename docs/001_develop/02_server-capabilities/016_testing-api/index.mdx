...
For example, one could define an interface in the production code called `MyInterface` with a default implementation `MyInterfaceImpl`.
...
@ImplementedBy(MyIntefaceImpl::class)
...
And override the implementation at runtime using fake/mock implementation.
