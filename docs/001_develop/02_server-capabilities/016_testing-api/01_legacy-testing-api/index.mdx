---
title: 'Legacy Testing API'
sidebar_label: 'Legacy Testing API'
id: server-testing-legacy
keywords: [component, testing, unit, test, curl, junit, api, legacy]
tags:
  - component
  - unit
  - testing
...
For the latest testing API, go to the [Testing API](/develop/server-capabilities/testing-api/) page.
...
| secure | no | advertises the process as secure, as a service definition's `secure="true"` does |

Any path provided for `genesisHome` and `initialDataFile` must be an absolute location.

Tests can also use **compiled GPAL definitions** (classes extending `GPalDataserver`, `GPalEventHandler`, and so on) by ensuring the definition's package is scanned (e.g. by providing an instance of the definition class). In that case no script file or `scriptFileName` is required.
...
for what those files contain, and [Running a process with TLS](/develop/server-capabilities/testing-api/#running-a-process-with-tls)
for how they behave in a test.

Under the `SELECTIVE_TLS` mode, TLS is decided per service, and a deployment turns it on with
`secure="true"` in its service definition. A test says the same thing with the `secure` property on
its `GenesisTestConfig`.

```kotlin
class SecureTradeAppTest : AbstractGenesisTestSupport<EventResponse>(
    GenesisTestConfig {
        packageName = "global.genesis.trade"
        genesisHome = "/genesisHome"
        parser = EventResponse
        secure = true
    }
) {
    override fun systemDefinition(): Map<String, Any> = mapOf("PROCESS_NAME" to "TRADE_APP")
}
```

A test extending `GenesisTestSupport` directly overrides `secure()` instead.
