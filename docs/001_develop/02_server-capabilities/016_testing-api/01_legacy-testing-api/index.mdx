...
When running tests against SQL databases, the database schema/namespace used by the legacy test support can be controlled via JVM system properties:

- `DbNamespace`: if set, this is always used.
- `uniqueTestDbNamespace=true`: if `DbNamespace` is not set, generates a **unique random SQL namespace** for the test JVM (constrained to **30 characters** for Oracle compatibility).
