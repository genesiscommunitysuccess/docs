...
| maxBy         | complex function that allows for selecting a field a row with a maximum value | any value  | any value     | sometimes `+` |
| minBy         | complex function that allows for selecting a field a row with a maximum value | any value  | any value     | sometimes `+` |
...
These functions will find, for a specific group, the record with the largest or smallest value, for then to select a value from that row.
For example:
```kotlin
select {
...
`tieBreaker` will select the smallest value, and `tieBreakerDesc` select the largest value.
