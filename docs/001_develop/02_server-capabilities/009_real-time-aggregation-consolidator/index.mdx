...
By default, Consolidators use incremental consistency. Under highly concurrent inputs, you can opt in to eventual consistency to prevent stored counts from diverging from the source.
...
| `ConsistencyMode.EVENTUAL` | Marks groups as dirty on update and rebuilds them in paced batches. |
...
consolidator("CON_ORDER_FROM_TRADES", CONSOLIDATOR_TRADE, ORDER) {
    ...
    groupBy { Order.ByIdDate(orderId, tradeDate) } into {
        ...
        indexScan { ConsolidatorTrade.byOrderId(groupId.orderId) }
    }
}
```
