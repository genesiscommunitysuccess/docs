...
The test class brings it in with `@ProvidedInstance`, using a `KClass` reference so Guice constructs and injects it — no script file, and no `addPackageName`/`scriptFileName` builder config:
...
@GenesisTest
@PackageNameScan("global.genesis.pal.consolidator")
class ConsolidatorTest {
    ...
    companion object {
        @ProvidedInstance
        @JvmStatic
        val orderFromTradesConsolidator = OrderFromTradesConsolidator::class
    }
...
`@PackageNameScan("global.genesis.pal.consolidator")` scans the Consolidator PAL module's own package alongside your definition — unlike Data Server, Event Handler, and Request Server, a compiled Consolidator needs this in addition to `@ProvidedInstance`/`@PackageScan`. This example mirrors `genesis-pal-consolidator/src/test/kotlin/global/genesis/pal/consolidator/service/CompiledConsolidator.kt` in the genesis-server framework's own test suite, which uses this exact combination.
