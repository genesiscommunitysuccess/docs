...
This Consolidator builds the ORDER table records using the CONSOLIDATOR_TRADE table. It assumes the ORDER records do not exist in the system and are created automatically by the consolidator based on the ```ORDER.ORDER_ID``` and ```ORDER.ORDER_DATE``` fields (using `groupBy` and `build`). It uses max, min, sum and count functions, and the table has been marked as transient, so it can be cleared and recalculated with a consolidator `coldStart`.
...
TODO Add Position table to data model?
```
consolidators {
...
| onNotFound           | determines what to do if an output record is not found (see build) TODO add link                                                                                                                                                                                                                                                                                                   | BUILD, WARN, LOG, FAIL                                                                                                                                                      | BUILD                  |
| batchingPeriod       | the time in ms accumulating updates before writing to the database                                                                                                                                                                                                                                                                                                                                      | Integer values                                                                                                                                                              | 500                    |
| ignoreIndexScan      | disables index scans (see indexScan)  TODO add link                                                                                                                                                                                                                                                                                                                                | true, false                                                                                                                                                                 | false                  |
...
| tableTransient       | Marks the output table as transient. This means the table can be cleared on consolidator restart and all records can be recalculated without any other implications. used to group multiple consolidations targetting the same `output` table to avoid potential aggregation race conditions. Consolidations with the same group will synchronize calculations between each other. | true, false                                                                                                                                                                 | false                  |
| backwardsJoin        | enables backward join support for `input` views (see backwarsJoin) TODO add link                                                                                                                                                                                                                                                                                                   | true, false                                                                                                                                                                 | true                   |

#### `groupName`
// TODO All our databases are transactional now that Aerospike is gone. Do we still need this?
...
in order to calculate the correct value. TODO Add link to index scan explained in group by
...
operations based on the consolidation group TODO add groupby link.
...
etc. TODO add link to transformations.
...
The `withOperations` block enables you to specify the behaviour of the different consolidation operations based on grouping logic TODO add link to groupby.
...
TODO Review example code, make it better.
```kotlin
onCommit {
...
### `groupBy` - `into` explanation

// TODO sort out examples to be relevant to data model in this whole section.
...
For service Consolidators, table syntax can add complexity, as records might need to be loaded, created or re-scanned (see TODO indexScan link)
...
TODO Should the other consolidator relevant logging docs go here instead? Some are done in config
...
TODO add custom log messages from existing snippet.
...
1. Ensure `global.genesis.pal.consolidator` is included in `package`
