...
You can test using **compiled definitions** (classes extending `GPalConsolidator`) by ensuring the definition's package is scanned (e.g. by providing an instance of the definition class). In that case no script file or `scriptFileName` is required. The example below uses a script-based definition with `scriptFileName`.
