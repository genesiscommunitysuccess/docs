...
Consolidators are defined in the `*-consolidator.kts` files or in compiled Kotlin classes extending `GPalConsolidator`.
...
For the concise syntax, the block inside `select` must be opened with the ALL-CAPS table or view reference, e.g. `global.genesis.gen.config.tables.TRADE`, or `global.genesis.gen.config.view.TRADE_VIEW`:
...
| maxBy         | selects a field from the row with the maximum value (for the key) | any value  | any value     | sometimes `+` |
| minBy         | selects a field from the row with the minimum value (for the key) | any value  | any value     | sometimes `+` |
...
#### `maxBy` and `minBy` functions

These functions will find, for a specific group, the record with the largest or smallest value, and then select a value from that row.
For example:
```kotlin
select {
    maxBy { price } value { quantity } into MAX_PRICE_QTY
}
```

In the above example, we will find the record with the largest `price`, and insert the `quantity` of that record into `MAX_PRICE_QTY`.
These functions are useful for retrieving information from specific records from within your aggregation.
If multiple records with the same selected value are found, then the consolidator will pick one, but the resulting record is not guaranteed.
To provide more control in those instances, `tieBreaker` or `tieBreakerDesc` can be used:

```kotlin
maxBy { price } tieBreakerDesc { quantity } value { orderId }
```

In the example above, we will get the record with the largest `price`, let's say 100.
If we have multiple records with `price` of 100, we will select the record with the largest `quantity` of those records.
From that record we will get the `orderId`.

`tieBreaker` selects the smallest value, and `tieBreakerDesc` selects the largest value.

Please note that these functions always require an index scan if a grouping is updated, and `indexScan` is required to be set for the consolidator.
...
By default, Consolidators use incremental consistency. Under highly concurrent inputs, you can opt in to eventual consistency to prevent stored counts diverging from the source.

To configure the consistency mode, set the `consistencyMode` property inside the `config` block.

| Property value | Description |
|---|---|
| `ConsistencyMode.INCREMENTAL` | Default. Applies updates as ordered deltas. |
| `ConsistencyMode.EVENTUAL` | Marks consolidation groups as dirty on update and rebuilds them in paced batches. |

When you enable `ConsistencyMode.EVENTUAL`, you must define an `indexScan` in the `groupBy` block. The Consolidator fails to start if `indexScan` is missing, because eventual consistency requires re-reading source records by group key.

:::important
Under eventual consistency, rebuilds of a group replay the [`onCommit`](#oncommit) block, which is idempotent. Any writes made in `onCommit` (for example, audit fields) are silently lost on replay and shouldn't be relied on as the sole place a durable update happens.
:::

###### Examples

The following example configures a Consolidator to use eventual consistency:
...
consolidator("CON_ORDER_FROM_TRADES", CONSOLIDATOR_TRADE, ORDER) {
    ...
    groupBy { Order.ByIdDate(orderId, tradeDate) } into {
        ...
        indexScan { ConsolidatorTrade.byOrderIdTradeDate(groupId.orderId, groupId.tradeDate) }
    }
}
```
...
To include your Consolidator definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.pal.consolidator` is included
- **module**: ensure `genesis-pal-consolidator` is included
- **script**: your `*-consolidator.kts` file(s), comma-separated if more than one
- **language**: `pal`
...
- **Gradle dependency**: add `implementation(genesis("pal-consolidator"))` to your application module's `build.gradle.kts` so `GPalConsolidator` is on the compile classpath
...
If you wish to run a dedicated process for a consolidator, the following gives an example using script files:
...
A cold start will remove existing data, before recalculating to ensure these values are correct.
Once all values have been recalculated, real-time event-driven consolidations will continue.

There are two ways a consolidator can remove existing data, depending on the configuration:
```kotlin
config {
    tableTransient = true
}
...
* if `tableTransient` is not set or is false, then all fields affected by the `select` configuration of the output table are set to zero on all records.
* if `tableTransient` is true, then all records in the output table are deleted.

There are three ways to initiate cold start:
* starting the process with the `--coldStart` argument: `startProcess GENESIS_CONSOLIDATOR --coldStart`.
* adding `--coldStart` in the `<arguments>--coldStart</arguments>` as per above.
* setting the system definition item `CONSOLIDATOR_COLDSTART` to `true`.

<CommonProcesses/>

## Testing (legacy)

You can test using **compiled definitions** (classes extending `GPalConsolidator`) by scanning the definition's package instead of loading a script file. In that case no script file or `scriptFileName` is required — see [Testing compiled definitions](#testing-compiled-definitions) below. The rest of this section uses a script-based definition with `scriptFileName`.

:::info
For the latest information on testing, go to our page on [Integration testing](/develop/server-capabilities/testing-api/#integration-testing).
:::
...
First, we need to add the required packages and genesis home. Then we need to set the "IS_SCRIPT" System Definition property to true (this is required as part of the Consolidator initialisation when using script-based definitions).
...
The walkthrough above uses the legacy `AbstractGenesisTestSupport` API. On [**GenesisJunit**](/develop/server-capabilities/testing-api/#integration-testing) (GSF v8+), the current, non-legacy way to test a Consolidator is `@GenesisTest`, whether your Consolidator is script-based (`@TestScriptFile`) or a **compiled definition** — a class extending `GPalConsolidator` — brought in with `@ProvidedInstance`. Those annotations come from `testImplementation(genesis("testsupport"))` — see [Testing without a script file](/develop/server-capabilities/gpal-standards/migration-guide-script-to-compiled/#testing-without-a-script-file).

Given this compiled Consolidator (the same configuration as the script example above):

```kotlin
package myapp.definition

import global.genesis.gen.config.tables.CONSOLIDATOR_TRADE
import global.genesis.gen.config.tables.ORDER
import global.genesis.gen.dao.Order
import global.genesis.pal.consolidator.script.GPalConsolidator
import org.joda.time.DateTime

class OrderFromTradesConsolidator : GPalConsolidator({
    // full consolidator dsl support from here, for example
    consolidator("CON_ORDER_FROM_TRADES", CONSOLIDATOR_TRADE, ORDER) {
        config {
            tableTransient = true
        }
...
The test class scans `OrderFromTradesConsolidator`'s package instead of loading a script file:

```kotlin
import global.genesis.db.rx.entity.multi.AsyncEntityDb
import global.genesis.gen.dao.ConsolidatorTrade
import global.genesis.gen.dao.Order
import global.genesis.testsupport.jupiter.GenesisTest
import global.genesis.testsupport.jupiter.PackageScan
import jakarta.inject.Inject
import kotlinx.coroutines.runBlocking
import myapp.definition.OrderFromTradesConsolidator
...
@GenesisTest
@PackageScan(OrderFromTradesConsolidator::class)
...
Or, to register just this one class without scanning its whole package, use `@ProvidedInstance` with a `KClass` reference instead:
...
class ConsolidatorTest {
    ...
    @ProvidedInstance
    private val orderFromTradesConsolidator = OrderFromTradesConsolidator::class

    // ... same entityDb injection and test as above
}
```
