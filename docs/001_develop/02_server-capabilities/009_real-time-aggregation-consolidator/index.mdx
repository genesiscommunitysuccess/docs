...
:::important
The Genesis Platform contains two consolidator modules:
- **GPAL Consolidator** (`genesis-pal-consolidator`): This is the current standard module (package `global.genesis.pal.consolidator`) using GPAL `.kts` files and the `GPalConsolidator` base class. You must use this for all new development.
- **Consolidator 2** (`genesis-consolidator2`): Despite the name, this is the older, legacy module (package `global.genesis.consolidator2`) using XML configuration. It is deprecated and you must not use it for new work.
:::
