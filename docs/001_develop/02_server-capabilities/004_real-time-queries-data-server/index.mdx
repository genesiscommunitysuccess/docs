...
keywords: [query, real-time, real time, dataserver, lmdb, lmdb allocation, serialization]
tags:
- query
- real-time
- real time
...
Genesis Data Servers are defined in the `*-dataserver.kts` files or in compiled Kotlin classes extending `GPalDataserver`.
...
`criteriaTemplate` is a way to enhance the criteria on an incoming data logon.
Within `criteriaTemplate`, you have access to `userName`, `request` and `db`.
`userName` is of type `String`. `request` is the DATA_LOGON message of type `GenesisSet`.
`db` is a read-only version of `AsyncEntityDb` for database lookups to enhance criteria.

These templates are very powerful in combination with data server indices.

##### User-based criteria

To filter data for specific users, use `userName`.
...
##### Database-based criteria

To build criteria from database lookups, use `db`.
...
query(MY_DATA_TABLE) {
    ...
    criteriaTemplate {
        db.get(EntityUsers.byUserName(userName))
            ?.let { "ENTITY_ID == '${it.entityId}" }
            ?: "false"
    }
}
```
...
|-------------|-------------|---------|------------------------|
|`lmdbAllocateSize`|This sets the size of the memory-mapped file where the in-memory cache stores the Data Server query rows. (See the section on [LMDB allocation](#lmdb-allocation-and-warning-state) for information about memory usage.) Value must be defined in `MEGA_BYTE` (`512.MEGA_BYTE()`) or `GIGA_BYTE` (`2.GIGA_BYTE()`)|`2.GIGA_BYTE()`|No|
|`serializationType`| Serialization approach for converting query rows into bytes and vice versa. There are two options: `SerializationType.KRYO` and `SerializationType.FORY`. See the [Serialization](#serialization-considerations) topic for details. |KRYO|No|
...
###### LMDB allocation and WARNING state
When the LMDB consumes most of the space allocated by `lmdbAllocateSize`, the status of the Data Server process is automatically changed to `WARNING`. 

There are two simple ways to address this:

- Increase the `lmdbAllocateSize`.
- Set `compression` to `true`.

:::tip Looking into memory usage 
To examine memory usage, look at the base table sizes when the Data Server process starts and the queries are primed. 

The Data Server logs how many records are stored in LMDB for each query on start-up, so you know exactly how many entries are used for each table and view. Queries with a large number of entities and large record size are likely to be the problem.
:::
...
- `SerializationType.FORY` uses the [Fory library](https://github.com/apache/fory).

In our current Data Server implementation, internal tests show that Fory can serialize and deserialize row objects 13-15 times faster than Kryo; this leads to faster LMDB read/write operations (up to twice as quick in some cases). This performance improvement has a great impact on the latency incurred when requesting rows from a Data Server query, whether it happens as part of the first subscription message (DATA_LOGON messages) or subsequent row request messages (MORE_ROWS messages).

However, the serialized byte array size of a Fory object can be between 10 and 15 percent larger than a Kryo object, so there is a small penalty to pay for using this option.
...
| `MAX_VIEW`       | 1000    | Maximum number of rows to track as part of a front-end view; see below for more; [read more below](#rows--view-concepts). If set to 0, the view will grow to a maximum size of 2147483647 (Java Integer.MAX_VALUE)                                                                                                                                    |
...
###### MAX_VIEW
MAX_VIEW defines the maximum number of rows that can be tracked as part of a front-end view. This setting controls the total size of the data structure that the Data Server maintains for each client subscription.

**Unbounded views**: You can set `MAX_VIEW` to `0` to create an unbounded view that can grow up to 2147483647 rows (Java Integer.MAX_VALUE). This is particularly useful for infinite scrolling scenarios where you need to retrieve a large number of records. By using an unbounded view instead of setting a large fixed `MAX_VIEW` value, you avoid forcing the backend to pre-allocate a data structure sized for the maximum possible view, which would unnecessarily increase the memory footprint of the Data Server process.
...
To include your Data Server definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.dataserver.pal` is included
- **module**: ensure `genesis-pal-dataserver` is included
- **script**: your `*-dataserver.kts` file(s), comma-separated if more than one
- **language**: `pal`
...
- **Gradle dependency**: add `implementation(genesis("pal-dataserver"))` to your application module's `build.gradle.kts` so `GPalDataserver` is on the compile classpath
...
Use the same **package** and **module** as for compiled-only (your definition package and your app's module). Also set **script** and **language** as for script-based. Definitions from script files and from compiled classes are merged (e.g. all queries from both).

If you wish to run a dedicated process as a dataserver, the following gives an example using script files:
...
For compiled definitions only, you would use your app's module and definition package and omit `script` and `language`:
...
If your Data Server is a **compiled definition** (a class extending `GPalDataserver`) rather than a `.kts` script, use `@PackageScan` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, connecting a `QueryView`, making assertions — works exactly as shown above. You don't need a separate test dependency for the Data Server's client — `DataServerQueryViewClient` and `@GenesisTest`/`@PackageScan`/`@ProvidedInstance` all come from `testImplementation(genesis("testsupport"))` (see [Testing without a script file](/develop/server-capabilities/gpal-standards/migration-guide-script-to-compiled/#testing-without-a-script-file)).

Given this compiled Data Server:
...
The test class scans `MyDataServer`'s package instead of loading a script file:

```kotlin
import global.genesis.testsupport.client.dataserver.DataServerQueryViewClient
...
class MyDataServerTest {
    ...
    @Test
    fun testAllTradesSubscribes() {
        val view = dataServerClient.queryView(datasourceName = "ALL_TRADES")
...
`@PackageScan` and `@ProvidedInstance` each work standalone — you don't need both. `@PackageScan` scans the whole package, so it also picks up anything else defined there; `@ProvidedInstance` registers only the exact class (or instance) you give it. If your process mixes script files and compiled definitions, combine either with `@TestScriptFile` (and `@CsvData` for seed data) on the same test class — all definitions are merged, exactly as they are at runtime.
...
|`defaultCriteria`|This represents the default criteria for the query||
|`disableAuthUpdates`|Disables real-time auth updates in order to improve the overall data responsiveness and performance of the server|`false`|
|`backwardsJoin`|Enables backwards joins on a view query. Read [backwards joins](#backwards-joins) for more details|`true`|
|`backwardsJoins`|(Deprecated) Replaced by `backwardsJoin`. Functionally the same.||
|`backJoins`|(Deprecated) Replaced by `backwardsJoin`. Functionally the same.||
