...
| `VIEW_NUMBER`        | 0   | If set to a number different than 0, enables pagination mode and forces `MAX_VIEW` to have the same size as `MAX_ROWS` (i.e. `MAX_ROWS` becomes the page size) and sets `MOVING_VIEW` to `false` (i.e. rows in a page won't be replaced) |
