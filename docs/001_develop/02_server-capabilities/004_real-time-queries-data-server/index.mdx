...
When the LMDB consumes most of the memory allocated by `lmdbAllocationSize`, the status of the Data Server process is automatically changed to `WARNING`. 
...
If LMDB memory usage is a persistent issue, then the recommended strategy is to copy the entire LMDB store to a new file on a regular basis and delete the old LMDB file. This reduces the store size by removing deleted entries (these are not tidied up automatically - LMDB does not run cleaning tasks in the background).
...
If you are testing against an earlier version of the framework, go to the [legacy](/develop/server-capabilities/testing-api/#integration-testing-legacy) section.
