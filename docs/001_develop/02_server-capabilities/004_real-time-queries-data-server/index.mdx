...
If your Data Server is a **compiled definition** (a class extending `GPalDataserver`) rather than a `.kts` script, use `@PackageScan` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, connecting a `QueryView`, making assertions — works exactly as shown above.
