...
The Data Server is a microservice responsible for providing APIs to request real-time views of an application's data.  Developers configure specific data sets as named queries, which are then preloaded by the Data Server and kept up to date as the underlying data is changed by the application. The Data Server utilises the push mechanism of web sockets to update subscribed clients with real time updates the data they are subscribed to and filtering on, and also offers a REST API.
...
`query` takes a `name` (optional) and an `entity` (table or view)
...
TODOTODO : Can we also use withAlias and withPrefix here?0
...
In the example below, we add a field to our Data Server from the previous example. The new field is a derived field:
...
TODO : What else is available? can we use entity as input as well as fields?

#### `filter`

`filter` requires boolean logic and has access to all fields defined in the query.
...
Note in this example DateTime.now() will be run every row which comes with a small overhead. To stop this, you could define it outside of the query to have it created at runtime per the example below:
...
The definition and the behaviour of an index in a query are exactly the same as when you [define an index in a table](/develop/server-capabilities/data-model/#indices).
...
TODOTODO : Is it true we inherit table (or root table of view) indices too?
...
Outside of the `query` blocks, you can make configuration settings that apply to all queries defined. These control the overall behaviour of the Data Server. Some can optionally be overriden at the query level
...
<Tabs defaultValue="websocket" values={[{ label: 'Websocket', value: 'websocket', }, { label: 'REST', value: 'rest', }]}>
<TabItem value="websocket">
Request
```
{
...
:::info
Ensure you have [enabled metrics](/build-deploy-operate/operate/metrics/#enabling-metrics) in your environment to see metrics
:::
...
If you are testing against an earlier version of the framework, go to the [legacy](./#integration-testing-legacy) section.
