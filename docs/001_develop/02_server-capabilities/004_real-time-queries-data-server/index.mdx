...
|`backwardsJoins`|Enables backwards joins on a view query. Read [backwards joins](#backward-joins) for more details|`true`|
...
Outside the `query` blocks, you can make configuration settings that apply to all queries defined. These control the overall behaviour of the Data Server. Some of the global settings can be overridden at the query level (as indicated in the table below).
...
|`defaultStringSize`|This is the size to be used for string storage in the Data Server in-memory cache. Higher values lead to higher memory use; lower values lead to lower memory use, which can lead to string overflow. See the [String overflows](#handling-string-overflows) topic for details. |`40`|Yes|
...
Note that the type-aware evaluator is also available in `DbMon` for operations such as `search` and `qsearch`. 
