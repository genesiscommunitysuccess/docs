...
import QueryPermissions from '/snippet/_query-permissions.md'
...
```mermaid
graph TD
A[UI] --> |1. Connection| B(DATA_SERVER)
B -->|3. Initial data| A
B --> |5. Updates| A
B <-->|2. Connection & Initial Read| C[Database]
...
`withSize <NUMBER OF CHARACTERS>` can also be specified at a field level on the rare occassion you are seeing issues with LMDB truncating a String value you can specify the maximum number of characters by specifying this.
