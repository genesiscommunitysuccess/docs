...
#### `config`
The following settings can be applied at query-level. For settings that can also be defined to override at the query level, see [global config](#global-config).
...
Outside the `query` blocks, you can make configuration settings that apply to all queries defined. These control the overall behaviour of the Data Server. Some can optionally be overridden at the query level
