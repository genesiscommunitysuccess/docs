...
If your Event Handler is a **compiled definition** (a class extending `GPalEventHandler`) rather than a `.kts` script, use `@PackageScan` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, sending events, asserting on the reply — works exactly as shown above. You don't need a separate test dependency for the Event Handler's client — `EventClientSync` and `@GenesisTest`/`@PackageScan`/`@ProvidedInstance` all come from `testImplementation(genesis("testsupport"))` (see [Testing without a script file](/develop/server-capabilities/gpal-standards/migration-guide-script-to-compiled/#testing-without-a-script-file)).
