...
If your Event Handler is a **compiled definition** (a class extending `GPalEventHandler`) rather than a `.kts` script, use `@PackageScan` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, sending events, asserting on the reply — works exactly as shown above.
