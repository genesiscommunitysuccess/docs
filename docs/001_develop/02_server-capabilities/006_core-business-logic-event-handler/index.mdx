...
The `trigger` method can be used to call on another event within the same Event Handler script file. It takes the event metadata as a parameter.
...
You can use the `triggerRemote` method to call on an event from another Event Handler script file. It takes the following parameters:
