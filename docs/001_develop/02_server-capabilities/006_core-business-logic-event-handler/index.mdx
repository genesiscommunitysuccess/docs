...
`contextEventHandler` is similar to an `eventHandler`. You can use it where you want to pass context from the [`onValidate`](#onvalidate) block to the [`onCommit`](#oncommit) block. For example, on a modification event, you can first look up the original entity record and pass it to `onCommit` so that it doesn't need to be re-read from the database.
