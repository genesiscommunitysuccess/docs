...
You have to specify the business logic to process each of these events. You do this in the `*-eventhandler.kts` files or in compiled Kotlin classes extending `GPalEventHandler`.
...
You must specify an `ack` at the end of all `onValidate` and `onCommit` blocks; this signifies that the event was successful.
...
A `nack` can also take an object of type [`Throwable`](https://docs.oracle.com/javase/6/docs/api/java/lang/Throwable.html), in which case the `message` of the Throwable will be sent back to the client.

Where a nack is triggered, `MESSAGE_TYPE` returned to the client is an `EVENT_NACK`.

### `FieldError`

Use `FieldError` when validation fails on a **specific field** and you want client forms (for example `foundation-form`) to highlight that field inline after submit.

Return `EventReply.EventNack(error = errors)` where `errors` is a list of `FieldError` entries. Each entry must include:

- **`TEXT`** — message shown to the user
- **`FIELD`** — property name on the event (must match event metadata / JSON schema, e.g. `INSTRUMENT_ID`, `CCY`)
- **`CODE`** — typically `ErrorCode.VALIDATION_ERROR`
...
A plain string [`nack`](#nack) (without `FIELD`) still returns `EVENT_NACK`, but clients show it as a **generic** error (toast / `submit-failure`), not inline on a field.

:::info Client forms
When using **`@genesislcap/foundation-forms`**, `foundation-form` maps `FieldError` entries to inline field errors automatically. See [Server field validation](/develop/client-capabilities/forms/smart-forms/foundation-forms-server-field-validation/).
:::
...
- `transactional` : takes a boolean. Set to `true` where you want your `eventHandler` to comply with [ACID](/develop/glossary/#acid). Any exception returned will result in a complete rollback of any database writes. While an exception will trigger a rollback, the transaction will commit if a `nack` or `ack` is returned. This setting also enables auto-auditing for writes to any [audited tables](/develop/server-capabilities/data-model/#audited-tables).
...
You can use your table entity [DAO](/build-deploy-operate/build/bdo-build-overview/#dao) as the `<Type>`. In this case, the event will inherit the table's fields, their types and their nullability (which determines whether the field is optional or mandatory).
...
Otherwise, you can create a [kotlin `data class` to define your metadata](/develop/server-capabilities/communications/#input-messages) and use it.
...
An exception is when you are using [optimistic concurrency](#optimistic-concurrency) mode. In this case, it is essential to include the `TIMESTAMP` field in the event meta. This happens automatically where `excludeMetadataFields` has not been set.
:::

#### `overrideMetadataFields`

`overrideMetadataFields` enables you to customize the JSON schema metadata that is exposed by a specific event handler definition. This is particularly useful when you want to modify field properties such as making fields optional, providing default values, or changing descriptions without altering the underlying table or entity structure.

**Important**: This functionality only affects the metadata exposed by the event handler - it does not modify the actual data model or entity definitions.

For more information on how the Genesis Application Platform uses type-safe messages and JSON schema, see [Type-safe messages](/develop/server-capabilities/communications/#type-safe-messages).

The most common use cases for `overrideMetadataFields` include:

- Adding or updating field descriptions and titles
- Setting default values for fields
- Marking fields as read-only or constant at the event handler level
- Making mandatory fields optional by providing default values (fully functional since version 8.13)
- Making optional fields mandatory to enforce stricter validation at the event handler level

To use this feature, you need to import the `OverrideMetaField` class:

```kotlin
import global.genesis.process.metadata.pal.common.OverrideMetaField
```

The `OverrideMetaField` class has the following structure:

```kotlin
OverrideMetaField(
    name: String,              // Required: The name of the field to override
    title: String? = null,     // Optional: Override the field's display title
    description: String? = null, // Optional: Override the field's description
    type: MetaFieldType? = null, // Optional: Override the field's type metadata
    optional: Boolean? = null,   // Optional: Override whether the field is optional
    readOnly: Boolean? = null,   // Optional: Override whether the field is read-only
    default: Any? = null,        // Optional: Provide a default value
    constant: Any? = null        // Optional: Set a constant value
)
```

When you specify an `OverrideMetaField`, any non-null properties you provide will override the corresponding properties from the original field metadata. Properties that you leave as `null` will retain their original values from the entity definition. For example, if you only specify `name` and `default`, all other properties (such as `title`, `description`, `type`, etc.) will remain unchanged from the original field definition.

**Making mandatory fields optional**

One capability of `overrideMetadataFields` is the ability to make mandatory fields optional. However, there is an important constraint: **you can only make a mandatory field optional if you provide a default value**. This ensures that the system always has a valid value for the field, even when the client doesn't provide one.

```kotlin
import global.genesis.process.metadata.pal.common.OverrideMetaField

eventHandler<Company>(name = "OVERRIDE_META_COMPANY_INSERT") {
    overrideMetadataFields = listOf(
        OverrideMetaField(
            name = "COMPANY_NAME",
            optional = true,        // Making the field optional
            default = "MY_COMPANY"  // Must provide a default value
        )
    )
    onCommit { event ->
...
In this example, `COMPANY_NAME` is made optional with a default value of `"MY_COMPANY"`. If a client submits an event without providing `COMPANY_NAME`, the system will automatically use the default value.

:::warning
Attempting to make a mandatory field optional without providing a default value will result in an error. The system enforces this rule to maintain data integrity.
:::

**Making optional fields mandatory**

You can also use `overrideMetadataFields` to make optional fields mandatory at the event handler level. This is useful when you want to enforce stricter validation for specific event handlers without changing the underlying entity definition.

```kotlin
import global.genesis.process.metadata.pal.common.OverrideMetaField

eventHandler<Trade>(name = "TRADE_INSERT_STRICT") {
    overrideMetadataFields = listOf(
        OverrideMetaField(
            name = "COUNTERPARTY_ID",
            optional = false  // Making the field mandatory
        )
    )
    onCommit { event ->
        val trade = event.details
...
In this example, `COUNTERPARTY_ID` (which might be optional in the `Trade` entity) is made mandatory for this specific event handler. Clients must provide a value for this field when submitting events to this handler.

**Other examples**

Setting a default value without changing optional status:

```kotlin
eventHandler<Trade>(name = "TRADE_INSERT", transactional = true) {
    overrideMetadataFields = listOf(
        OverrideMetaField(name = "QUANTITY", default = 1)
    )
}
```

Updating field descriptions and titles:

```kotlin
eventHandler<Order>(name = "ORDER_UPDATE") {
    overrideMetadataFields = listOf(
        OverrideMetaField(
            name = "ORDER_STATUS",
            title = "Current Status",
            description = "The current processing status of the order"
        )
    )
}
```

By default, `overrideMetadataFields` is an empty list.
...
As well as returning an [`ack`](#ack) (success) or a [`nack`](#nack), the `onValidate` block allows users to send soft warnings back as part of the validation. The client can ignore these warnings, for example if the client is a user interface.
...
Within the `onvalidate` block, a `verify` block is helpful for common relational checks. For example, you can use it to check that referenced ID-based entities already exist in the database.
...
If the boolean logic fails, validation of the event has failed, and a [nack](#nack) is returned.

##### `require`

`require` is [a kotlin construct](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/require.html) often used in `onValidate` blocks, because where the defined boolean logic statement fails, it automatically returns a [`nack`](#nack) with the given error message.
...
A `warningNack` may also take an object of type [`Throwable`](https://docs.oracle.com/javase/6/docs/api/java/lang/Throwable.html). In this case, the `message` of the Throwable will be sent back to the client.
...
An `approvableAck` should be returned in an onValidate block where [`requiresApproval`](#require-approval-on-an-event) has been set on the event.
...
You can use the `trigger` method to call on another event within the same Event Handler script file. It takes the event metadata as a parameter.

```kotlin
    eventHandler<TriggerTrade>(name = "TRIGGER_TRADE_INSERT") {
        onValidate {
            ack()
        }
        onCommit { event ->
            val trade = Trade {
                // do some logic with event details to get new details for next event call
            }
            // set the messageType to the name of the event you wish to call and details to the new trade object
            trigger(event.copyMeta(messageType = "TRADE_INSERT", details = trade))
        }
    }
```
...
You can use the `triggerRemote` method to call an event that is not defined in this script file, for example:

- another event from this application, which is defined in another file
- an event defined in the platform
- a business component that you have added to this project

`triggerRemote` takes the following parameters:

- event metadata
- timeout on the remote event call in milliseconds (optional)

```kotlin
    eventHandler<TriggerTrade>(name = "TRIGGER_REMOTE_TRADE_INSERT") {
        onValidate {
            ack()
        }
        onCommit { event ->
            ...
            // set the messageType to the name of the event you wish to call and details to the new trade object
            triggerRemote(event.copyMeta(messageType = "TRADE_INSERT", details = trade))
        }
    }
```
...
`contextEventHandler` is similar to an `eventHandler`. You can use it where you want to pass context from the [`onValidate`](#onvalidate) block to the [`onCommit`](#oncommit) block. For example, on a modification event, you can look up the original entity record and then pass it to `onCommit` so that it doesn't need to be re-read from the database.
...
- The definition takes two `<Type>` (aka Generic) parameters instead of one:
  - the context object, which will be passed to the [`onCommit`](#oncommit) block
  - the event metadata (which is the same as for a regular [`eventHandler`](#eventhandler))
- The [`onValidate`](#onvalidate) block must return a [`validationAck`](#validationack) instead of a regular [`ack`](#ack).
- The [`onCommit`](#oncommit) block has two lambda parameters:
  - the standard event
  - a reference to the context object returned in the [`validationAck`](#validationack)

#### `validationAck`

For a `contextEventHandler`, you must specify a `validationAck` at the end the `onValidate` block to signify that the event was successful. Do not use the standard `ack` used in standard `eventHandler` blocks.

The `validationAck` must return an object matching the type of the first `<Type>` parameter in the `contextEventHandler` definition.
...
Optimistic Concurrency helps prevent users from updating or deleting a stale version of a record. Our page on [Optimistic Concurrency](/develop/server-capabilities/data-access-apis/#event-handler) provides details around how this works for event handlers.
...
2. Add the new file's name to the `GENESIS_CLUSTER` process definition's `<script></script>` element found in your application's `cfg/genesis-processes.xml` (values should be comma-separated). If this file is not in your application, [read here how to find and override the file](/develop/server-capabilities/runtime-configuration/site-specific/#overriding-platform-provided-files).
...
* `insertMessage` - an instance of the `PendingApprovalInsert` class, which is used to populate the `APPROVAL` table if successful. The content of this class consists of several properties:
    ...
    * `approvalType` - equivalent to the property with the same name provided as part of `approvableAck` (see [pending approval workflow](#pending-approval-workflow)).
    * `additionalDetails` - equivalent to the property with the same name provided as part of `approvableAck` (see [pending approval workflow](#pending-approval-workflow)).
    * `generated` - equivalent to the property named `entityDetails` provided as part of `approvableAck` (see [pending approval workflow](#pending-approval-workflow)).
...
- `selectPredicate` is a function that accepts an indeterminate number of functions returning a boolean value, as well as a mandatory `default` function to handle messages that do not fall into any defined category. The `default` function  provides a [GenesisSet](/develop/server-capabilities/communications/#genesisset) object with the contents of the original message payload.
- `onEvent` works very similarly to any other GPAL [Event Handler definition](#eventhandler). It enables you to treat the incoming message in the same way as you would have done within the original Event Handler; however, each function must return a boolean expression.
...
To include your Event Handler definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.eventhandler.pal` is included
- **module**: ensure `genesis-pal-eventhandler` is included
- **script**: your `*-eventhandler.kts` file(s), comma-separated if more than one
- **language**: `pal`

### Using compiled definitions only

- **package**: the package containing your compiled definition class, or any package above it — scanning covers subpackages too. For example, a process configured with `<package>com.company.app</package>` will still find a class at `com.company.app.event.MyEventHandler`
- **module**: your application's module (e.g. `position-app`)
- **Gradle dependency**: add `implementation(genesis("pal-eventhandler"))` to your application module's `build.gradle.kts` so `GPalEventHandler` is on the compile classpath
...
If you wish to run a dedicated process for an Event Handler, here is an example using script files:
...
You can test using **compiled definitions** (classes extending `GPalEventHandler`) by scanning the definition's package with `@PackageScan` instead of loading a script file. In that case no script file or `@TestScriptFile` is required — see [Testing compiled definitions](#testing-compiled-definitions) below. The rest of this section uses script-based definitions with `@TestScriptFile`.
...
This testing relies on [**GenesisJunit**](/develop/server-capabilities/testing-api/#integration-testing), which is designed to make testing easy.
...
At this point you have tested a very simple Event Handler. You haven't even had to use the database!
However, you have covered the basics of communication between tests and Event Handlers:
...
For more details about testing with Genesis, take a look at our [integration test documentation](/develop/server-capabilities/testing-api/#integration-testing).


### Testing compiled definitions

If your Event Handler is a **compiled definition** (a class extending `GPalEventHandler`) rather than a `.kts` script, use `@PackageScan` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, sending events, asserting on the reply — works exactly as shown above. `EventClientSync` comes from `testImplementation(genesis("testsupport"))`, same as the annotations — see [Testing without a script file](/develop/server-capabilities/gpal-standards/migration-guide-script-to-compiled/#testing-without-a-script-file).

Given this compiled Event Handler:

```kotlin
package myapp.definition

import global.genesis.eventhandler.pal.script.GPalEventHandler

data class Hello(
    val name: String,
)

class HelloWorldEventHandler : GPalEventHandler({
    // full event handler dsl support from here, for example
    eventHandler<Hello>("HELLO_WORLD") {
        onCommit {
            ack()
        }
...
The test class scans `HelloWorldEventHandler`'s package instead of loading a script file:
...
import myapp.definition.Hello
import myapp.definition.HelloWorldEventHandler
import org.junit.jupiter.api.Test

@GenesisTest
@PackageScan(HelloWorldEventHandler::class)
...
Alternatively, use `@ProvidedInstance` to hand the test an already-constructed instance instead of having GenesisJunit discover it via package scanning — useful when the definition needs a dependency you want to stub out yourself:
...
class HelloWorldEventHandlerTest {
    ...
    @ProvidedInstance
    private val helloWorldEventHandler = HelloWorldEventHandler::class
...
`@PackageScan` and `@ProvidedInstance` each work standalone — you don't need both. `@PackageScan` scans the whole package, so it also picks up anything else defined there; `@ProvidedInstance` registers only the exact class (or instance) you give it. If your process mixes script files and compiled definitions, combine either with `@TestScriptFile` on the same test class — all definitions are merged, exactly as they are at runtime.
...
Following that, here is a test to verify that a trade cannot be entered if the user is _not_ a Trader:
...
Before you can make any calls on these resources, you will have to permission yourself by obtaining a SESSION_AUTH_TOKEN. The details of how to do this are on our separate [Testing](/develop/server-capabilities/testing-api/#using-an-api-client) page.
...
| overrideMetadataFields | `fun overrideMetadataFields(): Map<String, OverrideMetaField>` | emptySet() | Contains a map (key-value entries) of metadata field names to metadata field definitions in the shape of `OverrideMetaField`. This enables you to override the metadata field properties extracted from input `I`. For detailed information on how this works and examples, see the [`overrideMetadataFields` section](#overridemetadatafields).                                                                                        |
...
| schemaValidation | `fun schemaValidation(): Boolean` | true | This option enables you to disable the automatic Json Schema validation enforced by the back end. See [type-safe messages](/develop/server-capabilities/communications/#type-safe-messages) for more information.
...
| requiresApproval | false | This particular event needs approval from a second user if set to true. For more details, see [approval workflow](#approval-workflow) |
| approvalKey | null | Auto-generated key ID for particular approval request. For more details, see [approval workflow](#approval-workflow)                  |
| approvalMessage | null | Optional message for approval request. For more details, see [approval workflow](#approval-workflow)                                  |
...
Sync works similarly to [Async](/develop/server-capabilities/data-access-apis/#asyncentitydb) and [Rx3](#rx3), but in this case, there is no `Single<O>` returned and no `suspend` modifier used for Kotlin coroutines. The expected output of the Event Handler logic is just the `O` type.
