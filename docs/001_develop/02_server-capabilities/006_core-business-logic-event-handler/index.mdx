...
:::tip
If you are using more than one Event Handler script file, use [`triggerRemote`](#triggerremote) to call an event in one of the other files.
:::
...
Where you have more than one Event Handler script file, you can use the `triggerRemote` method to call on an event from one of the other Event Handler script files. It takes the following parameters:
...
- The definition takes two `<Type>` (aka Generic) parameters instead of one:
  - the context object, which will be passed to the `onCommit` block
...
- The [`onCommit`](#oncommit) block has two lambda parameters: 
  - the standard event 
+ - a reference to the context object returned in the `validationAck`
