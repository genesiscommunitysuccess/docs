...
You can use the `triggerRemote` method to call an event that is not defined in this script file. For example another event from this application but defined in another file, or an event defined in the platform, or a business component that you have added to this project. `triggerRemote` takes the following parameters:
