...
The `trigger` method can be used to call on another event within the same event handler script and takes the event metadata as a parameter.
...
The `triggerRemote` method can be used to call on an event from another microservice and takes the following parameters:
- event metadata
