...
Add `testImplementation(genesis("testsupport"))` to your app module's `build.gradle.kts`. `@GenesisTest`, `@PackageScan`, `@ProvidedInstance`, and every GPAL type's test client (`DataServerQueryViewClient`, `EventClientSync`, `RequestClientSync`, etc.) all live in `genesis-testsupport` — one dependency, not one per type. There's no `genesis("pal-<type>")`-style test artifact; that naming only applies to the main compile dependency.
