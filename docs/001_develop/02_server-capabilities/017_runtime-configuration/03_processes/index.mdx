...
You can see that this file has separate code blocks for each of the application's processes. Each code block has tags that define the characteristics of the process. Processes can load definitions from script files, from compiled Kotlin classes in scanned packages, or both.
...
| module          | Defines the core jar file to be used for this process and its correspondent **base classpath**. This jar file must be available as part of the application binaries. For **script-based** GPAL processes use the PAL service module (e.g. **genesis-pal-eventhandler**). For **compiled GPAL only** use your application's module (e.g. **position-app**).                                                                                                                                                                   | No       | None               |
| package         | Defines the package(s) the process scans for dependency injection. For **script-based** GPAL processes use the PAL service package (e.g. **global.genesis.streamer.pal**). For **compiled GPAL only** use the package that contains your definition classes (e.g. **my.app**). Multiple package scans need to be comma-separated.                                                                                                                                                                                          | No       | None               |
...
| script          | Required when the process loads GPAL from script files (use with the PAL service **package** and **module**). Omit when the process uses **only** compiled GPAL definitions (your definition package and your app's **module**); dependency injection then discovers and instantiates the service. When **mixing** script and compiled, include **script** and **language**; definitions from both are merged. Specify one or more script filenames, comma-separated. See [GPAL Standards](/develop/server-capabilities/gpal-standards/#script-and-compiled-definitions) and each GPAL type's runtime configuration. | Yes       | None               |
| description     | Provides a simple description of the process.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | No       | None               |
| language        | Defines what language the script is using. Required when using script-based GPAL (typically `pal`). Not used when using only compiled GPAL definitions. The default value is groovy. Accepted values are `pal`, `groovy`.                                                                                                                                                                                                                                                                                               | No       | None               |
