---
title: 'Process cache'
sidebar_label: 'Process cache'
id: server-process-cache
keywords: [server, configuration, config, cache]
tags:
  - server
  - configuration
  - config
...
If you need to change a process-scoped system-definition value at deploy time without changing the `*-process-config.kts` file, you can override it using environment variables (see [Process-scoped system-definition overrides](/develop/server-capabilities/runtime-configuration/system-definition/#process-scoped-system-definition-overrides)).
