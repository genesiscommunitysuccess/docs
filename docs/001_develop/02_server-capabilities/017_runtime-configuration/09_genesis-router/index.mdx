...
The configuration options are:
  * `enabled` defines whether the cookie authentication mechanism will be enabled or disabled at the Router level. Default: true (since 8.14)
  * `httpOnly` sets the generated cookies to be 'HttpOnly' (more information [here](https://owasp.org/www-community/HttpOnly)). Default: true (since 8.14)
  ...
  * `sessionCookies` When `true`, the router sends auth cookies (session and refresh tokens) as session cookies without `Max-Age` so they expire when the browser closes; when `false` (default), cookies use their configured timeout values.
...
- `channelCapacity` sets the size of the channel buffer. Defaults to 32.
