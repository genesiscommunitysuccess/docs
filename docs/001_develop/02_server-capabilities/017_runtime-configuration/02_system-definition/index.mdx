...
**UseJsonSystemDefinition**: When set to `true`, services use the `generated-system-definition.json` file instead of reading and compiling all `*-system-definition.kts` files. The JSON file is generated during `genesisInstall`; using it avoids compiling and caching the `.kts` files.
