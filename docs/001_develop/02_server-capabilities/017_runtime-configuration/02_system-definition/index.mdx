...
**DbMaxRetryBeforeExit**: This setting defines the maximum number of database connection retry attempts before the Genesis process terminates. It prevents the application from retrying forever when the database is unavailable.

Default value: `-1` (infinite retries). When set to `-1`, the application will retry indefinitely.

Configuration:

```kotlin
item(name = "DbMaxRetryBeforeExit", value = "5")
```

The retry mechanism uses exponential backoff with the following pattern:
- 1s → 2s → 4s → 8s → … (capped at 64s max)
...
**LogFramework**: Contains name of the logging framework. Supported framework: LOG4J2. This covers Genesis's own internal processes; in a Spring app it stays `LOG4J2` even when the Spring side is configured to use Logback — see [Spring integration getting started: switch logging to Logback](/develop/server-capabilities/spring-integration/getting-started/#step-3-switch-logging-to-logback).

**LogFrameworkConfig**: Contains name of the log framework configuration file. In a Spring app, this is set separately from `LogFramework` to point the Spring side at `logback.xml`.
...
For SSL on process communication, see the [service definition](/develop/server-capabilities/runtime-configuration/service-definition/#enabling-ssl-for-processes) page.

### GPal script caching settings

These settings control compilation and caching of GPal scripts and system-definition files. Because these files rarely change between restarts, using cached versions speeds up process start-up and reduces memory use.


**ScriptCacheDir**: The directory used for script caching. Defaults to `%GENESIS_HOME%/runtime/scriptcache` if not set.

**ScriptCachePerService**: When `true`, the script cache is process-specific rather than shared. Defaults to `false` if not provided. Process-specific caching avoids cross-process cache conflicts and can make startup more reliable.
...
### Process-scoped system-definition overrides

System-definition items can be set at **process scope** (for example inside a `*-process-config.kts` file using `process { systemDefinition { ... } }`).

You can also set them as environment variables. To do this, prefix the variable name with the process name:

- `GENESIS_SYSDEF_<PROCESS_NAME>_<ItemName>=...`

If both a global override (`GENESIS_SYSDEF_<ItemName>`) and a process-scoped override are set, the process-scoped override takes precedence **for that process only**.

For example:

```bash
GENESIS_SYSDEF_DbSqlMaxPoolSize=10
GENESIS_SYSDEF_GENESIS_AUTH_MANAGER_DbSqlMaxPoolSize=50
```

In this example, all processes will use `DbSqlMaxPoolSize=10` by default, but the `GENESIS_AUTH_MANAGER` process will use `DbSqlMaxPoolSize=50`.
