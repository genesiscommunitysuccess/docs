...
* **Selective TLS**: selected services use TLS
* **TLS**: all services use TLS
* **Mutual TLS/MTLS**: all services use TLS, plus services require client certificates

Before version 8.15 of the Genesis Platform, only plain text and selective TLS were supported.
To configure previous versions, see [Legacy setup](#legacy-setup).
The legacy configuration is still supported in version 8.15 and beyond.

## Configuring TLS

To support TLS fine-tuning on a service, Genesis has two types of configuration files:
* `network-config.yaml` — the global network config settings
* `network-config-SERVICE_NAME.yaml` — process-level overrides
...
For a full overview of all available options, see the [Network configuration YAML reference](./yaml-reference).

## Legacy Setup


Genesis supports TLS for client->server and process->process communications where needed.
...
The keystore (.jks) is, in a way, the private key to be used in the two-way authentication in the TLS protocol. As such, you need to use it to generate the certificate that has to be installed by the target computers, or to be loaded by the processes that the application intends to communicate with.
...
First, you must enable processes to communicate via TLS. In order to do this, we must ensure that the secure element is set to "true". Without this, your application would send communications along with any non-encrypted HTTP requests to your domain.
...
Therefore, it is essential that you set the **service-definitions.xml** file this way for every service or module for which you want to enable TLS.
:::
