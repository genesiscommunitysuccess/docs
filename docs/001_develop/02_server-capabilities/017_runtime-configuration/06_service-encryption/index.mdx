...
The same files apply to a GenesisJunit integration test, so you can run a process with TLS end to
end. See [Running a process with TLS](/develop/server-capabilities/testing-api/#running-a-process-with-tls).
...
For production, use certificates from a trusted CA (see your CA's documentation for keystore creation). For creating test certificates only, see [Generating certificates for testing](#generating-test-certificates) below.
