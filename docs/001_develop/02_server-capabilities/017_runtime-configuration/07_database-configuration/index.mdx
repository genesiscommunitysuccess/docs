...
#### SQL database mode configuration

You can configure the SQL database mode using the `DbMode` property. The platform supports two internal SQL modes:

* `ADVANCED` (Default): This is the default mode. If you do not set `DbMode`, or if you set it to any value other than `LEGACY` (such as `POSTGRESQL` or `VANILLA`), the platform resolves it to `ADVANCED`. Note that `POSTGRESQL` is a string alias for `ADVANCED`, not a distinct mode.
* `LEGACY`: Use this mode if you need to maintain compatibility with older legacy database schemas.

To set the database mode, add the following line to your system definition file:

```kotlin
item(name = "DbMode", value = "ADVANCED")
```
