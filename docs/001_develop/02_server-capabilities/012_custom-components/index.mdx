...
id: server-custom-components
keywords: [core, logic, custom, component]
tags:
- core
- logic
- custom
...
The Genesis Application Platform enables you to create [modules for business logic](/build-deploy-operate/build/project-structure/#adding-new-modules-to-a-project). 
...
```
If the package for this class is available in the classpath (```<classpath>``` section in [processes.xml](/develop/server-capabilities/runtime-configuration/processes/)) and is also being scanned (i.e. ```<package>``` section in [processes.xml](/develop/server-capabilities/runtime-configuration/processes/)), `ProductMessageTracer` will take precedence when starting the process and `DefaultMessageTracer` will be ignored. Otherwise, `DefaultMessageTracer` will be instantiated.
...
Lastly, you can use a combination of both "conditional on property" and "conditional on missing class" annotations. Referring to the previous example, you can use both features at once like this:
...
If the condition fails (for example, if the system definition value for `MessageTracer` is set to anything other than `DEFAULT`), the module is not registered. Note that this annotation does not directly cause a start-up failure. Instead, a start-up failure only occurs if another component attempts to `@Inject` the now-unbound type, resulting in a standard Guice missing-binding error.
