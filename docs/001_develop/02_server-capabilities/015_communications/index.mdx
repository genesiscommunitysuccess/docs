...
To protect your application from Cross-Site Scripting (XSS) attacks, you can sanitise string inputs and outputs.

#### System-wide sanitisation

System-wide XSS sanitisation is gated by `GenesisJacksonConfig`, which has two independent boolean properties (both default to `false`):

- `inputStringXssSanitiser`: Enables XSS sanitisation during deserialisation of incoming messages.
- `outputStringXssSanitiser`: Enables XSS sanitisation during serialisation of outgoing messages.

#### Per-field sanitisation

To apply sanitisation to specific fields rather than system-wide, annotate your message data class fields with `@field:JsonDeserialize` using `XssSanitizingDeserializer::class`.

###### Examples

```kotlin
import com.fasterxml.jackson.databind.annotation.JsonDeserialize
import global.genesis.commons.jackson.XssSanitizingDeserializer
