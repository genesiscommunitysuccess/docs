...
You can protect against Cross-Site Scripting (XSS) by enabling string sanitisation for outgoing message data. The platform supports two approaches: system-wide sanitisation for all outgoing string properties, or per-field sanitisation on type-safe message data classes.

**System-wide XSS sanitisation**

Enable XSS string sanitisation for all outgoing string properties by setting the router-config property `xssStringSanitizer` to `true` on `genesis-router.kts` file. By default this property is set to `false`.

**Per-field XSS sanitisation on type-safe messages**

For type-safe messages, you can annotate specific string fields to use the XSS sanitising serialiser. Only annotated fields are sanitised; other string fields are left unchanged.

Apply `@field:JsonSerialize(using = XssSanitizingSerializer::class)` to the field. You need the following imports:

```kotlin
import com.fasterxml.jackson.databind.annotation.JsonSerialize
import global.genesis.jackson.serialization.XssSanitizingSerializer


data class SanitisedDataClass(
    @field:JsonSerialize(using = XssSanitizingSerializer::class)
    val sanitised: String,
    val unsanitised: String
)
```

When serialising, the `sanitised` field is sanitised (script tags are stripped and certain characters are escaped), while `unsanitised` keeps the raw value. For example:
