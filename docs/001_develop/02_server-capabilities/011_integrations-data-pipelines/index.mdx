...
### `source`
The source function takes an instance of an object that implements the source interface. This can be defined in-line in the script, one of the sources provided by the core platform or any of the extension modules such as kafka-genesis, or a custom class written in your application that implements one of the Data Pipelines API source interfaces.
...
### `sink`
To make the pipeline complete, you must add a sink using the `sink` function. The sink function takes an instance of an object that implements any of thje sink interfaces. This can be defined in-line in the script, one of the sinks provided by the core platform or any of the extension modules such as kafka-genesis, or a custom class written in your application that implements one of the Data Pipelines API sink interfaces. Note that if your pipeline produces only single elements and not streams of elements as a result of each input, the sink must be able to handle single one element at a time. All of the provided sink implementations in the platform implement both `Sink` and `StreamSink` for this reason.
...
This implementation writest to a table called DATA_PIPELINE_PERSISTENCE which contains the following fields:
