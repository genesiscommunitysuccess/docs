...
```mermaid
graph TD
A[UI] --> |1. Connection| B(REQUEST_SERVER)
B -->|3. Set Response| A
B <-->|2. Read|C[Database] 
```
...
Keeping with the [data model examples](/develop/server-capabilities/data-model/#example-configuration) the following show example request configurations.
...
Request servers will attempt to use the most efficient mechanism to retrieve data from the data base. Given the `REQUEST` fields which are input, and not wildcarded, it will cycle through the indices of the table or view used and score accordingly. If it is a full match we give max score to that index, any partial matches are scored and that with the highest score is used.

###### Audit table recommendations

Audit table request servers are very helpful for requesting all the audit records for a given record, or set of records. Audits can get very large, so having real-time data server can come with a large overhead due to it's caching nature. It is encouraged to instead use request servers for requesting audit rows for of a particular record ID.
...
:::tip
Request servers are also accessible via [Open API](/develop/server-capabilities/integrations-open-api/)
:::

### `REQ_<REQUEST_SERVER_NAME>`

To query a Request Server, the client sends a `REQ_` message.

There are several options that can be optionally specified:
...
Request
```
GET http://localhost:9064/req-trade?REQUEST.COUNTERPARTY_ID=1&REQUEST.DATE_FROM=1731369600000&DATE_TO=1730419200000&DETAILS.MAX_ROWS=5&DETAILS.CRITERIA_MATCH=DIRECTION%20%3D%3D%20%27BUY%27 HTTP/1.1
