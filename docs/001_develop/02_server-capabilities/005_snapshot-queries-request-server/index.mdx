...
* You can set up a `requestReply` that can accept [empty queries](#using-requestreply-with-no-parameters)
...
Note that you can also set up your requestReply to [accept empty requests](#using-requestreply-with-no-parameters).
...
If you want to your `requestReply` to accept empty data requests (where no parameters are returned), use `Unit` as part of your declaration. For example:
