...
Genesis Request Servers are defined in the `*-reqrep.kts` files or in compiled Kotlin classes extending `GPalRequestServer`.
...
requestReplies {
  ...
  requestReply(COMPANY) {
      config {
          criteriaOnlyRequest = true
      }
  }
...
###### COMPANY
The request server name is defaulted as no name is specified.
This request is criteria only, which means that criteria are evaluated on the database directly, when supported.
...
Using `criteriaOnlyRequest` enables enhancements targeted at SQL databases:

* Criteria evaluation on the database, where supported
* Sorting at the database level
* Native database pagination support

Internally, this uses the [`getByCriteria`](/develop/server-capabilities/data-access-apis/#getbycriteria) database operation. See the linked documentation for details.

When using criteria only mode, use [`criteriaTemplate`](#criteriatemplate), rather than `filter` type operations.
Criteria templates are merged with criteria on request, and evaluated eagerly on the database, where possible.
Filters, on the other hand, are always evaluated lazily; requests will therefore take more time and cost more memory and CPU.
...
Optionally a `config` block can be provided.
...
`name` is optional. Left unspecified, it is defaulted `REQ_{TABLE_NAME or VIEW_NAME}`.
...
When using criteria only request, the `request` block is ignored.
...
The `config` block is supported at both the `requestReplies` or file level, as well as the `requestReply` level.
Values specified at the file level, can be overwritten at the `requestReply` level.

| Property                         | Effect                                                                                                                                                            |
|----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| criteriaOnlyRequest              | Enables criteria only mode                                                                                                                                        |
| supportLazyPredicates            | Supports lazy criteria in criteria mode, see [here](../data-access-apis/#database-vs-lazy-evaluation) for more details                                            |
| timeoutInSeconds                 | Timeout in seconds                                                                                                                                                |
| maxRows                          | Max results in reply                                                                                                                                              |
| enableTypeAwareCriteriaEvaluator | Enhanced criteria evaluation when not using criteria only, see [here](../real-time-queries-data-server/#using-the-type-aware-criteria-evaluator) for more details |
...
`criteriaTemplate` is a way to enhance the criteria on an incoming request.
It has a similar effect to the `filter` functions; however, a criteria `String` is expected.
This string will be merged with the incoming `CRITERIA_MATCH` string.

`criteriaTemplate` should be preferred over `filter`-type functions when using criteria-only request mode, as it is more efficient.

Within `criteriaTemplate`, you have access to `userName`, `request` and `db`.
`userName` is of type `String`. `request` is of type `GenesisSet`.
`db` is a read-only version of `AsyncEntityDb` for database lookups to enhance criteria.

##### Static criteria

To apply static criteria, use the same syntax as normal.

```kotlin
requestReply("ALT_INSTRUMENT_RIC", ALT_INSTRUMENT_ID) {
    config {
        criteriaOnlyRequest = true
    }
    criteriaTemplate {
...
To include your Request Server definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.requestreply.pal` is included
- **module**: ensure `genesis-pal-requestserver` is included
- **script**: your `*-reqrep.kts` file(s), comma-separated if more than one
- **language**: `pal`
...
- **Gradle dependency**: add `implementation(genesis("pal-requestserver"))` to your application module's `build.gradle.kts` so `GPalRequestServer` is on the compile classpath
...
If you wish to run a dedicated process for a request server, the following gives an example using script files:
...
You can test using **compiled definitions** (classes extending `GPalRequestServer`) with `@PackageScan` or `@ProvidedInstance` instead of loading a script file. In that case no script file or `@TestScriptFile` is required — see [Testing compiled definitions](#testing-compiled-definitions) below. The rest of this section uses script-based definitions with `@TestScriptFile`.
...
If your Request Server is a **compiled definition** (a class extending `GPalRequestServer`) rather than a `.kts` script, use `@PackageScan` or `@ProvidedInstance` in place of `@TestScriptFile` to bring it into the test. Everything else — injecting a client, building the workflow, asserting on the reply — works exactly as shown above. You don't need a separate test dependency for the Request Server's client — `RequestClientSync` and `@GenesisTest`/`@PackageScan`/`@ProvidedInstance` all come from `testImplementation(genesis("testsupport"))` (see [Testing without a script file](/develop/server-capabilities/gpal-standards/migration-guide-script-to-compiled/#testing-without-a-script-file)).

Given this compiled Request Server:

```kotlin
package myapp.definition

import global.genesis.requestreply.pal.script.GPalRequestServer
...
class HelloWorldRequestServer : GPalRequestServer({
    // full request server dsl support from here, for example
    requestReply<Hello, World>("HELLO_WORLD") {
        replySingle { hello: Hello ->
            World("Hello, ${hello.name}!")
        }
...
The test class scans `HelloWorldRequestServer`'s package instead of loading a script file:

```kotlin
import global.genesis.message.core.workflow.message.requestReplyWorkflowBuilder
import global.genesis.testsupport.client.requestserver.RequestClientSync
...
import myapp.definition.HelloWorldRequestServer
import myapp.definition.World
import org.junit.jupiter.api.Test
import kotlin.test.assertEquals

@GenesisTest
@PackageScan(HelloWorldRequestServer::class)
class HelloWorldRequestServerTest {
    ...
    fun testHelloWorldRequestServer() {
        ...
        val result = client.sendRequest(helloWorldFlow, hello)
        assertEquals(1, result.size)
        assertEquals("Hello, John!", result[0].message)
    }
}
```

Or, to register just this one class without scanning its whole package, use `@ProvidedInstance` with a `KClass` reference instead — this is the form the framework's own test suite uses for compiled Request Servers:
...
class HelloWorldRequestServerTest {
    ...
    @ProvidedInstance
    private val helloWorldRequestServer = HelloWorldRequestServer::class

    // ... same client, workflow and test as above
}
```

`@PackageScan` and `@ProvidedInstance` each work standalone — you don't need both. If your process mixes script files and compiled definitions, combine either with `@TestScriptFile` on the same test class — all definitions are merged, exactly as they are at runtime.
