...
Genesis Streamers are defined in the `*-streamer.kts` files or in compiled Kotlin classes extending `GPalStreamer`.
...
To include your Streamer definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.streamer.pal` is included
- **module**: ensure `genesis-pal-streamer` is included
- **script**: your `*-streamer.kts` file(s), comma-separated if more than one
- **language**: `pal`
...
- **Gradle dependency**: add `implementation(genesis("pal-streamer"))` to your application module's `build.gradle.kts` so `GPalStreamer` is on the compile classpath
...
If you wish to run a dedicated process for a streamer, the following gives an example using script files:
