...
```kotlin
import global.genesis.karim.CreateTradeResponse
import global.genesis.karim.InstrumentClient
import global.genesis.karim.TradeInformation
import global.genesis.karim.TradeInformationClient
...
Includes authorisation so only users with access to the INSTRUMENT will see the data.
...
`requestAuth` provides entity-level authorisation on the request input similar to Event Handlers.
...
`responseAuth` provides entity-level filtering on the response similar to Request Servers.
...
If you are testing against a previous version of the framework, go to the [legacy](/develop/server-capabilities/testing-api/#integration-testing-legacy) section.
...
In this example, we shall test the following Web Endpoint. Note, the web handler scripts must exist in your test Genesis Home folder.
