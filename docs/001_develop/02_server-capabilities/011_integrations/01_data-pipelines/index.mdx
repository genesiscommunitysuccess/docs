...
If there is a requirement to stop the pipeline using the pipeline manager API, you must set a `routeID` on all routes created within the route builder function so that these routes can be removed from the Camel Context when the pipeline is stopped.
If no `routeId` is provided, one will be assigned by Camel internally, and the pipeline runtime will be unable to deregister the route. This means if the pipeline is stopped but the process is still running, Camel will continue to process files. See example below where `routeId` has been set:
