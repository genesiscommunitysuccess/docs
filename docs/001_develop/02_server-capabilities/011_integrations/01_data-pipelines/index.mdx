...
An `eventSink` delivers each pipeline element to an event handler as it flows through the pipeline (per-element sink position). This is different from `sendEventOnCompletion`, which fires once after the pipeline finishes. Use an `eventSink` when a pipeline's primary job is to forward each record as an event (for example, streaming to a FIX gateway).
...
The following example shows how to configure an `eventSink` that targets an explicit FIX gateway service, holds events until delivered, and throws an exception on any NACK:
