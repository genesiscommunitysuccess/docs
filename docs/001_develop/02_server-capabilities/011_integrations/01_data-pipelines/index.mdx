...
If you are already familiar with Camel APIs, you should feel right at home here. If not, [you can read more about them here.](https://camel.apache.org/manual/routes.html)

Passing in a route builder gives you the most flexibility and control over your routes logic, allowing for more advanced configurations, such as specifying processors and idempotency logic.
...
If no batch size is provided, the elements from the database poll will be returned individually and the latest index persisted will be of each record.
If a batch size is provided, the elements from the database poll will be batched into the size you have specified and returned as a list. The latest index persisted will be from the last record in that batch.
