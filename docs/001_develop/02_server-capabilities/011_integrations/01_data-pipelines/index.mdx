...
This source implementation is not recoverable, meaning on each invocation of this pipeline all records in the table will be processed.

:::

The source returns a flow of `Bulk` objects for your chosen entity which will need to be converted to the underlying record using a mapper, an example of which has been provided below:
...
If you want to use this source to subscribe to a view, you have the option of setting `backwardsJoin` to `true`. Note: in order to receive updates from changes to any non-primary table in your view, you must set this configuration in the pipeline source
regardless of whether the view itself has joins with `backwardsJoin` set to `true`.
This setting is defaulted to false but can be set to true as shown below:
...
This is a batch executable source for doing a bulk query on a table or view. It returns a flow of the object you have specified when defining the source:
...
This is a convenience source which allows you to easily wrap a custom function in a GPAL pipelines definition. For example, if you want to call an external REST API using one of our generated service classes.
...
A connector is a pipeline element that acts as both a source and a sink, consuming elements from one pipeline and emitting to another. This is particular useful for when you require blending multiple inbound streaming feeds together.
...
The `ackConnector` is the exact same as the `connector` except it incorporates the acknowledgement mechanism. The sink call does not return until the downstream sink has processed the element.
...
This handler works in the exact same way as `sendEventOnCompletion` above except the return type of the event can be specified. This can therefore be used when the return type of your custom event is anything other than EventReply.
