...
:::important
This is a single consumer source and can only be called on by one pipeline at a time, using the same camel file source for multiple pipelines will result in an error where the pipeline is not able to start.
:::
