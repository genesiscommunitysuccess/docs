...
Genesis Data Pipelines are defined in the `*-pipelines.kts` files or in compiled Kotlin classes extending `GPalPipeline`.
...
```
Within this block, the pipeline function is available, which is the starting point for creating a pipeline, and requires a name. 
This function returns a builder object, where you can chain calls to add a source, operators, error handlers and a sink to your pipeline.
In order to complete your pipeline, you must call the `build` function.
```kotlin
pipelines {
    pipeline(name = "My Pipeline")
        // chain builder calls here
        .build()
}
```
...
:::note
`camelFileSource` is a single consumer source and can only be called on by one pipeline at a time, using the same instance for multiple pipelines will result in an error where the pipeline and the process configured to run it are not able to start.
:::
...
:::important
If there is a requirement to stop the pipeline using the pipeline manager API, you must set a `routeID` on all routes created within the route builder function so that these routes can be removed from the `CamelContext` when the pipeline is stopped.
If no `routeId` is provided, one will be assigned by Camel internally, and the pipeline runtime will be unable to deregister the route. This means if the pipeline is stopped but the process is still running, Camel will continue to process files.
:::
...
- pollFromLatest (optional): when set to `true` and no persisted index exists for this source, polling starts from the current latest record in the table rather than from the beginning, skipping all existing records. This is useful when you only want to process new data from the point the pipeline first starts. Defaults to `false`.

:::note
`pollFromLatest` only takes effect when there is no previously persisted index for the given `source`. Once an index has been persisted, polling always resumes from that index regardless of this flag.
:::
...
When using the batching functionality, you also have the following flags available to set in the configuration:
- `delayBetweenBatches` (optional): this determines whether the source should wait before sending through the next batch of records. If set, the delay time is the same as `dbLookupDelayMs` as mentioned above. This flag is defaulted to `false`.
- `delayBetweenReads` (optional): this determines whether the source should wait before polling the db again after sending through the batches for that read. If set, the delay time is the same as `dbLookupDelayMs` as mentioned above. This flag is defaulted to `false`.
...
val batchPollSource = dbBatchQuery<TestIndexTable>(batchSize = 200) {
  source = "SINGLE_INDEX_SOURCE"
  index = TestIndexTable.ByIndex
  // set the following flag(s) to true if required, otherwise no need to include them when defining your source as they are defaulted to false
  delayBetweenBatches = true
  delayBetweenReads = true
}
...
This is a real-time bulk subscription to the database for a particular entity and can be used for tables or views.

:::important

This source implementation is not recoverable. On each invocation of this pipeline, all records in the table will be processed.

:::

The source returns a flow of `Bulk` objects for your chosen entity. This needs to be converted to the underlying record using a mapper, an example of which has been provided below:

```kotlin
// the following mapper class extracts the Trades record from each Bulk<Trades> object and returns it to the pipeline:
class TradesMapper : SuspendElementOperator<Bulk<Trades>, Trades> {
    override suspend fun apply(input: Bulk<Trades>): Trades? = when (input) {
        Bulk.Prime.Completed,
        is Bulk.Update.Delete -> null
        is Bulk.Prime.Record -> input.record
        is Bulk.Update.Insert -> input.record
        is Bulk.Update.Modify -> input.record
    }
}

// then in your pipelines script:
pipeline("TEST_DB_BULK_SUBSCRIBE")
        .source(dbBulkSubscribe<Trades>())
        .map(TradesMapper())
        .sink(TradesSink())
        .build()
```

If you want to use this source to subscribe to a view, you have the option of setting `backwardsJoin` to `true`. 

In order to receive updates from changes to any non-primary table in your view, you must set this configuration in the pipeline source, regardless of whether the view itself has joins with `backwardsJoin` set to `true`.

This setting is defaulted to false. Here is an example of how to set it to true:

```kotlin
pipeline("TEST_DB_BULK_SUBSCRIBE")
        .source(dbBulkSubscribe<Trades>() { backwardsJoin = true })
        .map(TradesMapper())
        .sink(TradesSink())
        .build()
```
...
This is a batch executable source for doing a bulk query on a table or view. It returns a flow of the object that you specified when you defined the source:

```kotlin
pipeline("TEST_DB_BULK_QUERY")
        .source(dbBulkQuery<Trades>())
        .sink(TradesSink())
        .build()
```
...
This is an abstract implementation of a real-time acknowledgement source. This source additionally has a send method, which can be used by any custom code to provide data to the pipeline.

The source is dependent on the calling code being a part of the same process as the pipeline using this source. Upudate your process definition to ensure this is done.
...
#### Database Sinks
Sinks that make modifications to the Genesis Application Database use a common type for the elements they consume; `DbOperation`.

This is a wrapper object, the type of which defines the type of database operation to perform, and also contains a record to use in the operation.
The record must extend TableEntity i.e. it must be a DAO object that was created as part of the Genesis code generation process, in order to be compatible with the Genesis database layer.

`DbOperation` has 4 subtypes: Insert, Modify, Upsert and Delete.

For modifications (Upsert and Modify operations types), you can supply a list of modified fields in order to perform a delta update. Only the fields specified will be updated in the database.

```kotlin
  DbOperation.Update(
    entity = myTrade,
    modifiedFields = listOf("FIELD_1", "FIELD_2")
  )
```
...
##### `dbSink`
This is a sink into the Genesis database. The sink operation for each input element (real-time) or execution (batch) occurs outside of any database transaction. Use this version of the sink if you want the behaviour to be 'best effort'.
...
##### `txDbSink`
This is a transactional sink into the Genesis database. The sink operation for each input element (real-time) or execution (batch) occurs within a single database transaction. Use this version of the sink if you want the behaviour to be 'all-or-nothing'.
...
This sink uses the Genesis messaging API to send an event to an event handler for every element that flows through the pipeline. This is different from the `sendEventOnCompletion` handler (see [Completion handlers](#completion-handlers)), which fires once after the pipeline finishes. Use an `eventSink` when a pipeline's primary job is to forward each record as an event (for example, streaming to a FIX gateway).

The `eventSink` separates three concerns:

- **What** — the event payload, built per element via `buildEvent`.
- **Where** — the destination. By default, the platform resolves the target service from the event name. You can explicitly pin a service using `to = service("serviceName")`. Pin the service explicitly when one event name is served by several processes (otherwise, name resolution is ambiguous).
- **How** — the delivery policy, configured in the `delivery` block.

The delivery policy controls how the platform handles timeouts, retries and negative acknowledgements (NACKs):

| Option | Description | Default |
|---|---|---|
| `timeout` | The timeout in seconds for each event delivery request. | Platform default |
| `retry` | The retry mode when a target is unavailable. Use `holdUntilDelivered()` to hold the event until the target is reachable (never drops the event). Use `holdWithDeadline(seconds)` to give up and throw an exception after the specified deadline. | `holdUntilDelivered()` |
| `onNack` | The action to take when a NACK is received. Use `throwOnNack()` to throw an exception and propagate the failure to `onOperationError`. Use `logOnNack()` to log the failure and continue. | `logOnNack()` |

To prevent data loss, any failure that occurs before the event is successfully sent (such as an unresolvable service or no reachable instances) is automatically held and retried according to your configured `retry` policy. This prevents the pipeline checkpoint from advancing and dropping the event.

Other exceptions that occur during or after transmission are propagated to `onOperationError` as normal.

###### Streaming each element to a FIX gateway

The following example configures an `eventSink` that targets an explicit FIX gateway service, holds events until delivered, and throws an exception on any NACK:

```kotlin
.sink(eventSink<FIXOutData, RawFixEvent> {
    event = "EVENT_SEND_RAW_FIX_MESSAGE"
    to    = service("REVERSE_AUDIT_FIX_GATEWAY")
    delivery {
        timeout = 10
        retry   = holdUntilDelivered()
        onNack  = throwOnNack()
    }
    buildEvent { RawFixEvent(element.message.toString()) }
})
```

### Connectors

A connector is a pipeline element that acts as both a source and a sink, consuming elements from one pipeline and emitting to another. This is particularly useful when you want to blend multiple inbound streaming feeds together.

#### connector

This is an example of how to use the connector in a pipeline.

```kotlin
val tradesConnector = connector<Trade>()

pipeline("TEST_CSV_ENCODER")
        .source(dbBulkQuery<Trade>())
        .transform(csvEncoder())
        .sink(tradesConnector)
        .build()


pipeline("TEST_TRADE_TO_KAFKA")
    // same connector object can be used as source for another pipeline
    .source(tradesConnector)
    .sink(kafkaSink) // see how to define a kafkaSink further down
    .build()
```

#### ackConnector

The `ackConnector` is the same as the `connector`, except it incorporates the acknowledgement mechanism. The sink call does not return until the downstream sink has processed the element.

```kotlin
val tradesConnector = ackConnector<Trade>()
...
val insertTradeOnCompletionHandler = sendEventOnCompletion<Bulk<Trade>, TradeDetails> {
            eventName = "EVENT_TRADE_LOGIC"
            buildEvent { context: PipelineContext<Bulk<Trade>> ->
                TradeDetails(
...
#### `sendCustomEventOnCompletion`

This handler works in the same way as `sendEventOnCompletion` above, except the return type of the event can be specified. This can therefore be used when the return type of your custom event is anything other than EventReply.

```kotlin

val customTradeOnCompletionHandler = sendCustomEventOnCompletion<Trade, TradeDetails, EventCustomTradeReply> {
            eventName = "EVENT_CUSTOM_TRADE"
            buildEvent { context: PipelineContext<Bulk<Trade>> ->
                TradeDetails {
                    // construct GenesisSet here
                }
            }
        }
...
pipeline("TEST_CSV_ENCODER")
        ...
        .transform(csvEncoder())
        .sink(streamSink)
        .onCompletion(customTradeOnCompletionHandler)
        .build()
```
...
- username (optional): the username used to create the queue connection - must be provided if password is set
- password (optional): the password used to create the queue connection - do not set if credentials are not required to establish a broker connection
...
To include your Data Pipeline definitions in a runtime process, configure the process as follows depending on whether you use script files, compiled definitions, or both.

### Using script files

- **package**: ensure `global.genesis.pipeline` is included
- **module**: ensure `genesis-pal-datapipeline` is included
- **script**: your `*-pipelines.kts` file(s), comma-separated if more than one
- **language**: `pal`
...
- **Gradle dependency**: add `implementation(genesis("pal-datapipeline"))` to your application module's `build.gradle.kts` so `GPalPipeline` is on the compile classpath
...
If you wish to run a dedicated process as a data pipeline, the following gives an example using script files:
...
In addition to the standard element, split, and transform operators, the Pipelines API also supports **context-aware** variants. These allow access to the `PipelineContext`, which is useful for transactional logic, metadata handling, or tracing.

#### Map operator (with context)
Interface:
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    fun interface SuspendElementContextOperator<INPUT, OUTPUT> {
      suspend fun apply(input: INPUT, context: PipelineContext<*>): OUTPUT?
    }
    ```
  </TabItem>
  <TabItem value="java">
    ```java
    public interface ElementContextOperator<INPUT, OUTPUT> {
      @Nullable
      OUTPUT apply(@NotNull INPUT element, @NotNull PipelineContext<?> context);
    }
    ```
  </TabItem>
</Tabs>
...
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    class MyCustomContextMapper : SuspendElementContextOperator<INPUT, OUTPUT> {
      override suspend fun apply(input: INPUT, context: PipelineContext<*>): OUTPUT? {
        // your logic here
      }
    }
    ```
  ...
  <TabItem value="java">
    ```java
    public class MyCustomContextMapper implements ElementContextOperator<INPUT, OUTPUT> {

      @Nullable
      @Override
      public OUTPUT apply(@NotNull INPUT element, @NotNull PipelineContext<?> context) {
        // your logic here
      }
    }
    ```
...
#### Split operator (with context)
Interface:
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    fun interface SplitContextOperator<INPUT, OUTPUT> {
      suspend fun apply(input: INPUT, pipelineContext: PipelineContext<*>): Flow<OUTPUT>
    }
    ```
  </TabItem>
  <TabItem value="java">
    ```java
    public interface RxSplitContextOperator<INPUT, OUTPUT> {
      @NotNull
      Flowable<OUTPUT> apply(@NotNull INPUT element, @NotNull PipelineContext<?> context);
    }
    ```
  </TabItem>
</Tabs>
...
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    class MyCustomContextSplitter : SplitContextOperator<INPUT, OUTPUT> {
      override suspend fun apply(input: INPUT, pipelineContext: PipelineContext<*>): Flow<OUTPUT> {
        // your logic here
      }
    }
    ```
  ...
  <TabItem value="java">
    ```java
    public class MyCustomContextSplitter implements RxSplitContextOperator<INPUT, OUTPUT> {

      @NotNull
      @Override
      public Flowable<OUTPUT> apply(@NotNull INPUT element, @NotNull PipelineContext<?> context) {
        // your logic here
      }
    }
    ```
...
#### Transform operator (with context)
Interface:
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    fun interface StreamContextOperator<INPUT, OUTPUT> {
      suspend fun process(inputFlow: Flow<INPUT>, context: PipelineContext<*>): Flow<OUTPUT>
    }
    ```
  </TabItem>
  <TabItem value="java">
    ```java
    public interface RxStreamContextOperator<INPUT, OUTPUT> {
      @NotNull
      Flowable<OUTPUT> process(@NotNull Flowable<INPUT> stream, @NotNull PipelineContext<?> context);
    }
    ```
  </TabItem>
</Tabs>
...
<Tabs defaultValue="kotlin" values={[{ label: 'Kotlin', value: 'kotlin', }, { label: 'Java', value: 'java', }]}>
  <TabItem value="kotlin">
    ```kotlin
    class MyCustomContextTransformer : StreamContextOperator<INPUT, OUTPUT> {
      override suspend fun process(inputFlow: Flow<INPUT>, context: PipelineContext<*>): Flow<OUTPUT> {
        // your logic here
      }
    }
    ```
  ...
  <TabItem value="java">
    ```java
    public class MyCustomContextTransformer implements RxStreamContextOperator<INPUT, OUTPUT> {

      @NotNull
      @Override
      public Flowable<OUTPUT> process(@NotNull Flowable<INPUT> stream, @NotNull PipelineContext<?> context) {
        // your logic here
      }
    }
    ```
