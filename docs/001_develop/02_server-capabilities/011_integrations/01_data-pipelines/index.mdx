...
This is a convenience source that enables you to easily wrap a custom function in a GPAL pipelines definition. Use this, for example, if you want to call an external REST API using one of our generated service classes.
...
val saveDocumentOnCompletionHandler = sendCustomEventOnCompletion<Trade, TradeDetails, EventSaveDocumentReply> {
            eventName = "EVENT_SAVE_DOCUMENT"
            buildEvent { context: PipelineContext<Bulk<Bond>> ->
                GenesisSet {
...
pipeline("TEST_CSV_ENCODER") {
        source(dbBulkQuery<Trade>())
            ...
            .transform(csvEncoder())
            .sink(streamSink)
            .onCompletion(saveDocumentOnCompletionHandler)
}
```
