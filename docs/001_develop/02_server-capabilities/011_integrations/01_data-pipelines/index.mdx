...
If a `batchSize` is provided, the elements from the database poll will be batched into the size you have specified and returned as a list. The persistence manager will use the index from the last record of this batch, this means if the system went down whilst in the middle of processing a batch of records,
it would recover by continuing from the beginning of that batch.
