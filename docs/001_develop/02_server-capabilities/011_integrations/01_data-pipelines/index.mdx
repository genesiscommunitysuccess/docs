...
###### Batch
A Batch pipeline consumes data using a request/response based model, utilizing data sources that return a fixed-size result set at a given point in time. Examples include:
...
###### Real-Time
A real-time Pipeline consumes data via a 'push' model, utilizing data sources that continuously deliver data based on subscriptions. Examples include:
...
If no `batchSize` is provided, the elements from the database poll will be returned individually and the persistence manager will use the index from each individual record to keep track of what has been processed.
This means that the index of each record will be persisted after it reaches the sink. This gives "exactly once" guarantee.
If a `batchSize` is provided, the elements from the database poll will be batched into the size you have specified in order of the index provided and returned as a list. The persistence manager will use the index from the last record of this batch, this means if the system goes down whilst in the middle of processing a batch of records,
it would recover by continuing from the beginning of that batch. This gives "at least once" guarantee. This option will greatly increase processing speeds when working with large amounts of data.
...
You can configure Camel routes by using the route builder function. You are not limitied to a single route, and can create as many routes as needed.
