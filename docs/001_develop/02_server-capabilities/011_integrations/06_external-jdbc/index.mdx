...
- **[Configuration](./01_configuration/)** — Set up `jdbcUrl` and connection properties for external SQL databases.
- **[Repository modules](./02_repository-modules/)** — Interface-based access with `@RepositoryModule`, `@SqlQuery`, and `@SqlUpdate`.
- **[Direct connection API](./03_direct-connection-api/)** — Low-level `SqlDatabaseConnection`, fluent query/statement API, and ExecutableQuery.
