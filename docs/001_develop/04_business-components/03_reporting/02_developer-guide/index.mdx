...
import { ReportingV2 } from '@genesislcap/pbc-reporting-ui'
ReportingV2.RapidReporting;

@customElement({
  name: 'reporting-pbc',
  template: html`<rapid-reporting-v2></rapid-reporting-v2>`,
})
...
You need to opt into the version two of the PBC to get the latest features. If you've installed it as a component manually then the code snippet already instructs you to do this. If you've installed via `genx` or Genesis Create then you'll need to make the following change in your `reporting/routes.ts` file.
...
export const reporting: AppRoute = {
  ...
  path: 'reporting',
  name: 'reporting',
  element: async () => (await import('@genesislcap/pbc-reporting-ui')).ReportingV2.RapidReporting,
...
Specifically, you need to add the `ReportingV2` namespace as part of your dynamic import.
...
This PBC supports many right code permissions for granular control over what each user/group can do.

| Permission | Description |
|------------|-------------|
| `ViewReportConfigs` | View report configurations (both the overall list and the individual configuration), but not delete, edit, or create them. |
| `EditReportConfigs` | Create and edit report configurations. With `EditReportConfigs` disabled but `ViewReportConfigs` enabled a user can open a report configuration but they can't save any changes. Requires `ViewReportConfigs`. |
| `DeleteReportConfigs` | Delete report configurations. Requires `ViewReportConfigs`.|
| `ViewReportDestinations` | View report destinations, but not delete, edit, or create them. If the user has `EditReportConfigs` but not `ViewReportDestinations` then they can't view or select them when configuring a report. |
| `EditReportDestinations` | Create and edit report destinations. Requires `ViewReportDestinations`. |
| `DeleteReportDestinations` | Delete report destinations. Requires `ViewReportDestinations`.  |
| `RunReport` | Execute an existing report config, and view generated reports (even if `ViewReport` isn't enabled). |
| `ViewReport` | View generated reports. |
| `DeleteReport` | Delete generated reports. Requires `RunReport` or `ViewReport`. |
