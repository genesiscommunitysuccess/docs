...
When a report is run, it will run as a user:

* **Run button** : The user who clicked the button for any "ad-hoc" running of the report (from the Run configuration's screen)
* **Scheduled report run** : The user who last edited the report configuration

This matters because the data sources configured for a report may have authorization permissions set, for example, you may be using **Orders** data and the user may only see rows for **Counterparty's** that they have been permitted to see in the application.

If you see any data discrepancies, this is likely the cause. If you are unsure/unaware of the authorization that has been set up in your application, speak to a developer who can check the underlying data source for any authorization configuration being applied to the data source.
