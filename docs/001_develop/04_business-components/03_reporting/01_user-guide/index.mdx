...
The Document Generator component developer documents give an example of how to reference data in templates. With the reporting engine you do not need to worry around the code elements in it's examples, but the templating guidance can help. An important consideration is that where used, 
[column renames](#column-renames) need to be referenced in the template, not the actual UPPERCASE_FORMATTED field name - which must be used where column renames have not been set.
