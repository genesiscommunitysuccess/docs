...
When using GCS as your storage solution, you must have a bucket already created which the manager will connect to, you must set this using the following configuration:
```kotlin
item(name = "GCS_BUCKET_NAME", value = "<bucketName>")
```
### Optional Configurations

You can also provide a folder prefix if you would like the storage manager to save all files under a specific path within your bucket. This configuration is not required but can be set like so:
```kotlin
item(name = "GCS_FOLDER_PREFIX", value = "<folderPrefix>")
```

If you require requests to be executed for a particular project ID, this can be provided using the following system definition:
```kotlin
item(name = "GCS_PROJECT_ID", value = "<projectId>")
```
Otherwise the system will use the project id of the default instance within your Google Cloud Platform.
