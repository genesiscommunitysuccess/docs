...
For assets to be included in the generated document, you must also either link the template to the assets or provide it as explained in [Providing a Template/Asset](/develop/business-components/doc-generate/developer-guide/#providing-a-templateasset). The easiest is using the UI. This GIF gives an example of uploading a template, an asset, and linking them:
...
- `templateId` - see [Providing a Template/Asset](/develop/business-components/doc-generate/developer-guide/#providing-a-templateasset)
...
- `assetReferences` - see [Providing a Template/Asset](/develop/business-components/doc-generate/developer-guide/#providing-a-templateasset)
...
#### Providing a Template/Asset

In order to provide a template to the document generator, both of the configurations listed below (`DocumentContentConfiguration`/`DocumentStorageConfiguration`) will take in a `templateId`.
This is a non-nullable String which represents either the `FileStorageId` of the uploaded template or the name of the template file.

In order to provide an asset to the document generator which hasn't been linked to the template as mentioned [here](/develop/business-components/doc-generate/developer-guide/#creating-a-template-and-other-assets), the `DocumentStorageConfiguration`
will optionally take in a list of `assetReferences` which can either be the storage ID of the uploaded but unlinked asset or the name of the asset file.

If a file name is provided for either of the above, the document generator will attempt to resolve the file from the following locations in order of priority:

1. document management system
2. cfg directory of your project
3. one of the directories listed in the following system definition:

```kotlin
// comma separated list of directories for the generator to look for the file in as per the provided file name
item("TEMPLATE_FILE_LOCATIONS", "/data/my-templates1/,/data/my-templates2/")
```

If multiple files are found within the document manager with the same name, the latest file uploaded will be returned.

If multiple files with the same name are present across the locations provided in `TEMPLATE_FILE_LOCATIONS`, the file will be returned in order of the location mentioned in the system definition.
...
You can provide XML templates to the document generator which will also use Thymeleaf to generate the resultant XML report.

Please see [HTML example usage](/develop/business-components/doc-generate/developer-guide/#html-example-usage) above for details on how to lay out your template for the Thymeleaf engine.
