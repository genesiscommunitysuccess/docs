...
The `SELF` entity type has two distinct behaviors depending on whether you populate the `ENTITY_ID` field:

* **Entity-blank** (where `ENTITY_ID` is unset or empty): Resolves the recipient to the sender of the notification.
* **Entity-scoped** (where `ENTITY_ID` is populated with a username): Scopes delivery to that specific user. The notification is delivered only if the sender of the notification matches the `ENTITY_ID`. This prevents duplicate notifications on shared topics where multiple users have active subscriptions.
