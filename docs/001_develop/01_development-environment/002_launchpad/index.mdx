...
#### Entity references
Entity references are useful, because they enable you to add attributes from different entities to form [views](#views) Once you have created an entity, its attributes are available for use in other entities. 

When you click at the end of the `Type` field, the dropdown list includes all the primitives (attributes that are not a primary key or index) and entity references (attributes that are primary keys or indices).
...
Underneath, this displays the entities you can join with to create a view. These are listed because your root entity includes entity references to attributes in those tables.
 
...
3.	Click on a `Select` checkbox for an attribute to add it to or remove it from the view.
