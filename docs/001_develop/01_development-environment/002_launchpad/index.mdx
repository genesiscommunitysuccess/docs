...
![Detect Excel button](/img/genesis-launchpad/detect-excel-button.png)

Genesis Create attempts to create a data model from the workbook that you have selected. If it is successful, it displays information about the data model it has created. For example, here it has created three entities and one view.

![Schema detected](/img/genesis-launchpad/detected-schema.png)

Note that the display includes advice on how to check that you are happy with the data model produced.

Once you have closed the dialog, you can see the details of the entities and views that have been produced, and you can work your way through the steps in Genesis Create to complete the project.

:::tip
You must start with an empty project. If you have a another project loaded:

Click on the three dots at the top right of the window and select `New`. You are warned that this clears any current changes. If necessary, cancel, then click on the three dots again and select `Save`. This downloads the project as a JSON file.
:::

##### Adjusting an Excel workbook
The real world is messy, and genesis Create will not be able to extract a data model from every workbook that you select. However, if you know the general rules for how the extraction process works, you can adjust your workbook and try again. 

:::tip
It is easier to adjust the Excel workbook before extracting the data model than it is to adjust the code after you have extracted.
:::
...
- The data for the entity begins at row 2. The data in each column determines the type of the attribute (in row 1). Where th process cannot determine a type, it defaults to STRING. The data in each worksheet is saved in a separate CSV file.
...
It is essential to look carefully at your workbook and adjust it to suit these rules. 

- In particular, make sure that the attribute that you want to be the primary key is in column A.
- It is worth adjusting the data in row 1 so that this produces meaningful attribute names. In particular, long names should be avoided.

:::tip
The data is from your workbook is not extracted at this stage. You can extract the data later, after you have generated a project in Create and built the app in Launchpad.
...
<iframe src="https://scribehow.com/embed/Download_and_run_your_Create_project_locally__IT-lknXzTDKFn3YUFb2Ftw?skipIntro=true&removeLogo=true&as=scrollable" width="100%" height="640" allowfullscreen frameborder="0"></iframe>
