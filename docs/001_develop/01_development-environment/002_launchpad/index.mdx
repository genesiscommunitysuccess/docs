...
You can create a data model with entities ([tables](/develop/server-capabilities/data-model/#tables))and [views](/develop/server-capabilities/data-model/#views) directly from an Excel spreadsheet. 

In the Entities main screen, there is a button that enables you to select a spreadsheet and preview how Genesis Create interprets it.

![Detect Excel button](/img/genesis-launchpad/detect-excel-button.png)

For example, a spreadsheet has been previewed here has found three entities and one view.

![Schema detected](/img/genesis-launchpad/detected-schema.png)

The preview includes suggestions for checking the schema to see that it matches your expectations. You can then go back to the spreadsheet and edit it to improve the result when you preview it again.
