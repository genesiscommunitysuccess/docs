...
:::info
If you already have an entity model in Genesis Create, this button is not displayed.

To clear the current entity model and start a new project, click on the three dots at the top right of the window and select `New`. You are warned that this clears any current changes. If necessary, cancel, then click on the three dots again and select `Save`. This downloads the project as a JSON file.
:::
