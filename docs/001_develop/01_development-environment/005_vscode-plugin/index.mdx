...
- You can edit the source folder path by clicking on `Setup Genesis View` or clicking on left panel `settings button > Settings` and searching for "genesis ui".

Clicking on the button will open the extension settings screen.
You can configure the source folder path and also OpenAI key in the extension's settings to use `Genesis View`.
...
:::tip
Ensure you install `literally-html` to enable the full functionality of snippets and items from the HTML catalogue.
:::
...
PS: Genesis View doesn't support React or Angular generation yet.
...
- Doesn't support React or Angular yet.
