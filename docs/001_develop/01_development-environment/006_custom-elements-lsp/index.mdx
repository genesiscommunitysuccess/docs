...
```
	...
	- The above codeblock is the default settings which are enabled for a new Genesis project. Unless you have restructured the project manually the above config will be all you need.
	- Edit the codeblock above to match your requirements if required.
...
| `designSystemPrefix`   | **Mandatory**.  Used to work with `%%prefix%%` to handle components registered as part of a design system. New Genesis projects will likely just want to use `rapid` as the design system. |
...
:::caution
If you use a monorepo pattern with workspaces, you must account for potential hoisting of the TypeScript library in the `node_modules` to a parent directory. If npm has hoisted your typescript install, make the account for this when you set `srcRootFromTSServer`.
...
| `fastEnable`           | **Optional**.  Enables Microsoft FAST parsing of local components, which is the dialect of the Genesis components. You need to enable the plugin too for full functionality (enabled by default in the above configuration).  |
...
For more information about the plugin, how to set it up, and troubleshooting steps, see the published package on npm [here](https://www.npmjs.com/package/@genesiscommunitysuccess/custom-elements-lsp).
