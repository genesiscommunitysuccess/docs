...
- **`formatDateTimeTimestampWithMilliseconds`** - Formats UNIX timestamps into ISO date and time strings with milliseconds (`YYYY-MM-DD HH:MM:SS.SSS`), providing millisecond-level precision.
...
import {
  ...
  formatDateTimeTimestampWithMilliseconds,
  formatTimestamp,
...
console.log(formatDateTimeTimestampWithMilliseconds(Date.now())); // Outputs current date and time with milliseconds in YYYY-MM-DD HH:MM:SS.SSS format
