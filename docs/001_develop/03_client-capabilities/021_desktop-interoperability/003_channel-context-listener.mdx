...
This is a utility element that listens to FDC3 app channel for the specified context.

Use cases:
* Listen to events for a specified context on an FDC3 app channel
...
|---|---|---|---|
|config|`AppChannelListenerConfig`|An array of objects which configure which channel, context type and a handler for channel context event payloads|`<fdc3-channel-event :mappingFunction="${() => ({TICKER}) => ({ id: { ticker: TICKER, cusip: CUSIP } })}"`|
