...
Declaration
```html
<InteropNotificationListener></InteropNotificationListener>
```

Usage
```tsx
import { InteropNotificationListener } from '@genesislcap/rapid-design-system/react';
...
export function MyComponent() {
    ...
    return (
        <InteropNotificationListener
            ref={interopNotificationListenerRef}
            resourceName="ALL_NOTIFY_ALERT_RECORDS"
        >
        </InteropNotificationListener>
    );
}
...
```
        ...
        <tr>
            ...
            <td>Position of toast notifications on the screen. Can be `top-left`, `top-right`, `bottom-left`, or `bottom-right`</td>
<td>
```typescript
    <rapid-interop-notification-listener toast-position="top-right">
```
</td>
        </tr>
    </tbody>
