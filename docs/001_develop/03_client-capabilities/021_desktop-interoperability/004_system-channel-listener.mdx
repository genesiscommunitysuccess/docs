...
This is a utility element that listens to FDC3 system channel for the specified system.

Use cases:
* Listen to events for a specified system on an FDC3 app channel

## Examples

This will listen to the specified app channels for changes in the specified system.
...
|---|---|---|---|
|config|`SystemChannelListenerConfig`|An array of objects which configure which context type and a handler for channel event payloads|`<fdc3-system-channel-listener :config="${() => [ { contextType: 'fdc.instrument', callback: (message) => console.log(JSON.stringify(message)} ]"`|
