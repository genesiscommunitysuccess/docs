...
The `Date`/`DateTime` [common expression](#date-operations) can be also used as extension functions/operators. The supported date format for `String` and `Long` fields remains the same.
...
To illustrate this functionality, here are a few examples using a TRADE_DATETIME field with `DateTime` type.
