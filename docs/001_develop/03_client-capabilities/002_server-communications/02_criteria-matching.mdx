...
This page looks at how you can apply criteria matching when you connect to [real-time queries (Data Server)](/develop/server-capabilities/real-time-queries-data-server/) and [snapshot queries (Request Server)](/develop/server-capabilities/snapshot-queries-request-server/) on the server.
