...
The platform provides an additional set of extension methods over the base Groovy capabilities to improve developer experience and usability.
...
- `String.containsWordsStartingWithIgnoreCase(String searchStr, Character delimiter = ' ')`
    ...
    This returns `true` when the field contains a word with the string. Casing is ignored. The default delimiter is a whitespace character, but an optional second parameter can be passed to specify a different delimiter.
...
#### `Date`/`DateTime` extension functions

The `Date`/`DateTime` [common expression](#date-operations) can be also used as extension functions/operators and the supported date format for `String` and `Long` fields remains the same.
...
And the same is applicable the other way around: `DateTime` fields can be directly compared against `String` and `Long` values.

To better illustrate this functionality we can show a few examples using a TRADE_DATETIME field with `DateTime` type.
...
|-----------------------------------|--------------------------------|--------|
| `TRADE_DATETIME == 1325376000000` | `TRADE_DATE == '2012-01-01'`   | true   |
| `TRADE_DATETIME != 1325376000000` | `TRADE_DATE != '2012-01-01'`   | false  |
| `TRADE_DATETIME >= 1325376000000` | `TRADE_DATE >= '2012-01-01'`   | true   |
| `TRADE_DATETIME <= 1325376000000` | `TRADE_DATE <= '2012-01-01'`   | true   |
| `TRADE_DATETIME > 1325376000000`  | `TRADE_DATE < '2012-01-01'`    | false  |
| `TRADE_DATETIME < 1325376000000`  | `TRADE_DATE > '2012-01-01'`    | false  |
...
It is important to note that both the epoch time value or the date in `String` format can be placed as the left operand of the expression too.

:::important
`DateTime` and epoch time UTC comparisons will ensure Dataserver queries attempt to match the correspondent Dataserver indices (if available) to improve query performance. Therefore, this type of comparison is the recommended approach for all `DateTime` related queries.
:::
