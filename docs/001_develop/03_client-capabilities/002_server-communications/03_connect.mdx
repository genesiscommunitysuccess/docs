...
- **HTTP Polling Configuration**: When using `DefaultHttpConnect`, tune `POLLING_INTERVAL_MAP` and `POLLING_FREQUENCY` per resource to balance data freshness with server load. See the [Configuration (HttpConnectConfig)](#configuration-httpconnectconfig) section for an example.
