...
The required class, template, and base styles are exported as part of the package, enabling a client to customize the styling of the layout system via design system extensions.

:::tip
All the customizable styles of the layout system are contained within the styles for [`<foundation-layout>`](./docs/api/foundation-layout.foundationlayout.md). So if you want to customize the styles in addition to setting the css variables, you only need to set the styles here.
:::
...
- **reload-buffer**: the time in milliseconds between each reload of the layout is reloaded. The default
is 500ms. In this case, the layout is only reloaded if the child elements of the layout region are manipulated
once every 500ms. This is to stop the layout being reloaded for every single item during initialization.
The higher the value is, the more performant the component is - but the first load will appear to take longer.
- **auto-save-key**: optional string; if set, this enables autosaving the layout under this key in local
...
The JavaScript API is [accessed through the methods on the root layout object](./docs/api/foundation-layout.foundationlayout.md) and allows for saving and loading the layout state, and for dynamically adding items to the layout at runtime.

### Dynamic registration and adding items

To have a pane displayed on the layout system, it must be *registered* with the layout system. When using the [declarative API](#declarative-html-api), the layout system takes care of this for you, but as you start to add items dynamically and then serialize the layout, you need to consider which panes are registered. See this [contained example](#contained-example), which allows users to add pre-determined items to the layout dynamically.
...
This API enables you to register an item at runtime, but it will not be displayed in the layout. This could be used to register components in anticipation of displaying them when loading a serialized layout - [see this example](#loading-serialized-layouts).
...
Static function to read a layout config. It returns a list of all the required registrations required to load it in the layout system. See this [example](#loading-serialized-layouts).
...
The JavaScript API can be used to save and load layout states manually. This only describes the state of the dynamic layout itself. It is the responsibility of each component within the layout to serialize its own state, if required.
...
Loads a serialized layout. All items that are described in the config to load must already be registered with the layout system - using either the declarative or JavaScript API. If there are items missing (because either items are missing or there is a mismatch of registered names), then a `LayoutUsageError` will be thrown containing the names of the missing items. Alternatively, you can request placeholder items to be added.

## Events

### [Emitted events](./docs/api/foundation-layout.layoutemitevents.md)

Some actions that are performed by the user interacting with the layout emit events. See the [API document](./docs/api/foundation-layout.layoutemitevents.md) for details of these events and when they're emitted. Interacting with these events enables your client code to interact dynamically with the layout, such as enabling or disabling buttons, or adding items to the layout when they're removed or added.
...
To ensure that every item gets the button as expected, apply the custom button definitions as early as possible. If you are using the html API, then you'll probably want to apply the definitions in the template.
...
If you are only using the javascript API, then just apply the property as soon as you can.
```typescript
...
When you have enabled autosave, you are still able to use the manual [serializing commands](#serializing-layout).
...
This is an elaborate example of using the JavaScript API with consideration of the registered names. Before reading this example, you should familiarize yourself with the [API Section](/develop/client-capabilities/layout-management/foundation-layout/#javascript-api):
...
```javascript
const layout = document.querySelector('foundation-layout'); // as FoundationLayout in TypeScript;
...
As shown in [this example](/develop/client-capabilities/layout-management/foundation-layout/#binding-events-inline-in-the-declarative-api), you need to wrap html that uses fast bindings and event listeners into their own custom components. In this section, we explain why this is necessary. 

For this, it is required that we make use of `cloneNode` to allow the layout to add multiple instances of a registered component.
...
## API documentation
For further details, see our [API docs](./docs/api/index.md).
