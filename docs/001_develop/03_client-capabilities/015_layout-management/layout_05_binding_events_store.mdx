...
```tsx
import { Checkbox, Layout, LayoutItem } from '@genesislcap/rapid-design-system/react';
...
function WrapperTwo({ isSelected }: { isSelected: boolean }) {
  return <Checkbox checked={isSelected}>Checkbox</Checkbox>;
}
...
export function WrapperTwo({ checked }: { checked: boolean }) {
  return <Checkbox checked={checked}>Checkbox</Checkbox>;
}
