...
export function ContainerElement() {
  return (
    <my-element onChange={/* handler */} property={/* value */} />
  );
}
```
...
@Component({
  ...
  template: `<my-element (change)="onChange($event)" [property]="propertyValue"></my-element>`,
  standalone: true,
...
The question you may ask with the above answer is how you access to the properties that are represented in binding expressions (for example, event handlers and property bindings). If they were statically defined functions or `const` values on the class, then you can move them onto the `ContainerElement` class. More likely though they are dynamically generated.
...
export function MyElement() {
  ...
  return (
    <>
      ...
      <rapid-checkbox checked={isSelected} />
    </>
  );
}
```
...
@Component({
  selector: 'my-element',
  template: `
    <rapid-checkbox [(checked)]="isSelected">Checkbox</rapid-checkbox>
    <rapid-checkbox [checked]="isSelected"></rapid-checkbox>
  `,
  standalone: true,
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
...
```tsx
function WrapperOne({ isSelected, onChange }: { isSelected: boolean; onChange: (v: boolean) => void }) {
  return (
    <rapid-checkbox checked={isSelected} onChange={(e) => onChange(e.target.checked)}>
      Checkbox
    </rapid-checkbox>
  );
}

function WrapperTwo({ isSelected }: { isSelected: boolean }) {
  return <rapid-checkbox checked={isSelected}>Checkbox</rapid-checkbox>;
}
...
export function MyElement() {
  ...
  return (
    <rapid-layout>
      <rapid-layout-item>
        <WrapperOne isSelected={isSelected} onChange={setIsSelected} />
      </rapid-layout-item>
      <rapid-layout-item>
        <WrapperTwo isSelected={isSelected} />
      </rapid-layout-item>
    </rapid-layout>
  );
}
...
```typescript
// Define wrapper-one and wrapper-two components with CUSTOM_ELEMENTS_SCHEMA, then use them inside rapid-layout
```
...
export function WrapperOne({ onToggle }: { onToggle: (val: boolean) => void }) {
  return (
    <rapid-checkbox onChange={(e) => onToggle(e.target.checked)}>
      Checkbox
    </rapid-checkbox>
  );
}

export function WrapperTwo({ checked }: { checked: boolean }) {
  return <rapid-checkbox checked={checked}>Checkbox</rapid-checkbox>;
}
...
export function MyElement() {
  ...
  return (
    <>
      <WrapperOne onToggle={setCheckboxToggle} />
      <WrapperTwo checked={checkboxToggle} />
    </>
  );
}
```
...
```typescript
// Use a shared store/service for checkboxToggle; wrapper components bind to it
// via (change) and [checked], mirroring the Genesis store pattern above.
```
