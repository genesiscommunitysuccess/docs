...
There's still got a big issue here though - the `isSelected` variable isn't linked between the two components anymore. To handle this, you need to use a state management library, such as the `foundation-store` module that is included in the platform.
