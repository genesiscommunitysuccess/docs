...
export function Analytics() {
  const showIndexFunds = true; // fixed at init — do not toggle after layout loads

  return (
    <>
      <button className="toggle">Toggle Index</button>
      <rapid-layout>
        <rapid-layout-region>
          <rapid-layout-item>
...
```typescript
// Use *ngIf for the index-funds pane at layout init only — same constraint as Genesis when
```
...
export function ContainedExample() {
  const layoutRef = useRef<any>(null);
  let addedPaneCount = 0;
  ...
  useEffect(() => {
    ...
    p1.innerHTML = 'Ex 1';
    // ...register items '2' and '3' similarly

    layout.registerItem('1', [h1, p1]);
    layout.tryLoadLayoutFromLocalStorage();
  }, []);

  const addItem = (registration: string) => {
    layoutRef.current?.addItem({
      registration,
      title: `${registration} (${(addedPaneCount += 1)})`,
      closable: true,
    });
  };

  const missingItemOverride = (missingItem: string) => `Missing Item: ${missingItem}`;

  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gridAutoRows: 'minmax(7vh, auto)' }}>
      <div>
        <rapid-button onClick={() => addItem('1')}>Test 1</rapid-button>
        <rapid-button onClick={() => addItem('2')}>Test 2</rapid-button>
        <rapid-button onClick={() => addItem('3')}>Test 3</rapid-button>
      </div>
      <div style={{ gridRowStart: 2, gridRowEnd: 12 }}>
        <rapid-layout
          ref={layoutRef}
          auto-save-key="layout-preview-contained-example"
          missingItemPlaceholder={missingItemOverride}
        />
      </div>
    </div>
  );
...
```typescript
// Use @ViewChild on rapid-layout and the same registerItem / addItem API in ngAfterViewInit
```
