...
import { Button, Layout } from '@genesislcap/rapid-design-system/react';
...
export function ContainedExample() {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gridAutoRows: 'minmax(7vh, auto)' }}>
      <div>
        <Button onClick={() => addItem('1')}>Test 1</Button>
        <Button onClick={() => addItem('2')}>Test 2</Button>
        <Button onClick={() => addItem('3')}>Test 3</Button>
      </div>
      <div style={{ gridRowStart: 2, gridRowEnd: 12 }}>
        <Layout
          ref={layoutRef}
          autoSaveKey="layout-preview-contained-example"
