...
export class ExampleComponent extends GenesisElement {
  checkbox: Checkbox;
...
As explained in the [lifecycle info section](#element-lifecycle-gating), this component may have its `disconnectedCallback` and `connectedCallback` lifecycle at unnecessary times, effectively wasting time re-initialising a potentially heavy resource.
