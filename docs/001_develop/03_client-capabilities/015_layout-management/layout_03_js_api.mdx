...
To have a pane displayed on the layout system, it must be registered with the layout system. When using the [declarative API](./layout_02_html_api.mdx), the layout system takes care of this for you, but as you start to add items dynamically and then serialise the layout, you need to consider which panes are registered. See [this contained example](../client-layout-management-examples/#contained-example), which allows the user to add pre-determined items to the layout dynamically.
...
The JavaScript API can be used to save and load layout states manually. This only describes the state of the dynamic layout itself. It is the responsibility of each component within the layout to serialise its own state, if required. To enable autosaving the layout [see here](../client-layout-management-examples/#autosaving-layout).

## Methods

For more info with these methods see the method section of the API docs [here](../docs/api/foundation-layout.foundationlayout/#methods).
