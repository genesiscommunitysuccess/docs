...
import { Layout } from '@genesislcap/rapid-design-system/react';

export function MyLayout() {
  const layoutRef = useRef<HTMLElement & { addItem: (...args: unknown[]) => void }>(null);
...
@Component({
  selector: 'my-layout',
  template: `<rapid-layout #layout></rapid-layout>`,
  standalone: true,
...
export class MyLayoutComponent {
  ...
  doSomething() {
    (this.layoutRef.nativeElement as any).addItem(/* ... */);
  }
}
```
