...
This function has a side effect because it reads `.users.names` from an external store. The unit tests could be written to have code which sets up the store with data and then tests the function. However, this has issues:
...
The ensures that before every test runs, all `sinon` mocks are cleared and the code runs as normal. This isn't strictly necessary for our simple example.
