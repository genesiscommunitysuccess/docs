...
Notice that if a rule is highlighted in red when it's partially defined (e.g. you've chosen the field but not an operator), then the model payloadon the right-hand side won't change. This is because the back-end model doesn't support partial rules. It is possible to look at the underlying model,, which contains a tree structure even for partial rules.
