...
Declaration
```html
<RapidValueExpressionBuilder></RapidValueExpressionBuilder>
```
...
import {
  ...
  type RapidValueExpressionBuilderRef,
...
export function MyComponent() {
  ...
  const configRef = useRef(valueConfig);
  configRef.current = valueConfig;

  const setBuilderRef = useCallback((el: RapidValueExpressionBuilderRef | null) => {
    if (el) {
      el.valueConfig = configRef.current;
    }
  }, []);

  const handleChangedValueExpression = useCallback((e: Event) => {
    const event = (e as { nativeEvent?: Event }).nativeEvent ?? e;
    const newModel = (event as CustomEvent<
      RuleExpression.Expression['MethodExpression'] | RuleExpression.Expression['BinaryExpression']
    >).detail;
    console.log({ newModel });
  }, []);
  ...
  return (
    <RapidValueExpressionBuilder
      ref={setBuilderRef}
      onChange={handleChangedValueExpression}
    />
  );
}
```
