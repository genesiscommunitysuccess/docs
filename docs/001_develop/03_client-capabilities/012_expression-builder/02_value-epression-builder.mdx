...
:::tip
This component is based on the [expression builder library](/develop/client-capabilities/expression-builder/) and is pre-configured to create transformation expressions that the back-end can work with. If you want to set your own operations and other configurations you can create your own component using the base library.
:::

## Example

You can add, change, or delete a rule to create different expressions based upon input fields. When you change the expression model on the UI you can see the json payload which is created change on the right hand side (click the button to show the model if it's currently hidden).
...
```
    ...
    <tr>
      <td>model</td>
      <td><code>RuleExpression.Expression['MethodExpression']</code> | <code>RuleExpression.Expression['BinaryExpression']</code></td>
