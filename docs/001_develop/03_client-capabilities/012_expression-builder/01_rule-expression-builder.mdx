...
Declaration
```html
<RapidRuleExpressionBuilder></RapidRuleExpressionBuilder>
```
...
import {
  ...
  type RapidRuleExpressionBuilderRef,
...
export function MyComponent() {
  ...
  configRef.current = ruleConfig;

  const setBuilderRef = useCallback((el: RapidRuleExpressionBuilderRef | null) => {
    if (el) {
      el.ruleConfig = configRef.current;
    }
  }, []);

  const handleChangedRuleExpression = useCallback((e: Event) => {
    const event = (e as { nativeEvent?: Event }).nativeEvent ?? e;
    const newModel = (event as CustomEvent<
      RuleExpression.Expression['PredicateExpression']
    >).detail;
    console.log({ newModel });
  }, []);
  ...
  return (
    <RapidRuleExpressionBuilder
      ref={setBuilderRef}
      onChange={handleChangedRuleExpression}
    />
  );
}
```
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Is current</td>
      <td>Current hour</td>
      <td>Matches times falling within the current hour.</td>
      <td>
        <ul>
    ...
    <tr>
      ...
      <td>Is previous</td>
      <td>Previous hour</td>
      <td>Matches times falling within the previous hour.</td>
      <td>
        <ul>
