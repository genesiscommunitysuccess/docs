...
Because `RapidButton` is made up of 'base styles' (These provide structural and default functional styles for all RapidButton instances.) and 'appearance styles' (These are variations layered on top of the base styles to create visually distinct versions of the component, such as primary, danger, neutral, etc.), we'll focus our attention on how to override these styles in our custom `MyButton` component.
...
This implementation allows the design system to leverage the flexibility of CSS variables for both consistency and dynamic runtime theming.

:::

#### Overriding Base Styles

Let's override structural properties defined in the `baseStyles` by adding or modifying CSS custom properties.
...
```typescript
import { css } from '@genesislcap/web-core';
import { appearanceBehavior } from '@genesislcap/rapid-design-system';  //this is coming from utils - check how to import
...
export const myButtonStyles = css`
  ${appearanceBehavior('neutral', customNeutralAppearance)}; // need to double check this function
`;


// TODO: break this function down 

// export const rapidButtonStyles = (
...
// ): ElementStyles =>
  ...
  // `.withBehaviors(
    // appearanceBehavior('primary', rapidPrimaryStyles),
    // appearanceBehavior('danger', rapidDangerStyles),
    // appearanceBehavior('primary-outline', rapidPrimaryOutlineStyles),
    // appearanceBehavior('neutral-outline', rapidNeutralOutlineStyles),
    // appearanceBehavior('link', rapidLinkStyles),
  // );
