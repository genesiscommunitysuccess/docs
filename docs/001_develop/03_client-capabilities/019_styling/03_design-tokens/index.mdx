...
`configureDesignSystem` merges your JSON with the platform defaults and publishes the result through `designTokensMap()`. The returned object exposes an observable `value` that updates whenever the design system is reconfigured. Layout and `rapid-grid-pro` subscribe to this flow so that tab chrome, header metrics, and default grid row and header heights stay aligned with the active sizing tokens (see [Sizing tokens](/develop/client-capabilities/styling/design-tokens/styling-tokens-sizing/) and [Layout declarative HTML](/develop/client-capabilities/layout-management/client-layout-management-html/#tab-height-and-the-design-system)).
