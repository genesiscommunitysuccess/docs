...
This is a **sustained** alert only. There is no separate “pulse” API; keep `alert` true while matches exist, and false when none remain.

Tabs and matching grid rows sample the same document-level flash clock (`--alert-flash-t`), so they stay in phase even when the rows render inside the grid’s shadow DOM. Drive both from the same [row match criteria](/develop/client-capabilities/grids/grid-pro/grid-pro-row-alerts) via `DatasourceRowMatchTracker` and `createRowAlert`.

### React example

```tsx
import { Tab, Tabs, TabPanel } from '@genesislcap/rapid-design-system/react';

function OrdersTabs({ equityAlerting, equityCount }: {
  equityAlerting: boolean;
  equityCount: number;
}) {
  return (
    <Tabs>
      <Tab slot="tab" alert={equityAlerting}>
        {equityCount > 0 ? `Equity (${equityCount})` : 'Equity'}
      </Tab>
      <TabPanel slot="tabpanel">{/* blotter */}</TabPanel>
    </Tabs>
  );
}
```
...
```html
<rapid-tabs>
  <rapid-tab id="equity" ?alert=${(x) => x.equityAlerting}>
    ${x => x.equityCount > 0 ? `Equity (${x.equityCount})` : 'Equity'}
  </rapid-tab>
