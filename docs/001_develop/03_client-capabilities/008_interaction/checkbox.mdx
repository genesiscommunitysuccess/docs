...
|---|---|---|---|
|readonly|_boolean_|Configures the component so the user cannot alter the state via the web browser. A readonly checkbox value can still be sent along with the form submit.|`<rapid-checkbox readonly>`|

### Properties

| Property | Type | Use | Example |
|---|---|---|---|
|checked|_boolean_|The boolean value of whether the checkbox is checked. |`<rapid-checkbox ?checked="${sync((x) => x.isSelected, 'boolean')}">`|
...
|---|---|---|---|
| change |_boolean_| Emits a custom change event when the checked state changes. Access the value on the event via `.target.checked`.|`<rapid-checkbox @change="${(x) => x.myChangeHandler(event)}">`|
