...
## Example
Lets look at a simple example of the component in action. If you want the task to be executed every Monday and Wednesday at 10:30 AM, you can select the `Weekly` recurrence type, choose `Monday` and `Wednesday` from the list of days, set the time to `10:30 AM`, and the component will generate the corresponding cron expression. The generated cron expression is displayed in the output field which is displayed at the foot of the component.  
