...
:::info
The `@genesislcap/foundation-login` package has been retired and replaced by **`@genesislcap/foundation-auth`**. If you still reference `foundation-login` in your application, see [Migrating from foundation-login](#migrating-from-foundation-login) below.
:::
...
:::info
`AuthConfig` also accepts `name`, `attributes`, `shadowOptions` and `elementOptions` (inherited from FAST's `PartialFASTElementDefinition`), letting you control the tag name and element definition used when `configure()` defines the custom element — this replaces the standalone `define()` call from `foundation-login`.
:::
...
## Migrating from foundation-login

`@genesislcap/foundation-login` has been retired; all applications should move to `@genesislcap/foundation-auth`. The package is not a drop-in rename — its configuration API was restructured. Key changes to be aware of:

- **`configure()` is now async and self-contained.** `configure(container, config)` + a separate `define({ name })` call are replaced by a single `await configure(config)`, imported from the `@genesislcap/foundation-auth/config` subpath. It both registers the DI config and defines the custom element, resolving with the element class — see [`configure()`](./docs/api/foundation-auth.configure.md).
- **Removed config options:** `autoConnect`, `autoAuth`, `showConnectionIndicator`, `defaultRedirectUrl`, `redirectHandler` and `submitButtonAppearance` no longer exist on `AuthConfig`.
  - `defaultRedirectUrl` / `redirectHandler` are replaced by the [`postLoginRedirect`](./docs/api/foundation-auth.authconfig.postloginredirect.md) and [`postLogoutRedirect`](./docs/api/foundation-auth.authconfig.postlogoutredirect.md) callbacks.
  - `submitButtonAppearance` is superseded by the more general [`templateOptions`](./docs/api/foundation-auth.authconfig.templateoptions.md) mechanism.
- **`messageDelays` (a per-message-type map) is replaced by a single [`feedbackDelay`](./docs/api/foundation-auth.authconfig.feedbackdelay.md) number** (milliseconds).
- **`sso.identityProvidersPath` has been removed** — the identity providers list endpoint is now resolved internally rather than being configured per app.
- **SSO helper functions changed shape.** `fetchIdps`/`getSsoLoginUrl`/`Idp`/`IdpResponse` are replaced by an injectable `IDPService` (`getList`/`getLoginUrl`) and new `IdentityProvider`/`IDENTITY_PROVIDER` types.
- **Routes gained `logout` and `terms-and-conditions`**, and lost `mfa-mail-sent` — see [`AuthRoutes`](./docs/api/foundation-auth.authroutes.md).
- **`defaultLoginConfig` is now `defaultAuthConfig`**, and `LoginConfig`/`LoginRouting`/`getLoginRouting` are now `AuthConfig`/`AuthRouting`/`getAuthRouting` respectively.

See the full API reference [here](./docs/api/index.md) for the complete, current shape of `AuthConfig` and related types.