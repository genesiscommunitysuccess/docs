...
When the back end requires a second factor after the initial credentials are accepted, Foundation Auth navigates to the **Verify** route (`verify`). The authentication machine enters the `awaitingMfa` state and the user must enter the one-time code before authentication can complete.

High-level flow:

1. User submits username/password on the Login screen.
2. The server responds that MFA is required (`MFA_REQUIRED`).
3. The auth machine transitions to `awaitingMfa` and redirects to Verify.
4. The user enters the code; on success, authentication completes. The back button cancels MFA (`cancel-mfa`) and returns to Login.

MFA itself is configured on the server. See [authentication — `mfa`](/develop/server-capabilities/access-control/authentication/#mfa). This page does not cover enabling MFA, user enrolment, or admin MFA management.

The Verify page uses the [Verification Code Input](/develop/client-capabilities/forms/form-inputs/client-interaction-verification-code-input/) component for digit entry. You can omit the Verify route via [`omitRoutes`](./docs/api/foundation-login.loginconfig.omitroutes.md) if you provide an alternative MFA UI.
...
<tr>
  ...
  <td>`{ src: string; type: 'video/ogg' | 'video/mp4' | 'video/webm'; } | null`</td>
