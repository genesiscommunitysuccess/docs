...
Authentication is primarily configured on the back end. It's essential to familiarize yourself with the [authentication](../../02_server-capabilities/007_access-control/02_authentication/index.mdx) section of the back-end configuration to ensure proper integration.
...
The **Foundation Login** supports Single Sign-On (SSO) integration, allowing users to authenticate using a unified set of credentials, which may also include credentials from the Genesis low-code platform. Genesis supports SSO with both JWT and SAML.
