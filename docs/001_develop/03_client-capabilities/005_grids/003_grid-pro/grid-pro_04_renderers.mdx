...
pagination_next: develop/client-capabilities/grids/grid-pro/grid-pro-column-selection
---
...
Renders either a design-system icon (`iconName`) or an image icon (`iconSrc`) inside a grid cell.

<GridProExampleIconRenderer />

#### Key features

- Render icon by name using `iconName` (optionally `iconSize`)
- Render icon by image using `iconSrc` (optionally `iconHeight`)
- Supports custom coloring via `iconColor`
- Supports optional secondary icon overlay via `secondaryIcon`
- Supports disabled state via `disabled` (disables hover/click behavior)

#### Usage example

```typescript
const iconRendererColDef = {
  ...
  cellRendererParams: (params) => {
    ...
    return enabled
      ? {
          ...
          showHover: true,
          clickHandler: (event, rowData) => handleIconClick(rowData),
        }
      : {
          iconName: 'xmark',
          iconColor: 'red',
...
export function AutoCellRendererGrid() {
  return (
    <GridPro autoCellRendererByType>
...
@Component({
  selector: 'auto-cell-renderer-grid',
  template: `
    <rapid-grid-pro auto-cell-renderer-by-type>
      <grid-pro-client-side-datasource resource-name="ALL_TRADES">
      </grid-pro-client-side-datasource>
...
export class AutoCellRendererGridComponent {}
```
...
export function MyGrid() {
  return (
    <GridPro gridComponents={gridComponents} autoCellRendererByType>
      <GridProClientSideDatasource resourceName="ALL_TRADES"></GridProClientSideDatasource>
    </GridPro>
  );
}
```
...
@Component({
  selector: 'my-grid',
  template: `
    <rapid-grid-pro [gridComponents]="gridComponents" auto-cell-renderer-by-type>
      <grid-pro-client-side-datasource resource-name="ALL_TRADES"></grid-pro-client-side-datasource>
    </rapid-grid-pro>
  `,
  standalone: true,
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
