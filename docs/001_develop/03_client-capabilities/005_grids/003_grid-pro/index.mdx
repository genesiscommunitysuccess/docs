...
To update the label text and/or value, call updateParams with configuraton as below: 

```typescript
    const statusBarLabelValue = this.gridApi.getStatusPanel(GridProStatusBarTypes.labelValue);
    if (statusBarLabelValue) {
      (statusBarLabelValue as any).updateParams?.({
        label: "New label"
        value: "New value",
      });
    }
