...
| `:customErrorHandlerTemplate`    | `(prefix: string, gridErrorItems: any[]) => ViewTemplate` | - | **Custom error handling**: Function that returns a custom template for displaying datasource errors. If not provided, uses the default error dialog. |
...
```typescript
interface GridProErrorItem {
  detail: any;  // Error details (message, object, etc.)
...
const customErrorTemplate = (prefix: string, errors: any[]) => html`
...
const advancedErrorHandlerTemplate = (prefix: string, errors: any[]) => html`
  ${when(() => errors.length > 0, html`
