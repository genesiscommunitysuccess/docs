...
- **Column Selection**: Built-in or standalone column picker for show/hide, search, and reorder. See [Column Selection](./grid-pro_05_column-selection.mdx).
...
export function SimpleGridExample() {
  ...
  useEffect(() => {
    if (gridRef.current) {
      gridRef.current.gridOptions = gridOptions;
    }
  }, [gridOptions]);

  return <GridPro ref={gridRef}></GridPro>;
}
```
...
With a Rapid Genesis grid theme, `rapid-grid-pro` sets default `row-height` and `header-height` from your design-system sizing tokens (`baseHeightMultiplier`, `density`, and `designUnit`) when you do not pass explicit values. Those defaults track updates when `configureDesignSystem` runs or changes. Set the `row-height` and `header-height` attributes when you need fixed pixel values instead. See [Sizing tokens](/develop/client-capabilities/styling/design-tokens/styling-tokens-sizing/) for how those tokens are defined in `design-tokens.json`.
...
| `enable-column-selection`        | `boolean`     | `false`     | Shows a Column Selection button above the grid. For custom placement, use the standalone Column Selection component instead — see [Column Selection](./grid-pro_05_column-selection.mdx). |
...
| `size-columns-to-content`        | `boolean`     | `false`     | Auto-size columns to fit their content using `autoSizeAllColumns()`. When combined with `size-columns-to-fit`, sizing intelligently picks the best mode based on available space. |
| `size-columns-to-fit`            | `boolean`     | `false`     | Fit columns to the available horizontal space using `sizeColumnsToFit()`. When combined with `size-columns-to-content`, sizing intelligently picks the best mode based on available space. |
...
export function CustomErrorGridExample() {
  return (
    <GridPro customErrorHandlerTemplate={customErrorTemplate}>
...
```html
<grid-pro
  :customErrorHandlerTemplate="${() => customErrorTemplate}">
...
@Component({
  selector: 'custom-error-grid-example',
  template: `
    <rapid-grid-pro [customErrorHandlerTemplate]="customErrorTemplate">
...
export class CustomErrorGridExampleComponent {}
```
...
```tsx
import { GridProServerSideDatasource } from '@genesislcap/grid-pro/react';
import { RapidGridPro as GridPro } from '@genesislcap/rapid-grid-pro/react';

export function AdvancedCustomErrorGridExample() {
  return (
    <GridPro customErrorHandlerTemplate={advancedErrorHandlerTemplate}>
      <GridProServerSideDatasource resourceName="ALL_POSITIONS" />
    </GridPro>
  );
}
```
...
@Component({
  selector: 'advanced-custom-error-grid-example',
  template: `
    <rapid-grid-pro [customErrorHandlerTemplate]="advancedErrorHandlerTemplate">
      <grid-pro-server-side-datasource resource-name="ALL_POSITIONS">
      </grid-pro-server-side-datasource>
    </rapid-grid-pro>
  `,
  standalone: true,
...
export class AdvancedCustomErrorGridExampleComponent {}
```
...
- **Server-side datasources (pagination enabled)**: row count is handled by the pagination status panel
- **Server-side datasources (pagination disabled / infinite scroll)**: `rows` uses a dedicated row-count panel and can show uncertain totals using `+`

```typescript
interface GridProStatusBarConfig {
  rows?: boolean;                                    // Enable row count component (default: false). In server-side infinite scroll, this uses the row-count status panel.
...
|-----------|------------------------|-------------------------|-------------|
| `rows` | Available | Available | Shows row count information. In server-side infinite scroll mode, this can display uncertain totals using `+`. |
...
When using a server-side datasource without pagination, `statusBarConfig.rows` uses a dedicated row-count status panel.

- If more rows are still available from the server, the panel shows a `+` suffix (for example, `Rows: 250+`).
- Once the final page is reached, the panel shows the exact count (for example, `Rows: 347`).
...
const serverInfiniteStatusBarConfig = { rows: true, reload: true };

export function ServerSideInfiniteGridExample() {
  return (
    <GridPro withStatusBar statusBarConfig={serverInfiniteStatusBarConfig}>
...
```typescript
@customElement({
  name: 'server-side-infinite-grid-example',
  template: html`
    <grid-pro
      with-status-bar
      :statusBarConfig="${(x) => x.serverInfiniteStatusBarConfig}">
      <grid-pro-server-side-datasource
        resource-name="ALL_TRADES"
        max-rows="250">
      </grid-pro-server-side-datasource>
...
export class ServerSideInfiniteGridExample extends GenesisElement {
  @observable serverInfiniteStatusBarConfig: GridProStatusBarConfig = {
    rows: true,
    reload: true,
  };
}
...
@Component({
  selector: 'server-side-infinite-grid-example',
  template: `
    <grid-pro with-status-bar [statusBarConfig]="serverInfiniteStatusBarConfig">
      <grid-pro-server-side-datasource
        resource-name="ALL_TRADES"
        max-rows="250"
      ></grid-pro-server-side-datasource>
...
export class ServerSideInfiniteGridExample {
  serverInfiniteStatusBarConfig = { rows: true, reload: true };
}
```
...
const clientStatusBarConfig = {
  rows: true,
  maxRows: true,
  loadMore: { appearance: 'accent', tooltip: 'Load more trades from server' },
  reload: { tooltip: 'Refresh all trade data' },
  aggregation: true,
};
...
export function ClientSideGridExample() {
  return (
    <GridPro
      ...
      withStatusBar
      statusBarConfig={clientStatusBarConfig}
      persistColumnStateKey="client-grid-columns"
      persistFilterModelKey="client-grid-filters"
      enableRowFlashing
      enableCellFlashing
      headerCaseType="headerCase"
      theme="alpine-dark"
      asyncUpdate
...
@Component({
  selector: 'client-side-grid-example',
  template: `
    <grid-pro
      pagination
      pagination-page-size="50"
...
export class ClientSideGridExample {
  clientStatusBarConfig = {
    rows: true,
    maxRows: true,
    loadMore: { appearance: 'accent', tooltip: 'Load more trades from server' },
    reload: { tooltip: 'Refresh all trade data' },
...
### 2. Grid Pro with server-side datasource and status bar (pagination mode)
...
const serverStatusBarConfig = {
  rows: true,
  maxRows: true,
  reload: { tooltip: 'Refresh trade data from server' },
  aggregation: true,
};
...
export function ServerSideGridExample() {
  return (
    <GridPro
      ...
      withStatusBar
      statusBarConfig={serverStatusBarConfig}
      persistColumnStateKey="server-grid-columns"
      enableRowFlashing
      theme="balham"
    >
      <GridProServerSideDatasource
...
export class ServerSideGridExample extends GenesisElement {
  // Server-side datasource with pagination enabled
  @observable serverStatusBarConfig: GridProStatusBarConfig = {
    rows: true,              // Available - row counts are shown
...
@Component({
  selector: 'server-side-grid-example',
  template: `
    <grid-pro
      pagination
      pagination-page-size="100"
...
export class ServerSideGridExample {
  serverStatusBarConfig = {
    rows: true,
    maxRows: true,
    reload: { tooltip: 'Refresh trade data from server' },
    aggregation: true,
...
const eventsAndCallbacks = {
  ...
  onGridReady: (event) => console.log('Grid ready:', event),
};

export function ManualDataGrid() {
  return (
    <GridPro
      rowData={rowData}
      gridComponents={gridComponents}
      eventsAndCallbacks={eventsAndCallbacks}
      theme="balham"
      headerHeight="50"
      persistColumnStateKey="manual-grid-state"
      enableCellFlashing
    />
  );
}
...
export function ThemedGrid() {
  return (
    <GridPro
      theme="material"
      gridFontFace={customFontFace}
      headerHeight="48"
      rowHeight="42"
      headerCaseType="sentenceCase"
    >
      <GridProServerSideDatasource resourceName="ALL_POSITIONS" />
    </GridPro>
  );
...
### 5. Overriding the label value

By default when maxRows is enabled, a label is shown on the left of the status bar which shows the max rows count.
However it is possible to utilise this label for other purposes when max rows is not enabled, or override it when it is. 
To update the label text and/or value, call updateLabelValueStatusBar with configuraton as below: 

```typescript
    this.grid?.updateLabelValueStatusBar({
      label: 'Custom Label',
      value: 'Custom Value',
    });
```
...
### Request only visible columns (Data Server)

:::info Platform version
Requires **`@genesislcap/grid-pro` 14.489** or later.
:::

DOM virtualisation limits **rendering** cost; it does not reduce how many column values travel over the wire on a Data Server subscription. On [`grid-pro-genesis-datasource`](/develop/client-capabilities/grids/grid-pro/grid-pro-datasources/#grid-pro-genesis-datasource), enable **`request-visible-columns-only`** so the datasource **`FIELDS`** list tracks visible grid columns and reloads when users show or hide columns. See [Grid Pro datasources](/develop/client-capabilities/grids/grid-pro/grid-pro-datasources/#request-visible-columns-only).
