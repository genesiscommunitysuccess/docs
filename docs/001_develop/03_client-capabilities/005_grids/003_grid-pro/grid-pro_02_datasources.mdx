...
- For row count display in infinite scroll mode, enable `with-status-bar` and set `statusBarConfig.rows = true` on `grid-pro` without enabling `pagination`. See [row count in server-side infinite scroll](/develop/client-capabilities/grids/grid-pro/grid-pro/#row-count-in-server-side-infinite-scroll).
- **Limitations**:
