...
- **`setApprovalMessage`** (boolean): Enables proactive approval workflow for this custom action form. When set to `true`, the form includes an `APPROVAL_MESSAGE` field from the start and submits it at root level alongside `DETAILS`.

If `setApprovalMessage` is configured both on `entity-management` and in a custom action, the custom action value takes precedence while that action is active.
...
For more information about UI schemas, refer to the [Forms documentation](/develop/client-capabilities/forms/smart-forms/).
