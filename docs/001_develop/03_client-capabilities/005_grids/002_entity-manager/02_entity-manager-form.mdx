...
:::tip
For more detailed guidance on defining and customizing forms, refer to our section on [Forms](/develop/client-capabilities/forms/smart-forms/) where you'll find useful tips and working examples. 
:::
...
@Component({
  selector: 'entity-management-form',
  template: `
    <entity-management
      design-system-prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
export class EntityManagementFormComponent {
...
export function EntityManagementExample() {
  return (
    <EntityManagement
      ...
      createFormUiSchema={counterpartyCreateUIschema}
      updateFormUiSchema={counterpartyUpdateUIschema}
...
@Component({
  selector: 'entity-management-example',
  template: `
    <entity-management
      enable-search-bar
      design-system-prefix="rapid"
...
export class EntityManagementExampleComponent {
  counterpartyCreateUIschema = counterpartyCreateUIschema;
  counterpartyUpdateUIschema = counterpartyUpdateUIschema;
}
```
...
### Configuring submit-failure notifications

Entity Management exposes an `errorNotificationConfig` property to configure how submit failures are displayed.

- By default, errors are shown as an error snackbar.
- You can override the notification surface (for example, toast), while keeping titles and body messages unchanged.
- The same config is reused for optimistic concurrency warnings and other submit errors.
...
const errorNotificationConfig: NotificationStructure['config'] = {
  toast: {
    type: 'warning',
    autoClose: true,
    closeTimeout: 5000,
...
export function EntityManagementNotificationExample() {
  return (
    <EntityManagement
      prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
@customElement({
  name: 'entity-management-notification-example',
  template: html`
    <entity-management
      design-system-prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
export class EntityManagementNotificationExample extends GenesisElement {}
```
...
@Component({
  selector: 'entity-management-notification-example',
  template: `
    <entity-management
      design-system-prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
export class EntityManagementNotificationExampleComponent {
  errorNotificationConfig = errorNotificationConfig;
}
```
