...
- **`updateEvent="EVENT_COUNTERPARTY_UPDATE"`** defines the event used for updating existing counterparty records. This displays the form and fills it with the data of the selected entity for editing.
- **`deleteEvent="EVENT_COUNTERPARTY_DELETE"`** defines the event used for deleting counterparty records. A button will be displayed for users to delete the selected entity from the grid.
- **`createEvent="EVENT_COUNTERPARTY_INSERT"`** automatically generates a form with all the fields from the metadata of the `EVENT_COUNTERPARTY_INSERT` event. This also adds a button at the top of the grid so that the user can access this form to create new entities.
- **`enable-search-bar`** displays a functioning search bar in the grid, so that users can search for specific entities.
