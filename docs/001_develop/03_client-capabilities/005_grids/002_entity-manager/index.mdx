...
- **Approval workflow support**:
  - Supports proactive and reactive approval-message flows for events that require additional justification.
  - Allows custom labeling of the approval message prompt.
...
- **Custom actions**:
  - Extend functionality beyond standard CRUD operations with custom buttons and actions.
  - Trigger backend events or execute client-side callbacks with flexible positioning options.
  - Support form-based actions with default value mapping from selected row data.

- **Configurable error notifications**:
  - Lets you control the notification surface used for submit failures (for example, snackbar or toast).
  - Uses the same config for optimistic concurrency warnings and other submit errors.
...
### Configuring the datasourceConfig

The `datasourceConfig` property controls how `entity-management` interacts with the configured backend resource.
This lets you tune filtering, sorting, paging, polling, and request payload behavior without changing the component structure.
...
#### 4) Request/reply resource example
...
#### 5) Trigger extra polls from events with `pollTriggerEvents`
...
For the full `DatasourceConfiguration` reference, see the [API docs](./docs/api/foundation-entity-management.datasourceconfiguration.md).
...
| `set-approval-message`     | enables proactive approval workflow in create/update forms by adding `APPROVAL_MESSAGE` to the form schema and sending it alongside `DETAILS`. | No |
| `approval-message-label`   | customizes the label used for the approval message field in the approval dialog. | No |
...
| `errorNotificationConfig`  | configures how submit failures are displayed (for example, snackbar/toast) while preserving the default message content. | No  |
| `customActions`            | an array of custom actions that extend entity management functionality beyond standard CRUD operations. | No  |

:::info
For a full list of attributes and properties with examples, see the [Entity Manager declarative HTML](./05_entity-manager-api.mdx) or [API documents](./docs/api/foundation-entity-management.md).
:::
