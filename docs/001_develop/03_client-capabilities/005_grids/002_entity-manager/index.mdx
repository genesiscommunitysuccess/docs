...
The Entity Management component supports a configurable status bar that provides real-time data insights and interactive controls. The status bar includes row count, max rows, aggregation, load more, and reload buttons.
...
const statusBarConfig: GridProStatusBarConfig = {
  ...
  loadMore: {
    tooltip: 'Load more counterparties',
  },
  reload: {
    tooltip: 'Reload counterparty data',
  },
};

@customElement({
  name: 'entity-management-status-bar-example',
  template: html`
   <entity-management
      design-system-prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
export class EntityManagementStatusBarExample extends GenesisElement {}
```
...
export default function EntityManagementStatusBarExample({}) {
  return (
   <entity-management
      design-system-prefix="rapid"
      resourceName="ALL_COUNTERPARTYS"
...
In this example, the following attributes are used to configure the status bar:

- **`:statusBarConfig=${() => statusBarConfig}`** provides configuration for the status bar components, including which elements to display and their tooltips.
- **`rows: true`** displays the current row count in the status bar.
- **`maxRows: true`** displays the maximum number of rows that can be loaded.
- **`aggregation: true`** shows the aggregation panel for selected columns.
- **`loadMore`** configures the load more button with a custom tooltip.
- **`reload`** configures the reload button with a custom tooltip.

:::info
The status bar requires AG Grid Enterprise module to be enabled. If the Enterprise module is not available, the status bar will not be displayed.
:::
...
| `statusBarConfig`          | enables you to configure the status bar components, including row count, max rows, aggregation, load more, and reload buttons. | No  |
