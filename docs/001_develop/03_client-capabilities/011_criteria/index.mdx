...
```tsx
import { ChartDatasource, G2PlotChart } from '@genesislcap/g2plot-chart/react';
...
export function MyComponent() {
  return (
    <G2PlotChart type="line" config={lineChartConfiguration}>
      <ChartDatasource
        resourceName="ALL_PRICE_HISTORY"
        serverFields="DATE VALUE TICKER"
        chartFields="date value ticker"
...
@Component({
  ...
  template: `
    <rapid-g2plot-chart type="line" [config]="lineChartConfiguration">
      <chart-datasource
        resourceName="ALL_PRICE_HISTORY"
        server-fields="DATE VALUE TICKER"
        chart-fields="date value ticker"
