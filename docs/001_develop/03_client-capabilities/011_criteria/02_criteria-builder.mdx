...
The criteria builder includes specialized operators for date-based filtering:

- `IS_CURRENT` - Matches records within the current time period (hour/day/week/month/year)
- `IS_PREVIOUS` - Matches records within the previous time period (hour/day/week/month/year)
...
// Combine date filtering with other criteria
const currentDayTradesExpression = new CriteriaBuilder()
  .withExpression(currentDayExpression)
  .And(tradeIdEqualsExpression)
  .build()
```
