...
keywords: [presentation, status-pill, countdown, status, enum, icon, status-indicator, pill]
tags:
- presentation
- status-pill
- countdown
...
```
    Usage
    ```tsx
import { StatusPill } from '@genesislcap/rapid-design-system/react';
...
export function MyComponent() {
    ...
    return (
      <>
        ...
        iconInitial="S"
        text="Live"
        iconInitialBackgroundColor="#0c52b9"
        backgroundColor="#547cb6"
        ></StatusPill>
        <StatusPill
        icon="clock"
        text="Market expires in"
        backgroundColor="#9c851b"
        dateCountdown={fiveMinutesTimeInMiliseconds}
        ></StatusPill>
      </>
    );
  }
