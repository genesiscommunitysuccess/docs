...
To populate options from a Data Server or Request Server resource, slot an [options datasource](/develop/client-capabilities/forms/form-inputs/client-options-datasource/) inside the select. For long reference lists, enable **`infinite-scroll`** so the dropdown loads pages on demand instead of holding the full result set in memory.
