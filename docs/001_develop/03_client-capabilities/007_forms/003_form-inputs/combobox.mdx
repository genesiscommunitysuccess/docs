...
For server-backed option lists, use an [options datasource](/develop/client-capabilities/forms/form-inputs/client-options-datasource/) with **`infinite-scroll`**. The combobox then filters on the server as the user types, which is required when only one page of options is loaded at a time.
...
<Combobox>
  ...
  <ListboxOption value="m">Medium</ListboxOption>
  <ListboxOption value="l">Large</ListboxOption>
</Combobox>
```
  Usage
  ```tsx
import { Combobox, ListboxOption } from '@genesislcap/rapid-design-system/react';
...
<table>
  ...
  <tbody>
    <tr>
      <td>autocomplete</td>
      <td><code><a href="#autocomplete">"inline" | "list" | "both" | "none"</a></code></td>
