...
Select controls and comboboxes support different filtering modes that determine how options are filtered based on user input. You can configure the filtering behavior using the `filterMode` option in the UI schema:
  ...
  elements: [
    {
      ...
      options: {
        ...
        data: countries,
        filterMode: 'contains' // Example of filter mode usage
      }
    },
    {
      type: 'Control',
      ...
      options: {
        ...
        data: cities,
        filterMode: 'startsWith' // Example of alternative filter mode
      }
    }
  ]
});
