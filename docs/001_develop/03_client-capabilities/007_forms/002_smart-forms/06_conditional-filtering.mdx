...
In addition to conditional filtering between fields, select and combobox controls support different filtering modes that determine how options are filtered as users type. These modes can be configured through the UI schema options:

- `contains` - Filters options that contain the search text anywhere in the value (default)
- `startsWith` - Filters options that begin with the search text

To configure the filter mode, add the `filterMode` option to your UI schema:
...
const locationUISchema = {
  ...
  options: {
    filterMode: 'startsWith', // or 'contains'
    valueField: 'CITY_ID',
  ...
  elements: [
    ...
    },
    {
      ...
      options: {
        ...
        labelField: 'CITY_NAME',
        data: cities,
        filterMode: 'contains' // Example of filter mode configuration
      }
    }
  ]
});
...
export function LocationForm() {
  ...
  useEffect(() => {
    (async () => {
      await loadCountries();
      await loadCities();
      setUiSchema(locationUISchema(countries, []));
    })();
  }, []);

  const filterCities = (countryId: number) =>
    cities.filter((city) => city.COUNTRY_ID === countryId);

  const handleDataChange = (evt: CustomEvent) => {
    const { data } = evt.detail;
    if (existingData.COUNTRY_ID !== data.COUNTRY_ID) {
      setExistingData({ ...data, CITY_ID: undefined });
      setUiSchema(locationUISchema(countries, filterCities(data.COUNTRY_ID)));
    }
  };
...
@Component({
  ...
  template: `
    <foundation-form
      ...
      [jsonSchema]="locationJsonSchema"
      [uischema]="uiSchema"
      (data-change)="handleDataChange($event)"
    ></foundation-form>
  `,
  standalone: true,
...
export class LocationFormComponent {
  uiSchema: UiSchema;
  existingData = { COUNTRY_ID: undefined, CITY_ID: undefined };
  countries = [];
  cities = [];
  ...
  // loadCountries, loadCities, filterCities — same as Genesis tab

  handleDataChange(evt: CustomEvent) {
    const { data } = evt.detail;
    if (this.existingData.COUNTRY_ID !== data.COUNTRY_ID) {
      this.existingData = { ...data, CITY_ID: undefined };
      this.uiSchema = locationUISchema(
        this.countries,
        this.filterCities(data.COUNTRY_ID),
      );
    }
  }
}
