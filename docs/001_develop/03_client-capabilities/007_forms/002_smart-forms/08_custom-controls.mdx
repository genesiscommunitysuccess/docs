...
When using connected select controls or combobox components, you can configure how filtering behavior works using the filter mode options:
...
- `contains`: Filters options that contain the search text (default)
...
export function FormWithCustomRenderers() {
  return (
    <FoundationForm
...
@Component({
  selector: 'form-with-custom-renderers',
  template: `
    <foundation-form
...
export class FormWithCustomRenderersComponent {
  formRenderers = [...renderers, SegmentedControlRenderer];
}
```
