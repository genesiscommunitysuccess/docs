...
In addition to the events that you create to handle your business logic, you'll also need to emit events to setup and control
the state of the store itself.

### `'store-connected'`

In your main application class you need to fire a `'store-connected'` event in-order to
...
an additional `store-ready` event to dispatch. It's not a hard requirement to emit this, but is considered best practice. If you've no work to
...
Example Main class in Genesis. Highlighted lines are directly related to initialising the store - other configuration may be different in
