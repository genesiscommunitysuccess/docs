...
A `<slot>` is a placeholder inside a component. Any content you add between `<my-button>` tags in HTML will appear in this slot. This set-up makes the `<my-button>` component more flexible, as it can display different content depending on how it’s used. See more info [here](../quick-start/#using-slots).
...
:::warning
Recycling components can cause issues if you have a set of components with different attributes or properties and you plan to re-render the list while changing the components, or their attributes and properties. If you observe behaviour where component are not changing as you expect then it's a good time to try disabling recycling `{recycle: false}`.
:::
