...
Directives are special functions used in templates to control rendering or manipulate elements dynamically.
...
- `ref("buttonElement")`: The `ref` directive creates a reference to the `<button>` element and assigns it to the `buttonElement` property of the `<my-button>` component instance, allowing direct interaction with the DOM element.
...
- True Template: The template rendered if the condition evaluates to true.
- False Template: The template rendered if the condition evaluates to false.
...
@customElement({
  ...
  template: html<MyButtonList>`
    ...
    <ul>
      ${repeat(
        (x) => x.buttons,
        html<string>` <li><button>${(x) => x}</button></li> `
      )}
    </ul>
  `,
})
...
<ul>
  ...
  {repeat(
    (x) => x.buttons,
    html<string>`<li>${(x, c) => `${c.index + 1}. ${x}`}</li>`,
    { positioning: true }
  )}
</ul>
```
...
@customElement({
  ...
  template: html<MyForm>`
    <div>
      <label>
        Text:
        <input type="text" :value=${sync((x) => x.textValue)} />
      </label>
    </div>
    <div>
      <label>
