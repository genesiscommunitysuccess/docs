...
keywords: [
    ...
    Reusable styles,
    Encapsulation,
    Content containment (contain: content),
    Styling Templates,
    css helper,
    cssPartial utility
]
...
const styles = css`
  ...
  :host([primary]) { /* :host([primary]): Styles the element differently if it has the primary attribute. */
...
Let's say we want to compose our own utility style that ensures consistent baseline styles across browsers.
...
- Adding behavior-driven styles to custom elements using two key methods:
  ...
  - `createBehavior()`: Attaches dynamic behavior (e.g., runtime style updates) to an element.
...
Key Considerations for Shadow DOM Styling
...
The ::slotted() CSS pseudo-element lets you style content that is projected into a slot in your component's Shadow DOM.
...
- Lifecycle Hook: How styles are applied during the connectedCallback lifecycle phase.
