...
:::info
<CollapsibleSlot title="Basic info" description="Learn about the basics of attributes and properties in the browser. You need to understand these concepts before the rest of this document.">

<strong>Attributes</strong> are part of a HTML element's API. They are usually a key-value pair where the value is a string, such as `href`.
...
To create an attribute, use the `@attr` decorator. This creates a property-attribute pair - a binding. Once created, any change in one automatically updates (is reflected to) the other, keeping them synchronized.
...
@attr label = "Click Me";
```
this created both an attribute _and_ a property called "label". Because of the `@attr` decorator creating a pair, changes to either the attribute or the property are reflected to the other. Once you have done this, updates to one (e.g. setting a property) can update the other (e.g. the attribute in HTML).

:::important
You can define properties internally in the component class without linking them to HTML attributes. These internal properties are not exposed in the DOM as attributes. This isn't the same as the `private` keyword which prohibits access to a property from outside of the class.
:::
...
In all of the attributes examples you'll notice that when the attribute is changed its updated value is shown on the template. This is because all attributes declared with the `@attr` decorator are also `Observables`. `Observables` are a key concept to how you make the components reactive in the browser to their changing state.
...
Considering just attributes and properties, the above component declares the following API:
...
3. `*` The above API table is comprehensive, but in practise you should *not* set a property value via the template if it's linked to an attribute. E.g.
    ...
    2. `label` isn't recommend to set via the template `:label=${() => 'value'}` as you should set it via the label attribute `label='value'`, (or `label=${x => x.labelExpression}` if setting dynamically).
...
4. As shown in [this example](#interpolated-attribute-values) you can also use expressions when setting attributes too if you wish.
...
It's recommended when implementing reactivity in a custom component you use an `@attr` attribute where you can (string or boolean), and then an `@observable` property if you need to.
...
The above works to set the property via the component's API, but it would be better if the API was simply defined with two attributes.

```typescript
<my-component config-one=${() => 'foo'} config-two=${() => 'bar'}>
```

Attribute - property pairs have advantages over just a property - benefits include being able to set the values in the browser inspector, and better behaviour when a component is cloned using `.cloneNode()`.

There may be times where you need to pass a more complex property through the template, but if you end up passing a lot of state you may find it cleaner to refactor the component to use a state management library such as the `foundation-store` or Redux.
