...
- **Custom Events**. Custom events are unique, developer-defined events (such as `button-clicked` in the example below), which communicate specific actions or states. You can use the `$emit` helper to create and dispatch a custom event.
...
In this modified event:

- When `bubbles` is true, the event propagates upward through the DOM.
- When `bubbles` is false, the event does not bubble.
- If you want the event to be confined to the component emitting it and not propagate up the DOM tree, set `bubbles` to false.

- When `composed` is true, the event crosses the shadow DOM boundary and can be listened to outside the shadow DOM.
