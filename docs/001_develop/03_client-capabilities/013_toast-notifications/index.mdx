...
The `foundation-notifications` package enables you to create and display a range of different of notifications, including:
...
2. Run the `$ npm run bootstrap` command. (Whenever you change the dependencies of your project, ou should always run this command to rebuild.) You can find more information in the [package.json basics](/build-deploy-operate/build/bdo-build-overview/#packagejson) page.