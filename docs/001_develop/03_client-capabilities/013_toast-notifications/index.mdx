...
```tsx
import { DesignSystemProvider, NotificationListener } from '@genesislcap/rapid-design-system/react';
...
export function MyComponent() {
  ...
  return (
    <Button onClick={triggerNotification} ref={ref}>Show Notification</Button>
  );
}
```
<EventWrapper/>
