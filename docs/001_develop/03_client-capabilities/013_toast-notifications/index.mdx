...
You can style the notification components as normal, such as overriding their styling when registering them with the design system. There are cases though where you may need to target the notification container itself. A concrete example of this is when you have components on your page which are positioned and have a `z-index` style set - you may observe in this case that your component ends up on top of the notification which is unlikely behaviour that you want.

To fix this you can manually set the styles on the container. Here is an example for getting a reference to the container for a toast notification and setting the `z-index` style to be above any other components on the page. In this case we export a version of the function to create toast notifications but with a reference to the styled container bound to it.
...
Now you can import `showNotificationToast` from this file instead of from the library directly and get the version with your styles.

If you need to do a similar thing with banner notifications then you'd use the `getBannerAnchor` function to get a reference to the container instead.
