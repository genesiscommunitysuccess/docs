...
As discussed earlier `notification-listener` component needs to be placed at the top of component tree to ensure it can capture events from any child component. You can find the `rapid-design-system-provider` tag in your application and place the `notification-listener` tag inside it, whether yu're using React, Angular, or Genesis framework.
...
`customEventFactory` is a helper function to create events in the format the notification event expects. You can either import it from the PBC module if you're using it, or copy this function into your codebase.
