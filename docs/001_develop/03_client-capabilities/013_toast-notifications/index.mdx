...
As discussed earlier `notification-listener` component needs to be placed at the top of component tree to ensure it can capture events from any child component. 
```typescript
...
Lets complete the above example and add a button in there to trigger the notification event.
```typescript
