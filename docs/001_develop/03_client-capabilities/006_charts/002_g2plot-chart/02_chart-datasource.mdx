...
It is designed to fetch, filter, and structure data in real-time from a backend server, ensuring that the chart remains updated with the most recent data available. 
...
The `chart-datasource` component offers several key features that provide flexibility in managing and displaying data for G2Plot charts. These features enable you to control how data is fetched, filtered, structured, and visualized, ensuring that your charts are both dynamic and efficient.

- **Configurable Data Fields** - The `chartFields` and `serverFields` properties allow precise mapping of data to chart axes and categories, making it easy to define which data should be represented as x/y fields or series values.
  
- **Real-time Filtering** - The `criteria` property lets clients send expressions to filter data directly on the server, enabling efficient data handling without needing to re-fetch unnecessary information.
  
- **Grouping and Stacking Options** - With properties like `isGroup` and `isStack`, users can define whether data should be grouped by categories or stacked, enhancing flexibility in data representation.
...
- **Snapshot** - By setting `isSnapshot`, users can specify whether the data is retrieved as a one-time snapshot or continually updated, depending on application needs.
  
- **Row Limits and Data Management** - `maxRows` helps manage the volume of data fetched, and `rowId` allows for unique row identification, making the data flow organized and manageable for high-volume datasets.
