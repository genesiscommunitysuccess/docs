...
```
        ...
        <tr>
            ...
            <td>Maximum number of rows to be returned as part of the initial message, and as part of any additional MORE_ROWS messages (if your data source is a Data Server). This does not affect the number of chart elements displayed. A row is a record in the database, and each row provides more data points to the chart. More data points mean more detail in the chart displayed.</td>
<td>
...
```
        ...
        <tr>
            <td>maxView</td>
            <td><code>number</code></td>
            <td>Maximum number of rows to track as part of a client view or table. Note that this defaults to 500, so if you want to stream a whole day's trade data within the chart, you need to change this value to exceed the maximum number of expected rows added in a day.</td>
<td>
