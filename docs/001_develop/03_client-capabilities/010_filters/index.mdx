...
- Your app can filter runtime data or state to achieve an outcome. 
- Your app can filter using options such as `userTargeting` and `timeWindow` to customize data manipulation directly in the browser. This improves responsiveness by reducing server requests.
...
For detailed information on the API and configuration, see the [API documentation](./docs/api/index.md).

## Example
Here is an example of the `timeWindow` filter. 
...
### `nodeEnv`
The nodeEnv filter checks the current application environment (such as "development" or "production") and returns true if the filter’s specified environment matches the current one. Use this, for example, to ensure that specific features or configurations are available only in intended environments.
...
For example, if you set the filter to 20%, the each time it runs, it has a 20% chance of passing. Consequently only about 20% of users will see the feature or content. This is useful for controlled feature rollouts or A/B testing without requiring complex set-ups.
...
| Parameter  | Description                                                           |
|------------|-----------------------------------------------------------------------|
| `browsers` | One or more browser names, for example: 'Chrome', 'Firefox'.          |
| `devices`  | One or more device names, for example: 'Mobile', 'Tablet'.            |
| `oss`      | One or more names of operating systems, for example: 'IOS', 'Android' |
| `ua`       | One or more user-agent names, for example: `navigator.userAgent`.     |
