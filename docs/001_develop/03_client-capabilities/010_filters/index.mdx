...
import { Button, TextField } from '@genesislcap/rapid-design-system/react';

export function TimeWindowFilterExample() {
  const [startDate, setStartDate] = useState('2024-11-21T09:00');
  const [endDate, setEndDate] = useState('2024-11-21T18:00');
  const [resultString, setResultString] = useState('');

  const checkWindow = () => {
    setResultString(
      timeWindowFilter(startDate, endDate)
  ...
  return (
    <div className="container">
      <TextField
        type="datetime-local"
        value={startDate}
        onInput={(e: any) => setStartDate(e.target?.value ?? startDate)}
      >
        Start Period
      </TextField>
      <TextField
        type="datetime-local"
        value={endDate}
        onInput={(e: any) => setEndDate(e.target?.value ?? endDate)}
      >
        End Period
      </TextField>
      <Button onClick={checkWindow}>Check</Button>
...
@Component({
  selector: 'time-window-filter-example',
  template: `
    <div class="container">
      <rapid-text-field type="datetime-local" [(value)]="startDate">Start Period</rapid-text-field>
      <rapid-text-field type="datetime-local" [(value)]="endDate">End Period</rapid-text-field>
      <rapid-button (click)="checkWindow()">Check</rapid-button>
      <span>{{ resultString }}</span>
    </div>
  `,
  standalone: true,
...
export class TimeWindowFilterExampleComponent {
  startDate = '2024-11-21T09:00';
  endDate = '2024-11-21T18:00';
  resultString = '';
