...
## Example
lets do an example to test the `timeWindow` filter. In below example you need to select a start period and end period. When you click on check button it executes the timeWindow method with start and end period as input parameters.
...
**url**: A full URL to match.

**pattern**: A regular expression pattern for matching URLs.

**schemes**: An array of valid URL schemes (e.g., "https").

**hosts**: A list of acceptable hostnames or domain names.

**paths**: A list of URL path segments to match.
...
```
One usecase of this filter is checking if the user is an **admin**. For this purpose, `hasAdmnProfile` utility provided by this filter can be used to determine whether a user has administrative access.
```ts
import { UserTargeting } from '@genesislcap/foundation-filters';
...
@UserTargeting userTargeting: UserTargeting;
