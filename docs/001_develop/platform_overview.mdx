...
Every application needs a rich, responsive, real-time user interface. You can build this quickly from a wide choice of Web UI components in your framework of choice—from React and Angular to raw web components. You can select one, configure it and connect it the relevant data source through the APIs exposed by the Genesis microservices.

These components can be quickly composed into fully functional web application using any of the major Web UI frameworks, such as React or Angular. Or you can simply work with the raw Web Components.   
