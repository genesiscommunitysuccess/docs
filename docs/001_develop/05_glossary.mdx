...
The Document Object Model (DOM) is a cross-platform interface that treats an HTML or XML document as a logical tree structure. This is essential for enabling apps to change the content and style of the document via standard web browsers. Understanding this model is essential for any serious work on the front end of a Genesis application.

In the DOM structure, the document itself is a node and sits at the top of a hierarchic structure where each node is an object representing a part of the document. Each branch of the tree ends in a node, and each node contains objects. 

Each node can have event listeners (which are effectively event handlers). Any changes to the node then trigger these listeners, which then apply the relevant logic.

The DOM specification is maintained at: https://dom.spec.whatwg.org/
...
FIX has standard dictionaries of different messages. Typically, we use v4.4. note that some banks use earlier versions (usually 4.2) and some have already moved to 5.0. Also note that you can customize these dictionaries to create our own messages.

Genesis uses [QuickFIX](https://www.quickfixj.org/documentation/) (open source).
...
It means that you have access to autocompletion, Help and error highlighting as you code. Once you define an object in GPAL (such as a View), you can use it in multiple components. Configuration can be written in script files (`.kts`) or in compiled Kotlin classes (`.kt` extending `GPal*` base classes); see [GPAL Standards](/develop/server-capabilities/gpal-standards/).
...
A user agent is a piece of software built into a browser or internet application. It creates a string value that describes the web browser, operating system, or mobile device that is using the internet. Your web browser sends this string as an HTTP header to every website you visit.  

Applications can parse the string to find out about the user's browser and other technical details, in many cases more detail than is polite to request. 
