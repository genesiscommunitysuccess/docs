...
## EMRS (Bloomberg)
The Bloomberg Entitlements Management and Reporting System (EMRS) enables you to manage entitlements for individual users, user groups, applications and publishing serviceson the bloomberg platform. Each data service can be controlled for user and application access to data sources, ranging from B-PIPE to internal proprietary data being published over the Bloomberg Platform.
