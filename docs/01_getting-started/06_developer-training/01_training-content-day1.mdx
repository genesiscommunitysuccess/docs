...
As soon as the Build is done, you need to deploy the application. To do that, click on the Genesis plugin in your intellij (in the panel on the right).

![](/img/click-on-genesis.png)

When you click on it for the first time, it will map all the available processes and display these in the mon panel:
...
The plugin then creates your runtime folder containing all the generated files required for your local application to run. This runtime folder is available under the **server/alpha-app/build/** folder and it is called **genesis-home**:
...
Before you can run your application, you need to create a new schema for the database; this is where all the tables will be created. To do that,
...
<details>
  ...
  Type `table USER` and then `search 1`; this will  display the following:
