...
As soon as the Build is done, you need to deploy the application. To do that, click on the Genesis plugin in your intellij. It will appear on the right panel.

![](/img/click-on-genesis.png)

When you click on it for the first time, it will map all the available processes and will display it in the mon panel:

![](/img/genesis-process-first-run.png)

To deploy your application, click on `deploy Genesis`:

![](/img/deploy-project.png)

By doing that, the plugin will create your runtime folder containing all generated files required for your local application to run. This runtime folder will be available under the **server/alpha-app/build/** folder and it is called **genesis-home**:

![](/img/genesis-home.png)

Congratulations! You have built and deployed an application.
...
![Genesis Install](/img/remap-simple.png)


<h3>Send data to genesis</h3>

Go to **server/alpha-app/src/main/genesis/data/USER.csv**, right click on this csv file and select **Import CSV(s) to Genesis**.

![](/img/import-login.png)
...
===========================================================================================
TIMESTAMP                                2024-03-27 11:11:43.634(n:0,s:1)         NANO_TIMESTAMP
COMPANY_ID                                                                        STRING
...
Now we need to start the server. To do that, click on the start server button. This will start all the processes:

![](/img/start-server-simple.png)
...
:::warning
Some browsers automatically change **http** to **https** - which will cause a fail. Make sure the url starts with **http** before submitting. In case you still facing any difficulty, open a new incognito/private browser (this avoids any cache issues).
:::
