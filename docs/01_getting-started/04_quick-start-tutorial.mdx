...
For step by step instructions, follow the guide on [GenX](/getting-started/using-genx).
...
To see if we added a new trade successfully in home.template.ts, add (before Add trade span):
