...
Notice the expanded `client` folder. This is where we have all our UI-related projects.

The `server` folder follows the same concept, but it is where we have all our server-side projects.

:::note

By default, the **Quick Start Application** seed will:

- only generate the **web** project structure (more about this [here](/creating-applications/creating-a-new-project/alternative_options_supported/web-project-setup/#available-generators)).
- only generate the **jvm** project structure (more about this [here](/creating-applications/defining-your-application/intro/)).
:::
