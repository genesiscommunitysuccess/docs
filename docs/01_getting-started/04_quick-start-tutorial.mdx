...
After that we can create our zero-ag-grid in HomeTemplate const.

```ts
export const HomeTemplate = html<Home>`
<zero-card class="trade-card">
    <zero-ag-grid rowHeight="45" only-template-col-defs>
    <ag-genesis-datasource resourceName="ALL_TRADES" orderBy="TRADE_ID"}/>
...
public async insertTradeData() {
  const tradeDataEvent = await this.connection.commitEvent('EVENT_TRADE_INSERT', {
    DETAILS: {
  ...
  logger.debug('EVENT_TRADE_INSERT result -> ', tradeDataEvent);
}
```
