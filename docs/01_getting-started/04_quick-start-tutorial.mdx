...
- A gradle.properties file inside a .gradle folder on your user directory with the following content (password in clear text)
```shell
genesisArtifactoryUser=<your-actifactory-user>
genesisArtifactoryPassword=<your-actifactory-password>
```
- A local or cloud Linux environment for runtime that has a user created with the name of the application, e.g. [setup and host on wsl](/creating-applications/getting-ready-to-develop/running-applications/options/using-wsl-setup/)
...
In the next prompt we will select "Quick Start Application" from the list
...
? Set API Host Yes (wss://localhost/gsf)
...
First, you must deploy the server.
...
- Username: JohnDoe
- Password: Password11* (This is encrypted in the USER.csv file.)
:::

We wil run the task loadInitialData. This adds the data in USER.csv to the USER table in your
...
Now we can check that the user has been created by running "DbMon", either from the Gradle dropdown menu or by running:
...
Once we are inside the console we can type 'table USER' and after 'search 1'. If imported correctly the user JohnDoe should be listed.
...
#...
# You will need to enter your artifactory credentials at this point
#...
...
Now you have fully running application. If not opened automatically, you can navigate to http://localhost:6060/login, and you should see the following

![](/img/login-screen-quickstart.png)

:::tip
If the blue button to login is not clickable most likely want to revisit the previous section.
