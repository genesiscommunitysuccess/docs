...
After that, you will be asked to confirm the tables that you want to insert in your database; to do this, type **y**.

```powershell
WARNING: are you sure you want to commit changes to the database? (y/n)
```

After remapping is complete, the following prompt is displayed:

```text
=========Starting Data Load=======
This will load reference data needed to use all of the Genesis components utilised by the application,
...
This means that all data stored in the **data** folder will be loaded. Since this is a basic development, this is only the credentials data. 

Once you confirm, the data is loaded into the database, and you will be able to log in. 
