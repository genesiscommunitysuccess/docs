...
Please follow these instructions very carefully to ensure your environment is ready. It's not necessary to have admin rights on your workstation to develop with the Genesis platform, but you might have to check with your System Administrator how to install the required third-party software listed here.
...
Since version 11.3.0 of the Foundation UI, our libraries are public. To check that you have the right package, type `npm info @genesislcap/foundation-ui`. If everything is ok, you should see a response like this:
...
- If you're behind a corporate network, you may need to set up a proxy:
...
You need an individual database to create and test the back-end changes we are proposing. By default, an H2 embedded database is used for local development. For your production environment, make sure you have a [Database Technology supported by Genesis](../../../database/database-technology/overview/) available and running.
