...
Note we are extending `ZeroGridPro`, not `GenesisElement`.
...
[Genesis datasource](../../../web/web-components/grids/grid-pro/grid-pro-connected/) offers some [attributes](../../../web/web-components/grids/grid-pro/grid-pro-connected/#grid-pro-genesis-datasource) to parameterise how to retrieve the data. Some commonly used attributes are:
...
- **`resource-name: string`**: The target [Data Server](../../../develop/server-capabilities/real-time-queries-data-server/) or [Request Server](../../../develop/server-capabilities/snapshot-queries-request-server/) name. Example: "ALL_TRADES" or "ALT_COUNTERPARTY_ID"
