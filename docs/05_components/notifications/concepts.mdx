...
### Gateways
A gateway is a connection to any delivery system for notifications. Gateways are specified in a GPAL script file.
A single SMTP server, teams server or a single symphony instance would be examples of gateways. You can configure multiple gateways.
...
To create a notification in a Genesis System, all you need to do is create a NOTIFY record in the database, which is best done via the EVENT_NOTIFY_INSERT. GENESIS_NOTIFY will take care of the rest, based on the routes that are configured.
