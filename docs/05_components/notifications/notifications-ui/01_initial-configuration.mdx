...
## Installing in a new application
...
<Tabs
  ...
  <TabItem value="windows">
    ...
    npx -y @genesislcap/genx@latest init alpha -x --no-shell; npx -y @genesislcap/genx@latest add alpha -x -s genesislcap/pbc-notify-seed
    ```
  </TabItem>
...
If you want to add Notifications to an existing application, you have a choice:

- Install all the components in one step. 
- Manually install individual components.


### Install all components in one step

Run this command on the previous folder where your app is located (use the name of your app folder in place of `my-app`):

```shell
npx -y @genesislcap/genx@latest add my-app -x -s genesislcap/pbc-notify-seed
```

After this, run `npm run bootstrap` to install it.
