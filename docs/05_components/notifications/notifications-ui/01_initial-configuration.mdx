...
If you prefer, you have the option to add the components manually. On the `client/package.json` file add this in `"dependencies"` section:
...
Again, check the user permissions, because each tab of the dashboard is ruled by a permission (check the available [list of permissions](/docs/05_components/notifications/notifications-ui/03_permissions.mdx)). The user only sees the sections to which they have permission for it.
