...
Make sure the [back end](../../notifications-server/notify-configuring-runtime) has been configured correctly before you proceed to the installation of front-end components. 

Also, practically every section of every component requires a permission to perform the specific action related to that section; check the [permissions](../notify-permissions) available and make sure your user has the permission needed for the section you want to implement or test. More about permissions and rights at the [authorisation](../../../../server/access-control/authorisation#users-profiles-and-right-codes) section.
