...
You must configure the Foundation Router so that you can use `navItems` in your routes. See [Foundation Router](#foundation-router) for the relevant migration steps.
...
Again, check the user permissions, because each tab on the dashboard is ruled by a permission (check the available [list of permissions](../notify-permissions)). The user sees only the sections for which they have permission.
...
Go to the _my-app_**/client** folder and run the following command to install the required npm packages:

```
npm install --save @genesislcap/foundation-ui @genesislcap/web-core
```
This inserts the following packages in your **package.json** file:
...
Add the Foundation Router to your **components.ts** file to register the Foundation Router web component.
