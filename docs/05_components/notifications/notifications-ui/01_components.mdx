...
The available destinations are the ones registered in [Routes](#13-routes).
...
- **TEXT**: the user who creates or modifies a rule can specify a fixed value for the notification condition, e.g. `PRICE > 100` (*PRICE bigger than 100*). This template will be available for all users on the Inbox [Subscribe](#23-subscribe) section.
