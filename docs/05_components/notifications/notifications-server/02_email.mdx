...
You need a SendGrid account to be able to send emails using this method. Your credentials will then need to be passed into the following:
```kotlin
notify {
    gateways {
        sendGrid("SendGrid") {
...
You can configure Notify to work with more than one service at a time. There is an example in our page on [Symphony](../../../../components/notifications/notifications-server/symphony/#configuring-more-than-one-service).
