...
| enableSessionConnectionCheck           | false            | Set to true in order to periodically check if sessions have been disconnected and update the process status.                                                                                                        |
| sessionConnectionCheckIntervalSeconds  | 30               | Interval in Seconds between each session check.                                                                                                                                                                     |
...
The underlying QuickFIX library does not provide a built-in mechanism to trigger process updates under certain conditions such as failure to connect. When the enableSessionConnectionCheck variable is set, available sessions will be periodically checked to validate the connection, based on the sessionConnectionCheckIntervalSeconds, and the process status will be updated if necessary.
