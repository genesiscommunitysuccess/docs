...
## Enabling queries for Reporting

The Reporting Server uses Data Server queries and Request Server requestReplies as the sources of data for generating reports. However, these sources are not available by default (in case they contain sensitive information). 

To make a query or requestReply available as a source of data for a report, add it to the `REPORTING_DATASOURCE_LIST` system definition in the file **reporting-system-definition.kts**.
...
Here is an example `REPORTING_DATASOURCE_LIST` from a **reporting-system-definition.kts** file. It makes a Data Server query and two Request Server requestReply queries available to the Reporting Server:

```kotlin
item("REPORTING_DATASOURCE_LIST", listOf("ALL_SAVED_REPORTS", "SAVED_REPORTS", "ALL_REPORT_DATASOURCES"))
```

The Reporting Server then adds the following metadata services:
