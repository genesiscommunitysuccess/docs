...
In order to start using Document Management, you must specify your `STORAGE_STRATEGY` in your `system-definition.kts`.
...
```
Once you have set you `STORAGE_STRATEGY`, you must set your storage solution specify configuration.

## Local File Storage
When using Local File Storage, your files are stored under your specified folder inside your `genesis-home/runtime/fileupload/`.
There is no need to create your folder, it's created automatically.
...
## AWS S3
When using AWS S3 as your storage solution, you must first have an S3 bucket created either locally or remotely.
Next, you will need to set your `S3_STORAGE_MODE`.

`S3_STORAGE_MODE` supports the following values:
- LOCAL (app running locally with localy S3 bucket)
- DEV (app running locally with remote S3 bucket)
- AWS (app running remotely on a AWS resourse with remote S3 bucket)
...
Once you have set your storage mode and bucket config, your must set individual config for your chosen storage mode.
...
#### S3_STORAGE_MODE = AWS
No extra config is required. The AWS resource your app is running on will extract required config from the environment it's running on. 

## Sharepoint
When using Sharepoint as your storage solution, you must have sharepoint instance created either on-premises or remotely.
