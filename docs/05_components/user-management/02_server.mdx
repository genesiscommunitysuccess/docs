...
:::caution
It's important to load the initial data (.csv files) inside `data` folder. Do it via [SendIt](/operations/commands/server-commands#sendit) command or using [Import CSV(s) to Genesis](/server/tooling/intellij-plugin#loading-data-into-the-application) option using the IntelliJ Plugin.
:::
