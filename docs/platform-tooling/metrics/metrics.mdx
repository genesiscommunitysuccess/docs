...
To make use of the metric calls, you must set `MetricsEnabled` to `true` in the [system definition file](/platform-reference/essential-information/system_definitions-latest/). In addition, you should define the `MetricsReportType`
...
The object MetricService is available via the dependency injection mechanism used in the framework. Simply mark a constructor with the @Inject annotation and the parameters will be resolved automatically.
