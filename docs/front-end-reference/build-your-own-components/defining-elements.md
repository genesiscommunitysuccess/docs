...
:::note
All properties decorated with `@attr` are also *observable*. See [observables and state](/front-end-reference/build-your-own-components/observables-and-state/) for information about how observables enable efficient rendering.
:::
...
All Web Components support a series of lifecycle events that you can tap into to execute custom code at specific points in time. `FASTElement` implements several of these callbacks automatically in order to enable features of its templating engine (described in [declaring templates](/front-end-reference/build-your-own-components/declaring-templates/)). However, you can override them to provide your own code. Here's an example of how you would execute custom code when your element is inserted into the DOM.
