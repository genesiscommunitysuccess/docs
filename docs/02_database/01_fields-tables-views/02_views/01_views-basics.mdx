...
Joins enable you to bring other related tables into a view so we can serve up their fields in a single query row. In order to use joins, one of the fields to be used in the join must be an [index](/database/fields-tables-views/tables/tables-basics/#primary-key-and-other-indices). 
