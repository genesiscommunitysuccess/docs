...
- using `withEntity` enables you to specify a different table entity as the input
...
- The parameter bid receives its value from the field INSTRUMENT_PRICE.BID_PRICE.
- The parameter ask receives its value from the field INSTRUMENT_PRICE.ASK_PRICE.
...
## Using withEntity
This technique enables you to add a table entity as an input and then declare the specific fields you require. This is useful if you want to use fields from a different table. The table does not have to be part of the view you are specifying.
...
- when a derived field has multiple inputs from a single table, only one input is required in the definition
- non-null fields on the entity will be non-null

By default, all fields are populated in the entity. So there could be a performance impact if you are using large tables; it is likely that your calculations won't need many of the fields are loaded in your calculation or in the final view. 

There two ways of mitigating this.

The first way is to load only non-null fields. Here is an example:
Non-null fields will always be populated. 
...
The second way is to specify the fields to be populated. Here is an example:

```kotlin
derivedField("SPREAD", DOUBLE) {
    withEntity(INSTRUMENT_PRICES, fields = listOf(INSTRUMENT_PRICE.ASK_PRICE, INSTRUMENT_PRICE.BID_PRICE)) { price ->
        price.askPrice - price.bidPrice
    }
}
```
