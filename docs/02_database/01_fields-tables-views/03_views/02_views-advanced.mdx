...
Derived fields are used to serve up data that is constructed from one or more fields that are then modified programatically before progressing.

For example if a table had a Notional field, and a Price field, you might define a Payment_Amount derived field as Notional * Price.

It is possible to define the derived field inputs as either the individually specified entity fields, or alternatively the entity itself, which 
will make all fields in the entity available for the derived field definition. 
...
- for larger tables there may be an unnecessary performance impact
