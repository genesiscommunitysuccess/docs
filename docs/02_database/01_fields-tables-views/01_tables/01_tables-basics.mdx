...
- a unique name and unique id; the id must be a number; this ensures that you can rename a table without losing the data in it
...
Here is a very simple example table definition. It contains a single table with three fields in it.
...
- The other two fields - INSTRUMENT_ID and QUANTITY - have no type specified - so their type defaults to STRING.
...
The primary key is the most important index (plural: indices). It is mandatory have at least one primary key, which must be defined by one or more fields that have a unique value or combination of values.
