...
To add a field, use the `field` function:
...
In the JVM world, we like to see enum names follow the UPPER_SNAKE_CASE convention. But often, an ENUM field ends up driving a user selection on the front end. So, if you want to present something more human-readable, you could use this syntax:
...
At the same time, the database actively ensures that no two records have the same `ID` or `EXTERNAL_ID`; this prevents duplication and ensures data integrity.

The example also has a non-unique index on `INSTRUMENT_ID`. This means that we can return all trades related to a specific instrument. A non-unique index simply provides a way of searching the table quickly; it provides usability. 
...
- However, we **cannot skip** a field and query the primary key on only USER_ID. So, we have provided a non-unique index on USER_ID, which enables us to find all projects associated with a specific user.
