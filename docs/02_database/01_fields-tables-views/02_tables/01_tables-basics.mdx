...
While in the JVM world, we like to see enum names follow the UPPER_SNAKE_CASE convention. Often, an ENUM field ends up driving a user selection on the front end. So, if you want to present something more human-readable, you could use this syntax:
...
In the example below, we set `PRICE` to be non-nullable
...
Adding a field to an index makes that field non-nullable.
...
At the same time, the database actively ensures that no two records have the same `ID` or `EXTERNAL_ID`; this prevents duplication, ensuring data integrity.

We also have a non-unique index on `INSTRUMENT_ID`. This means that we can return all trades related to a specific instrument. A non-unique index simply provides a way of searching the table quickly; it provides usability. 

Non-unique indices make no data integrity guarantees, other than to ensure that a value is provided.

### Explicit indices

You can also specify indices explicitly. This is useful in two ways:

- You can specify an index with more than one field.
- You can specify a name for the index (which overrides the auto-generated name).

The table below has exactly the same specification as the previous table, except that the indices have been specified explicitly -- after the field definitions.

The inline definition has the benefit of being more succinct. However, if you want to use multiple fields on an index or if you want to name the index, you must declare it explicitly.
...
### Default index name
By default, names are auto-generated for all indices. In most cases, the names work well.
...
The default index naming behaviour can be overridden for specific use cases such as:

* When an index contains numerous fields, resulting in an overly verbose name.
