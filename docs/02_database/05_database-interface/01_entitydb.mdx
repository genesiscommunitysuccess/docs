...
`getRange` is an operation that could return multiple records from the database.

There are two types of `getRange` operations:
* Single-value ranges, e.g. trades with CURRENCY set to USD
* Interval ranges, e.g. all trades with TRADE_DATE between 01-Jul-2024 and 01-Aug-2024.

Unlike the `get` operation, the `getRange` operation can always return multiple records.
We pass index entities into `getRange`.
When using an interval range, both index entities should be of the same type, `Trade.byDate` in this example.
...
When using the `getRange` operation, you can tell the database how many fields in the index to use.
By default, all fields are used, however, this can be overwritten.
This allows users to query on part of the index.

:::note
Order matters here; you can look get a range on just field 1, field 1 and field 2, but not on field 2 and field 1.
:::

For example, if on the ORDER table, we had an index on COUNTERPARTY and INSTRUMENT_ID,
we could do a single-value `getRange` operation on either `COUNTERPARTY` or on both `COUNTERPARTY` and `INSTRUMENT`.
In the first example `numKeyFields` would be `1`, in the latter it would be `2`.
...
This will work in the same way as `getRange`, however rows are returned in the reverse order.
...
in this state will result in an `IllegalArgumentException`. Each generated property will have two read-only
...
This operation is useful when a class needs to read a full table and then receive updates of changes to the underlying table or view.
...
There are many optional parameters for bulk subscribe operations.
In Kotlin this works well in a single method due to named parameters and default values.
In Java however, we provide a fluent API to set optional parameters.
The function `bulkSubscribeBuilder` return a builder.
...
`rangeSubscribe` is similar to `bulkSubscribe`.
The `rangeSubscribe` operation combines a [`getRange`](#getrange) and a [`subscribe`](#subscribe) call into a single operation.
This operation is useful when a class needs to read a full table and then receive updates of changes to the underlying table or view.
...
In addition, range subscribe supports both static and dynamic ranges, more details below.

#### Note on Java

Like `bulkSubscribe` there are many optional parameters for range subscribe operations.
In Kotlin this works well in a single method due to named parameters and default values.
In Java however, we provide a fluent API to set optional parameters.
These functions `staticRangeSubscribeBuilder` and `dynamicRangeSubscribeBuilder` return a builder.
Optional parameters can be set with a "with-function", e.g. `withDelay(...)`.
Once all values have been set, the `toFlowable()` function transforms the builder into a `Flowable`.
...
Additionally, `rangeSubscribe` also supports dynamic ranges.
These are ranges that are refreshed either at a specified time, or interval.
This can be useful for long running subscriptions on dates.
For example, you might want to keep an eye on trades booked in the last hour.
Unless that range is refreshed, it will fall behind.

Here we have two dynamic ranges, the first one is filtering for a specific date, and updated at midnight.
...
There are some subtleties when it comes subscriptions on views.

1. Generally, only updates on the root table will be published
2. Updates relate to the view, not the table.
...
By default, when subscribing to updates on a view, only updates on the root table, unless we enable `backwardsJoin` on the join.
...
This means that, from the view's perspective, a TRADE_VIEW record only exists, if a TRADE joins onto a INSTRUMENT
...
Let's look at how this would work with an example:

Imagine we insert a trade with quantity 100
Within milliseconds we update this trade and set the quantity to 200
...
Or we could receive a single update:
...
However, even if these operations happen immediately after eachother, we could still receive two updates.
