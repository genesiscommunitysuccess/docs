...
The `getRange` operation selects a range within an index. For example, to select all trades by a single currency, there are a number of ways a range can be specified:
...
When selecting part of an index, the number of columns can be specified using the `numKeyFields` parameter. The fields are always selected in the order they are specified in the index.

There are 15 different `getRange` functions. These return records in ascending order, unless you use the `fromEnd` functions, in which case the records are returned in descending order. 
...
The `subscribe` operation starts a database listener that receives updates to tables or views. When subscribing to view updates, only updates to the root table will be published. 

If the view has any inner joins, these are handled in a number of different ways.
...
However, if there is an inner join, there is a range of possible outcomes.

For example, if you join onto the counterparty field, and you only return records for which you have a valid counterparty:

* If there is an insert with no valid counterparty, the listener to the view does not receive the insert.
* If there is a delete with no valid counterparty, the listener to the view does not receive the delete.

And if there is a modify:
* If both the previous record and the new record link to an existing counterparty, then nothing changes.
* If the previous record was linked to an invalid counterparty and the new record links to a valid counterparty, it transforms the modify to an insert – because the record has moved into the view.
...
| --- | --- | --- | --- | --- |
| Table name |`String`| ✔️ | The table to subscribe to | n/a |
| Index name | `index` | ❌ | The index to sort the read by (used for the `getBulk` part of the call)| primary key |
...
The `rangeSubscribe` operation is similar to bulk subscribe, but it combines a `getRange` with `subscribe`. This is useful when a class needs to read part of a table or view and then keep updated of any changes. The range will be applied to the `subscribe` as well. So, the update type will reflect whether a row comes into the range or moves out of the range, rather than what happened in the database. This means that a `ModifyResult<E>` might be converted to an `InsertResult<E>` or a `DeleteResult<E>`.

AlNote that `rangeSubscribe` can take a static range (for example, all USD trades) or a dynamic range (for example, all trades booked within the last 2 hours). The dynamic range will be updated on a schedule, either at an interval or at a specified time.
...
| --- | --- | --- | --- | --- |
| Table name |`String`| ✔️ | The table to subscribe to | n/a |
| Start index | `index` |✔️ | The index entry to read from | primary key |
| End index | `index` | ❌ | The index entry to read to | primary key |
| fields | `Set<String>` | ❌ | Only listen to changes on selected fields (filters `ModifyResult` on fields)| listen to all fields |
| delay | `Int` | ❌ | Group and publish updates every x ms | no grouping |
| subscribeLocally |`Boolean` | ❌ | When in a cluster, only listen to local updates | false |
| backwardJoins  | `Boolean` | ❌ | subscribe to changes on sub tables (backwards joins) | false |
