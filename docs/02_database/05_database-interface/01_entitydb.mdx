...
The batching period is always set globally, i.e. for an update queue subscription or on a data server query.
...
Updates are published on the update queue once a record has been written to the database, or when a transaction completes you are using a transaction database.
...
1. A trade is inserted on node 1.
2. The trade is modified on node 2. 
3. Node 2 receives the insert update from node 1.
