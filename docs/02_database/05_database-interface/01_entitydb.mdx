...
Within a multi-node set-up, updates are published locally and forwarded to other nodes in genesis-cluster. As a result, sometimes a node can receive updates out of sequence.

This means that, for any clustered set-up with high message-volumes, it is essential that you check the timestamp against the latest received update when listening to updates on tables. This enables you to handle any messages out-of-sequence.
For views this is not a problem, as Genesis refreshes the record from the database within a transaction to make sure that the published update is valid.

If you use an external mqtt broker or JMS broker, there is a single centralised place to publish and consume data, so the chances of out-of-sequence updates are lower. This is dependent on where the message broker is located - as a broker in the same availability zone as a Genesis node has different latency properties from one in a different availability zone.

When using message brokers genesis-cluster is not involved in terms of message forwarding.
