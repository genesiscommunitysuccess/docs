...
Subscriptions to tables work in the standard way. You subscribe to updates, and:
...
#### Views
A view is created on a root table. Any insert, modify or delete on the root table works the standard way.
...
The `rangeSubscribe` operation is like bulk subscribe, but it combines a `getRange` with `subscribe`. This is useful when
