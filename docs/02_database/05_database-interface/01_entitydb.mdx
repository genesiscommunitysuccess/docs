...
Within a high throughput setup, where updates on the same table, are published from multiple sources, updates can be received out of sequence.
This means that when listening to updates, in a high throughput application, it is essential to check record timestamps to detect and handler out-of-sequence updates.
