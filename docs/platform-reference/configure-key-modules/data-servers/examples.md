...
By default, the primary table and all joined tables and views are monitored through a backwards join. Any changes to these are sent automatically.

Any join that specifies **backJoins = false** in the data server query will instead not be monitored. This can help reduce additional processing overheads, and is suggested to be used whenever the data in question is not needed to be updated in real time. For example, Counterparty data, if changed, can wait until overnight. The rule of thumb is that you should only use backwards joins where the underlying data is being updated intraday.
