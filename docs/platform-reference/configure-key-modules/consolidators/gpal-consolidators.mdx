...
similar to a sql group by query, in that there is allows developers to select fields, and apply aggregation functions
...
After a function is defined, it's output can be directed to an output field. To do this use the `into` keyword.
...
| onNotFound           | what to do if an output record i not found    | BUILD, WARN, IGNORE, FAIL, DEFAULT | TBC           |
...
log all event with debug level TBC, but this can be overwritten with a custom messages. To do this, use the `logJoin`,
...
and the `output` objects. The `input` property can be of the input rows picked up during the consolidation, so this
...
2. how the consolidator interacts with the database `into { ... }`
    ...
    2. how output record are build when no record is fround in the database `into { build { ... } }`
