...
You can specify which fields are on the request and which fields are on the response. You can also define derived fields which will be included on the reply, where the input for the derived field is the reply entity. Here is an example:
