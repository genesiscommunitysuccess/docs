...
GenesisSet is a generic format for sending (and receiving) messages between Genesis proesses. The information itin the emssages must be stored as key-value pairs.
A genesisSet can store integers, booleans, text, etc. Importatnly, it can also contain other genesis ests.
