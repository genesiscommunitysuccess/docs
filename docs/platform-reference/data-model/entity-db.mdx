...
1. `updateBy` - a unique index
2. `updateRangeBy` - an index range
3. `updateAll` - a whole table
...
Similar to in the async version, the row should be modified in place.
...
Currently, this is supported when using **FoundationDb** and **Postgresql**. Using transaction on Aerospike will
result in a failure.

When code is expected to run on multiple database types and transactions transaction and transactions should be used
when available, developers can use `safeReadTransaction` and `safeWriteTransaction`. This will run operations in the
