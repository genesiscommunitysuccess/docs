...
The entity db enables developers to interact with the database layer; yu can use any generated type-safe entities for
...
* One supports RxJava signatures, where return types are either `Maybe`, `Single` or `Flowable`, this will be the entity db to use from Java.

* The other flavour supports Kotlin coroutines. Instead of `Maybe` and `Single` there are `suspend` functions that
...
* Write operations
    ...
    * `upsert` / `upsertAll`
    * `delete` / `deleteAll`
    * `update` / `updateAll` / `updateByRange`
* Transactions
    * readTransaction
    * writeTransaction
* Subscribe operation
...
* the first field of the index used in the view

If a view has any one-to-many joins, then all unique indices, including the primary key, will be downgraded to a
non-unique index.

In the examples below, kotlin will use the `AsyncEntityDb`, whereas the java examples will use
the `RxEntityDb` type. The `AsyncEntityDb` is only available through kotlin, but the `RxEntityDb` can be
used in both java and kotlin.

We shall use the following table definition as our base:
...
the database, and can be modified in place, which the changes applied to the database. All update calls will use the
...
* when a record is modified we get a modify update 
* when a record is deleted we get a delete updatethe published update will
...
a class needs to read a full table and then keep updated of changes to the underlying table or view. 

This operation supports backward joins for views. This means that it can receive updates to the
root table as well as joined tables. The view needs to support this in the definition, and it must be enabled
...
a class needs to read part of a table or view and then keep updated of changes. The range will be applied to the `subscribe`
...
When building index classes, you do not have to provide all fields. However, the fields you provide must always be listed in order.
A unique index will turn into a non-unique index if not all fields are provided.
...
The entity database will return type-safe results for write operations. There are four main write results, one for each type of write operation. In all cases, the records in the result will be a generated entity that matches the
...
have the same super class, `GenericRecordUpdate<E>`, where `E` is the entity type. Using the entity db, you can
subscribe to table and view updates. There are three different update subtypes, which match the update operations:
...
When subscribing to view updates, the update listener listens to updates on the root table. The update type is based on whether the record is shown in the view or not. For example, if after a modify operation a row is no
longer visible in the view due to a inner join, the view subscriber will receive the update as a delete, rather than a modify.
