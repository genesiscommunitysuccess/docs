...
## Genesis Injection Framework

Genesis supports a familiar injection annotation framework;

## @Module

All classes requiring creation by the genesis bootstrap process need to be annotated with this Annotation. You will typically have at least one class with this annotation in your module.

```kotlin
@Module
class PriceFeed @Inject constructor(
                @Named("CONNECTION_URL") private val connectionUrl: String,
                ) {
...
Lastly, we have a combination of both “conditional on property” and “conditional on missing class “annotations, for a more consistent implementation in certain use cases. Referring to the previous example, we could use both features at once like this:
...
Framework properties can now be injected instead of read using SystemDefinitionService. This is the preferred approach, as the target value type (Int, Long, Double, String, Boolean, etc) can be specified directly as part of the injection mechanism.
...
ConfigProperties
There is a new annotation called ```@ConfigProperties``` which can subsitute simple XML file readers. For example, the genesis-webmon-config.xml has the following structure:


<WebMonConfig>
    <socketPort>9056</socketPort>
    <port>4567</port>
    <maxNoRecords>10000</maxNoRecords>
    <logoText>GENESIS</logoText>
    <logoUrl>http://www.genesis.global</logoUrl>
</WebMonConfig>
So it could be parsed automatically by the framework by defining a simple @ConfigProperties class with all its field values defined at the constructor level and a getter value for each one of the fields. This is particular neat if you are using Kotlin data classes as shown below:

```
@ConfigProperties
data class WebMonConfig(
    val socketPort: Int = 0,
    val port: Int = 0,
    val maxNoRecords: Int = 0,
    val logoText: String? = null,
    val logoUrl: String? = null
)
```
After defining you config properties data class you can proceed to inject it using @Inject like any other framework @Module.