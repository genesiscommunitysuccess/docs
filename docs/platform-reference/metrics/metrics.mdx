...
Genesis metrics is a module that allows developers to capture metrics for their components by inserting programmatic calls into appropriate places in their code.
...
To make use of the metric calls requires that the system be ran with `MetricsEnabled` set to true. In addition, you should define the `MetricsReportType`
to include a comma seperated list of ReportType outputs, which should include at least one the following:-
...
### Setup (example using SLF4J and GRAPHITE)

In this example we use a SLF4J log and Graphite Server to capture metrics. Detailed setup of a GraphiteServer is beyond the scope of this document,
...
You can use the method `MetricUtils.getInstance()` repeatedly in your code to get an instance of MetricUtils and make appropriate metrics calls, against the various metric types. See examples below.
...
### Metrics Available from Genesis Components
