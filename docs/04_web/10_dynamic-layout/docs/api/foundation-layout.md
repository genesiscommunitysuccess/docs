...
|  [LayoutEvents](./foundation-layout.layoutevents.md) | <p>**_(BETA)_** Defines events that the layout system emits</p><p>'first-loaded' - emitted when the layout has finished loading the first time using the declarative API after [DEFAULT\_RELOAD\_BUFFER](./foundation-layout.default_reload_buffer.md) ms. <br/> 'item-added' - emitted when an item is added to the layout' <br/> 'item-removed' - emitted when an item is removed from the layout'</p> |
