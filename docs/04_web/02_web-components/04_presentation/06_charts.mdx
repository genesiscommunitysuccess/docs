...
```
- You can then declare the bar chart in the **.ts** file for the page. This example displays for three fields in the ALL_STOCKS table.

```javascript title="chart declaration in .ts page"
<alpha-g2plot-chart type="bar" :config=${(x) => x.chartConfiguration}>
    <chart-datasource
        resourceName = "ALL_STOCKS"
        server-fields = "SYMBOL TRADING_VOLUME INDUSTRY"
    ></chart-datasource>
</alpha-g2plot-chart>
```
...
This chart displays nine rows of data from the ALL_STOCKS table. Effectively, these are three years, and each year has three categories of data. The chart groups (stacks) these categories for each year into a single bar with each category displayed in a different colour. 

If we had configured a standard bar chart (the default, or `isStack: false`), the chart would have displayed nine single bars, each in the colour of the category.
