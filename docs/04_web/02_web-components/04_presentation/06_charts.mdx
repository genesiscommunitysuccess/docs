...
This chart displays data from the EXAMPLE_RESOURCE in the database - which could be a table or a view. 

Three years are displayed as bars, and each year has trading volumes for three categories of data. The chart groups (stacks) these categories for each year into a single bar, with each category displayed in a different colour. 
