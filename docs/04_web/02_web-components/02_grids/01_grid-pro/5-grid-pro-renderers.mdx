...
The `grid-pro-renderers` enable you to render data in a way that is meaningful to the user. The rendering can vary from one column to another, from boolean-type columns that need to be rendered as a `checkbox` to columns that need to be rendered as a percentage.
...
This adds a checkbox to the cell. To use this build-in renderer, follow these steps:
...
```
After these steps, you can see the new column with the boolean built-in renderer in your grid.
:::tip
...
It adds a nested action menu to a cell in your grid. To implement this feature, you need to follow these steps:
