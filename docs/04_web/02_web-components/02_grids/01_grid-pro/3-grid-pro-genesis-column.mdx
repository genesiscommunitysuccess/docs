...
This is a `slotted` component that allows a more "visual approach" when defining columns. Each `<grid-pro-column>` takes a `ColDef` typed object. To check all the available fields for the variable type `coldef`, [look here](https://www.ag-grid.com/javascript-data-grid/column-properties/).
...
To create or configure columns in the grid, there are two main approaches that need to be implemented

### ColDef

Create a new `ColDef` variable. This will store all the configuration for the columns in your grid. There is an extensive list of fields and parameters that can be used. Here are the most common fields:
...
When using `ColDef` objects, it's up to you to decide the approach. You can use the directive **repeat** to create an array of definitions, or you can create one column at a time.
...
You can now create a new custom column defining the `field`, using the same name as the field that comes from the back end. The grid then automatically sets the data to be displayed in this field. Let's see an example.

If you connect your grid with the back end through a Data Server query called `ALL_TRADES` that has these fields: `TRADE_ID`, `PRICE` and `QUANTITY`, the implementation would be like this:
...
Now, in the context/component's own class, you can define a new column. The code below defines a `field` that has the same as a field in the back end: **PRICE**. It pins this to the right.
...
- `cellRendererParams` enables you to supply any parameters that are required.
...
In this previous example, you can change the return of the `customCellRenderer` variable to other web components, such as buttons or comboboxes.
