...
`;
```
Note that inside the repeat directive, the context is `myMultipleCustomColumnConfigArray` instead of `YourTemplate`. That is the reason why we can use `${x => x}` to access the myMultipleCustomColumnConfigArray elements.
...
Now if you create a new custom column defining the `field`, and this `field` has the same name as the field that comes from the back end, then the grid automatically sets the data to be displayed in this field. Let's see an example:
...
If you don't define any additional columns, you would have this grid displayed:
...
We are defining the `field` with the same as a field that comes from the back end: **PRICE** and we are pinning it to the right. Now let's create a new custom column in our HTML template:
...
- **cellRenderer**: The cell renderer is responsible for creating the structure of the cell. Here you need to return a string containing the HTML that will be rendered in that cell.
