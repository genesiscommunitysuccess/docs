...
Now each record in the grid has a **Modify** and a **Delete** button displayed on the right of the grid.

### Common attributes

The most common attributes you can configure for `entity-management` are:

| Attribute                  | Description                                          |
| -------------------------- | -----------------------------------------------------|
| `title`                    | customises the title of the entity list so that the user can see what resource they are managing. |
| `resourceName`             | specifies the name of a resource on the server: a `query` in the Data Server, a `ReqRep` in the Request Server or an `eventHandler` in the Event Handler.  |
| `createEvent`              | specifies an event on the back end for adding a record to the table; once specified, this displays an **Add** button above the grid. |
|`updateEvent`               | specifies an event on the back end for modifying a record in the table; once specified, this displays an **Edit** button to the right of every record in the grid. |
|`deleteEvent`               | specifies an event on the back end for deleting a record in the table; once specified, this displays a **Delete** button to the right of every record in the grid. |
| `datasourceConfig`         | configures the [interaction with the back-end resource](./docs/api/foundation-entity-management.datasourceconfiguration/#example). |
