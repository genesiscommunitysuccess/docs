...
| `columns`         | enables you to supply an array of `ColDef[]` properties to configure the grid. | No  |
...
If you want to control the content of a form - for example, you might want to use different forms for inserting a new record and for modifying an existing record - use either the `createFormUiSchema` attribute or the `updateFormUiSchema` attribute. These enable you to provide a schema that configures the form. There is more information about using these attributes in the [API documentation](/web/micro-front-ends/foundation-entity-management/docs/api/foundation-entity-management.entitymanagement/)
