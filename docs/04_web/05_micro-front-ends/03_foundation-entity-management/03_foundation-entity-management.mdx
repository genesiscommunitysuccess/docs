...
When you use an `entity-management` you can use the following slots to define content and actions. The slots are dynamically generated based on the `crud-menu-position` attribute value.
...
The `crud-menu-position` attribute can be one of the following values: `column`, `top`, `bottom`, `action`. Based on this value, the following slots will be available:
