...
1. Bootstrap the packages, which installs all their dependencies and links any cross-dependencies.
...
From inside the web application's folder (e.g. client/web/), now run this command:
...
Please do not use npm to install or manage dependencies, because it causes problems with lerna. Unfortunately, lerna can
...
After this you should (must) run bootstrap again.
