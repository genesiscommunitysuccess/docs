...
It's possible to create a new route or a new component defining its name and clicking on Create.

The new route will be created under the folder `routes` and declared on the `config.ts` file.

The new component will be created under the folder `components` and declared on the `components.ts` file.

#### Component with Datasource

It's also possible to create a grid or a chart with a datasource plugged into the back-end. All the queries available will be identified by the AI and will be loaded for the user to pick up. After clicking on Create, all necessary files will be created automatically in the correct folder.
...
Within the extension, you can also find links to [Official Documentation](https://learn.genesis.global/docs/getting-started/) and [Genesis Playground](https://playground.demo.genesis.global/).
