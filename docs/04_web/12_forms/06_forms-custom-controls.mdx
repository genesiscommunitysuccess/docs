...
You need to create a custom renderer template to render your new form control. In this example you will create a control that renders options using the [Segmented Control](https://learn.genesis.global/docs/web/web-components/form/segmented-control/?_highlight=segment) element.
...
- `mapper` a function to map form state to control props. We use the `mapStateToControlProps` from json-forms here.
...
The last step to use your new custom renderer is to register it with foundation-forms and give the right config so that the form class knows to render it.

In creating your UiScchema remember that the tester function is checking if `segmented` is set in the options of the `UISchemaElement`.
