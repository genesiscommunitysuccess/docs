...
The password control schema creates a password field. This is a text field with the input characters obscured. You invoke this renderer by setting the `isPassword` property in `options` to be `true`.
...
The textarea control schema creates a textarea field. This a multi line entry field. You invoke this renderer by setting the `textarea` property in `options` to be `true`.
