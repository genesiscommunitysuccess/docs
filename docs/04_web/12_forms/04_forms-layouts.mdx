...
The layout for a form is configured in the `type` property of the [UISchema](/web/forms/docs/api/foundation-forms.uischema/?_highlight=uisc#uischema-type)
...
This is similar in principle to groups. Here, each group (section) is shown in its own tab, and only one tab is visible at a time.
