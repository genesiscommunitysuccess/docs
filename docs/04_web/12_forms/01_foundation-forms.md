...
1. Add `@genesislcap/foundation-forms` as a dependency in your **package.json** file. Whenever you change the dependencies of your project, ensure you run the `$ npm run bootstrap` command again. You can find more information on the [package.json basics](../../../web/basics/package-json-basics/) page.
...
Below is an example of a `foundation-form` using the `resourceName` attribute. This attribute must be an event defined in your_*application-name*_-eventhandler.kts**; in this case, the event is called EVENT_TRADE_INSERT, and it must be designed to insert all information provided in the forms to the database.
...
You can also provide a `UI schema` to the `foundation-forms` in order to customise which fields should be displayed in the forms. In the example below, we set only the fields `QUANTITY` and `SIDE`.
