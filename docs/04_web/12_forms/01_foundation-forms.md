...
Below you will find an example of a `foundation-form` using the `resourceName` attribute. This attribute must be an event from the eventhandler.

```html
<foundation-form resourceName="EVENT_TRADE_INSERT"></foundation-form>
```

By doing that, the event `EVENT_TRADE_INSERT` will insert all information given in the forms to the database. Remember that you need to create this event in the **{*application-name*}-eventhandler.kts**.

This example generates a working form based on the `JSON schema` for that endpoint. The DevTools console will output an autogenerated `UI schema` that you can use to configure the form.

### Configure form using UI schema

You can also provide a `UI schema` to the `foundation-forms` in order to customize which fields should be displayed in the forms. In the example below, we set only the fields `QUANTITY` and `SIDE`.
...
### An example of synchronizing values with datasource criteria
