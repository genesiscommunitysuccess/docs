---
title: 'How to join tables and expose the result'
...
- The view has been given the name TRADE_CLIENT. This is the the name that the server functions can call to retrieve the data.
- You need a root table as the basis if the view - in this case, TRADE.
...
Here is an example of an application's application's _app-name_**-dataserver.kts** file. It contains one query that makes the view ALL_TRADE_CLIENTS available as a data source to the front end.

- The view has been given the name TRADE_CLIENT. This is the the name that the front end can call to retrieve the data from the two underlying tables.
