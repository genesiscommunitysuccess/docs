...
Here is an example of a simple  _app-name_**-view-dictionary.kts** file that defines one view.

- The view has been given the name CD_TRADE_CP. This is the the name that the front end can call to rerieve the data.
- You need a root table as the basis if the view - in this case, CLIENT_DETAILS.
- There is a join that connects this table to the table 
...
```kotlin
views {
    view("CD_TRADE_CP", CD_TRADE) {
        joins {
            joining(CLIENT_DETAILS) {
                on(CD_TRADE.CLIENT_ID to CLIENT_DETAILS {CLIENT_ID})
            }
        }
        fields {
            CD_TRADE.allFields()
...
Here is an example of an application's application's _app-name_**-dataserver.kts** file. It contains one query that makes the view ALL_CD_TRADES_CP available as a data source to the front end.

- The view has been given the name CD_TRADE_CP. This is the the name that the front end can call to retrieve the data from the two underlying tables.

```kotlin
dataServer {
    query("ALL_CD_TRADES_CP", CD_TRADE_CP) {}
}
```
