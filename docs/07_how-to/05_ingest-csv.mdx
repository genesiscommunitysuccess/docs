...
### Operators
Operators are used to transform input format to output format (although they can do more). You can chain them together, where each operator simply takes the output type of the previous one. So in this case, so the output of `.split` goes into `.map` (and the output type of map is a list of DbOperations):
...
The example sink below takes the output from map (a set of DB operations) and handles the updating of the database accordingly.
...
<aside>
⚠️ Waiting on error handling to add a section on it here
...
Read the [README.txt](https://github.com/genesislcap/platform-howto/blob/master/howto-csv-ingress/README.md) for a full overview and details of how to test the application.
