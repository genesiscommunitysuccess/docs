...
Go to the file **OTE\server\OTE-app\src\cfg\OTE-tables-dictionary.kts**. Here we shall change the definitions of the following tables:
...
Currently, the code for the `PASSIVE_ORDER_BOOK_INSERT` and `PASSIVE_ORDER_BOOK_DELETE` events is in the file **OTE-eventhandler.kts**. But it would be useful [why?] to manage these in a different file. 
