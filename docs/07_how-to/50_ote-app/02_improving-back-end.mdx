...
A. Go to the file OTE\server\OTE-app\src\cfg\OTE-tables-dictionary.kts here we change the table definitions for the following tables:
`CLOSING_PRICE`
`CASH_BALANCE`
...
3. Find the `primaryKey` line for the PARTICIPANT_POSITION table definition. Update it as follows:
...
B. [Rationale for the below step to be provided:]
...
C. Go to the folder OTE\server\OTE-app\src\main\Kotlin, here we will add an additional folder structure and a file.

1. If using intelliJ: 

1.1. Right click on `add new package` and call this package genesis.global.message.event

1.2. Then right click on the genesis.global.message.event package and add a new "Kotlin class" called ExpireOrderMessage.kt

2. If in Launchpad:

2.1. Create the following folder structure manually OTE\server\OTE-app\src\main\Kotlin\genesis\global\message\event
2.2. Add file `ExpireOrderMessage.kt` to the event folder in the folder structure just created
...
As we made some changes to the primary Keys in step A above, we now need to edit the consolidators, to take this changes into consideration and solve for the new primaryKey aggregations
...
1. In the PARTICIPANT_POSITION_AGG consolidator find `PARTICIPANT_POSITION_AGG`. Update it as follows:
...
3. Find [ information to be added] . Update as follows:

```
eventHandler<PassiveOrderBook>("PASSIVE_ORDER_BOOK_MODIFY", transactional = true) {
```

J. All of the eventHandlers set out below will need to be removed in their entirety:
    ...
    eventHandler<ExpireOrderMessage>("RUN_CLOSING_PRICES") {
        onCommit {
            ...
            entityDb.getBulk(INSTRUMENT).toList().forEach {
                val trades = entityDb.getRange(Trade.byInstrumentTradeDatetime(it.instrumentId, now().withTimeAtStartOfDay().plusHours(priceCheckStartHour)),Trade.byInstrumentTradeDatetime(it.instrumentId, now().withTimeAtStartOfDay().plusHours(priceCheckEndHour)))
                    .toList()
