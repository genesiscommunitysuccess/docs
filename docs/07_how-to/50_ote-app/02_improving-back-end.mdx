...
To handle the passive order book correctly, you need to do two things:

- create a new file with the relevant logic; we provide a large code snippet here to provide that logic
- the code for the `PASSIVE_ORDER_BOOK_INSERT` and `PASSIVE_ORDER_BOOK_DELETE` events in the **OTE-eventhandler.kts** file is no longer necessary, so you need to remove it 
