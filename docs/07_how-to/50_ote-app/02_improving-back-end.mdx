...
```
This will ensure that the appropicate closing price will be used based on the closing date.
...
```
This allows the the application to determine a Participants Cash Balance for a particular currency
...
Modify the CASH_BALANCE_AAG consolidator so that the new aggregations are included as a result of the primary key changes that were made.
...
To ensure that the primaryKey changes can be handled by the relevant `eventHandler` codeblocks, the following changes need to be made:
...
Currently, the code for the `PASSIVE_ORDER_BOOK_INSERT` and `PASSIVE_ORDER_BOOK_DELETE` events is in the file **OTE-eventhandler.kts**. This will be managed in the New OTE-pob-eventhandler.kts file.

Go to **OTE\server\OTE-app\src\main\genesis\scripts**. Add a file called **OTE-pob-eventhandler.kts**.
