...
You can change the existing 2x2 grid layout to a more sophisticated layout where:
...
Go to **OTE\client\src\routes\orders\orders.template.ts**. Update it as follows:
...
Because you have updated the schema for creating a new record, you also need to change the schema for updaing a record.
...
In a Production application, these types of processes can be triggered automatically by a CRON_RULE.

But here, we can simulate these processes by adding buttons that enable the user to trigger the processes. Here, we shall add buttons to trigger three processes:
...
```
4. Now you can add the action buttons that enable the user to trigger the processes. Find the line `></entity-management>` and update it so that is is split as follows:
...
The buttons now are displayed at the top right of the menu for the component. When the user clicks on a buttons, it triggers the relevant event.
