...
To simulate the intraday and end-of-day processes that this type of application requires, you need to add that enable the user to trigger these processes. 

In a Production application, these types of processes can be triggered automtically by a CRON_RULE.
...
```
4. Find the line `></entity-management>` and update it so that is is split as follows:
