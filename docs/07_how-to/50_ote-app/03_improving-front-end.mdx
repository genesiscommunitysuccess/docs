...
In the GUI we are going to modify the grid layout, changing the 2x2 grid layout, to a layout whereby orders are displayed at the top of the screen and three section are displayed beneath the Orders grid.
...
As we updated the create form schema we also need to update the update form schema.
