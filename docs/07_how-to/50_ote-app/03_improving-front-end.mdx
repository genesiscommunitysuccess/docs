...
But here, we can simulate them by adding buttons that enable the user to trigger the processes. Here, we shall add buttons to trigger three processes:
...
```
4. Now you can add the action buttons that enable the user to trigger the processes. Find the line `></entity-management>` and update it so that is split as follows:
