...
IBM Message Queue enables different applications to send and receive messages reliably through message queues.

The Genesis Platform enables you to consume from and publish to any JMS message queue, including IBM.

These message queues are often used to access trade data, for example. You can create pipelines to consume trade data directly from the message queue and into your Genesis application.

## Example

We have created an [example application](https://github.com/genesiscommunitysuccess/howto-ibm-mq-pipelines/tree/main) that shows how to publish to an IBM message queue and consume incoming messages.
