...
## Handling null
his `eventHandler` has an `onValidate block`, which checks that the counterparty on the trade exists in the database. If the counterparty does not exist in the database, then it returns a nack() with an error response.
...
## For loop
This `eventHandler` validates the counterparty the same way as the previous example. 

in the `onCommit` block, it iterates through each trade in the `tradesForCounterparty` list and 
