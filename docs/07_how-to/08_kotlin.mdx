...
## Custom class
Sometimes you want to have an event that contains different fields to the generated classes that are created from Genesis code generation.

For that, you can create a custom class.

1. Create your custom class. This could be in your main module, a different module or even in your GPAL script. In this example we are going to create a new class in the main module. 
...
2. Add an import of the class to the GPAL script you want to use it in. This goes at the top of the file. In this example we will add it to the eventhandler script.

```kotlin
import global.genesis.EventTradeReview
```

3. Use the custom class in an event handler. Below `EventTradeReview` is the eventHandler type.
