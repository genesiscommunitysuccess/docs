...
## Handling null
This `eventHandler` has an `onValidate` block, which checks that the counterparty on the trade exists in the database. If the counterparty does not exist in the database, then it returns a `nack()` with an error response.
