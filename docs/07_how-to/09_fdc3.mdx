...
- a message on the color channel with `genesis.fxpair` - use the `mappingFunction` to create and send an object with source and target currency values.  // // use the mappingFunction... is this an instruction or a description?
