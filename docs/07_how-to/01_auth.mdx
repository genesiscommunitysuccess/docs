...
The Genesis Application Platform makes it easy to set up precise authorisation models based on your business requirements.

By default, everything is accessible by all users who have access to the application. But you can control **access to data** and **the ability to change it** at three levels:

- tables (data entities)
- records within a table (rows)
- fields within a table (attributes/columns)

All levels can be configured at the server level to ensure complete security; but you can also add authorisation at the front end to remove views and commands that would otherwise not work or be empty for a given user (i.e. where authorisation has not been granted).
