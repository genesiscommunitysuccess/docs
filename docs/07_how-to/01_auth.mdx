...
You can then easily create a further "super user" event for modifying trades in any state. The example below enables a user with the Right code **SuperUser** to perform any edit, where the prior `onValidate` rejects certain states in the standard event. This gives us a simple authorisation at an entity state level.
