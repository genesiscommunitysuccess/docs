...
To perform similar column-level authorisation for the event handler, you need to decide how you handle edits by a user who is not authorised to see (or therefore change) a column. For example, on **modify** you could force the unseen column to remain constant. The code below looks up the value on the prior version and sets it on the proposed modification before saving, based on the RIGHT CODE TradeViewFull.
```jsx
