...
- User AA can only see trades in Germany, but cannot create, edit or delete trades in other countries.
- User BB can see trades in France, and can create, edit or delete trades in other countries.
- User CC can see trades in Spain, but cannot create, edit or delete trades in that country.
- User DD can see, create, edit and delete trades for all countries. 
...
For testing the auth permissions we have described here on the front end and back end, you can use the common automation frameworks: cucumber and playwright. Test cases are written in the Gherkin language.
...
The same set of tests is used to verify the behaviour on the UI side.

In the feature file, we use tags to differentiate between the UI and API tests.

- @API means we are testing via the back end, so the back-end processes must be running and healthy.
- @UI the test requires the back end and the front end to be up.
