...
Until you start to specify any permissions, everything is accessible by any authenticated user. But you can insert permmissioning statements in the relevant places control **access to data** and **the ability to change it**.

You can also control which types of change to data are permitted: (create, edit, delete).

All you need to do is specify the permissions at each point. The GENESIS_AUTH component ensures that only users with these permissions are enforced. 

### Permissioning the back end
Broadly speaking, you can insert permissioning statements on the back end at three levels:

- tables (data entities)
- records within a table (rows)
- fields within a table (attributes/columns)

### Permissioning the front end
...
## Permissioning tables (data entities)
...
| User D |                                                                                                   | **view** the grid of trades, or **create**, **edit** or **delete** trathemdes |
...
So, as a reult of these permissions:
...
## Permissioning fields (attributes/columns)
