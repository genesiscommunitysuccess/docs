...
Until you start to specify any permissions, everything is accessible by any authenticated user. But you can insert permmission statements in the relevant places control **access to data** and **the ability to change it**.

You can also control which types of change to data are permitted: (create, edit, delete).
...
In a similar way, you can control the visibility of each User Interface component. For example, you can permission a component that displays a grid of data from the **TRADE** table so that different users have a different experience:
...
For example, continue looking at our **trade** table and consdier the following permissioning requirements:

| User          | **is permitted to**         |**is _not_ permitted to**   |
| ------------- | --------------------------- | -------------------------- |
| User AA | **view** trades (rows) only where the country is DE                                          | **create**, **edit** or **delete** trades in other countries             |
| User BB | **view** trades (rows) only where the country is FR, and **is permitted** to **create**, **edit** or **delete** trades in other countries   | **delete** trades                                                     |
| User CC | **view** trades (rows) in all countries except ES, and **create**, **edit** and *delete**  them in those countries |        **view** trades (rows) where the country is ES, or to **create**, **edit** or **delete** trades in this country           |
| User DD | **view**, **create**, **edit** or **delete** trades (rows) in all countries |                           |

As a result of these permissions:

- User AA can to **view** trades (rows) where the country is DE, but **is not permitted** to **create**, **edit** or **delete** trades in other countries.
- User BB can to **view** trades (rows) where the country is FR, and **is permitted** to **create**, **edit** or **delete** trades in other countries.
- User CC can **view** trades (rows) where the country is ES, and **is not permitted** to **create**, **edit** or **delete** trades in this country.
- User DD can **view**, **create**, **edit** or **delete** trades (rows) for all countries. 
...
When you do this, all queries or events within the  (file) receive these permissions. But you can override these within any specific query or event. So, effectively, you are providing default permission for all the queries or events in the file.
...
:::info
Go to our [**Testing**](/how-to/ht-auth-test/) page for details of how to test authorisation in your application.
:::
