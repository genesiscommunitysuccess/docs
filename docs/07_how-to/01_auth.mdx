...
- The [feature file](https://github.com/genesiscommunitysuccess/howto-permissions/tree/main/server/permissions-app/src/test/resources/featuresPermission.feature) contains the test cases, which are written in the Gherkin language. Tags are used to differentiate between the UI (`@UI`) and the API (`@API`).
