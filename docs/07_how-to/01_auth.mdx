...
The example below ensures the column is not returned by the query:

For column-level permissions, use `auth` and `hideFields`:
```jsx
  query("ALL_TRADES", TRADE) {
    permissioning {
      auth {
...
To perform similar column-level authorisation for an event handler, you need to decide how you handle edits by a user who is not authorised to see (or therefore change) a column. For example, on **modify** you could force the unseen column to remain constant. The code below looks up the value on the prior version and sets it on the proposed modification before saving, based on the Right Code **TradeViewFull**.
...
In addition to the authorisation shown it is also possible to apply authorisation levels at an entire dataserver or eventhandler level.  

When you do this, all queries or events within the respective component receive these permissions. But you can override these at the specific query or event level.
This can be useful where a project has collections of functions; you can create separate dataservers and eventhandlers so that authorisation can be done at a group level.

Here is an example, where each query takes the authorisation of the dataserver:
...
It is possible to use the authorisation model to manage more complex situations.

For example consider a state model - perhaps we want to ensure that only super users can modify trades that have settled.  By adding the following to the modify event we prevent users changing the transaction if it has a status of "Settled":
```jsx
  eventHandler<Trade>("TRADE_MODIFY", transactional = true) {
    permissioning {
...
We can then easily create a further "super user" event for modifying trades in any state:
```jsx
...
```
The above would enable a user with the RIGHT CODE "SuperUser" to perform any edit, where the prior onValidate rejects certain states in the standard event.  This gives us a simple authorisation at an entity state level.
...
Sell also https://learn.genesis.global/docs/server/event-handler/basics/#adding-validation for onValidate use and examples
