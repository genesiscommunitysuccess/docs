...
## Multiple field keys
For some tables, it makes sense to use a key based on multiple fields rather than one, as we may want to store multiple values for specific keys. 

For example, in the `FX_RATE` table, we would like to index on both `TARGET_CURRENCY` and `SOURCE_CURRENCY`. 

Let's add these now.

1. Go to the file **ALM\server\ALM-app\src\main\genesis\cfg\ALM-tables-dictionary.kts**. Here, we can change the table definitions for two tables:

- `POSITION`
- `FX_RATE`

2. Find the `primaryKey` line for the POSITION table definition. Update it as follows:
...
3. Find the `primaryKey` line for the FX_RATE table definition. Update it as follows:
...
Having made these changes, we also need to update the events in the Event Handler file, because these will still be calling the old primary keys.

1. Go to the file **ALM\server\ALM-app\src\main\genesis\scripts\ALM-event-handler.kts**.


2. Update the primary key for the POSITION_DELETE event to:
...
3. Update the primary key for the FX_RATE_DELETE event to:
...
It's worth then rebuilding the project at this point, as we'll be using these new `primaryKeys` in later steps.
...
As the `groupBy` block indicates, this consolidation uses only using the source currency from the`FX_TRADE` table. As a result, we only have a single row inserted or updated in the `POSITION` table for each `FX_TRADE`.
...
:::info
You can handle two consolidation operations with a single consolidator block simply by including two group by statements [documented here](https://learn.genesis.global/docs/server/consolidator/advanced/?_highlight=consolid#multiple-groupby-statements).
