...
Here is a simple example of a custom endpoint class. It defines an endpoint which takes file uploads called `file-handler/upload` that sends an HTTP 200 OK response on a successful upload.
...
In the examples above the initialisation step is annotated with `@PostConstruct`, and calls upon the `WebEndpointRegistry.registerEndpoint()` function with the subdirectory of the endpoint, and the endpoint itself.

### Endpoint Name
The `name()` method must be overridden to provide the endpoint a name. Once configured the endpoint will be reachable at the address comprised of the subdirectory given to the `WebEndpointRegistry` and the result of this `name` function. In the example above this would be `file-handler/upload`.

### Allowed Methods
The `allowedMethods()` function must be overridden to declare which of the HTTP request types are permitted for this endpoint. It must return a set of `RequestType` objects corresponding with the HTTP `GET`, `POST`, `PUT`, `PATCH`, and `DELETE` functions.
