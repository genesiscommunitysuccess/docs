...
Configuring the cache is covered later in this document, but you should certainly consder the following when you write R scripts that take advantage of the cache:

 * Avoid using `AddFilter()` in your snapshot pipeline unless the peformance improvement of the filter outweighs the performance improvement of the cache (which is possible) -  as the cache will not be used.
