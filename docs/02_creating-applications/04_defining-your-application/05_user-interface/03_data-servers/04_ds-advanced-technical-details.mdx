...
The Data Server criteria matching supports some common expressions, all called using the `Expr` special binding as mentioned above. All of these return a boolean value (`true` or `false`) and are especially helpful for `Date` and `DateTime` client-side filtering.
