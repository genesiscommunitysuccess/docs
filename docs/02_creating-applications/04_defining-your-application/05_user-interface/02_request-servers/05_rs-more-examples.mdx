...
A request server is defined in your _application_**-reqrep.kts** file.  It consists of one or more `requestReply` statements that return data from the database. The examples on this page should help you get started quickly.

The example below is the [simplest request server you could have](/creating-applications/defining-your-application/user-interface/data-servers/ds-technical-details/#the-simplest-possible-definition). It has a single `requestReply` statement that will return all the fields in the specified table.
...
The example below has two `requestReply` statements. Again, each one returns all the fields in the specified table.
...
The example below has just one `requestReply` statement. Note that it now [specifies the fields that should be returned](/creating-applications/defining-your-application/user-interface/request-servers/rs-technical-details/#specify-fields-on-request-and-reply), so you don't get all the fields in the table. 
```kotlin
requestReplies {
    requestReply(INSTRUMENT_DETAILS) {
...
The example below has a `requestReply` statement [`where` block](/creating-applications/defining-your-application/user-interface/request-servers/rs-technical-details/#where-block) that controls access to the request in two ways (not the code ALLL3 and an ALTERNATE_TYPE listing of either RIC or BLOOMBERG).
```kotlin
requestReplies {
