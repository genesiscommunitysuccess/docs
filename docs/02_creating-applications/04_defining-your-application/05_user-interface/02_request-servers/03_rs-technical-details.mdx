...
- A Request Reply is a component that supplies static data to the front end of your application.
...
When defining a `request` block, you must define at least one primary key or index.
...
```
Note the following:
- When you do not define a `request` block, the primary key is the default request field.
