...
Event hadlers can be written in Java using event handler [APIs](/reference/developer/api/event-handler-api). In this section events are written using the Rx3 Event handlers defined [here](/reference/developer/api/event-handler-api/#rx3eventhandler)

:::note

We recommend using **Kotlin** to implement Event Handlers.

- **Java** Event Handlers can be implemented using [**RxJava3**](#rx3) [**Sync**](#sync) Event Handlers only.
...
## Adding a name
Every `eventHandler` must have a unique name. If you do not provide one, it will be allocated automatically to default name as specified in previous example.
...
So far, we have overridden process method in our `eventHandler`. This is where the active instructions are - usually database changes.

If you want to provide some validation before the action, you need to implement Rx3ValidatingEventHandler interface and override onCommit and onValidate methods.

`onValidate` method will be executed first and `onCommit` method will only be executed if the `counterparty` field is not null.
...
In previous example the `onValidate` method:
...
If you want your  `eventHandler` to comply with ACID, you need to use RxEntityDb [writeTransaction](reference/developer/api/database/how-to/interface/entity-db/#write-transactions). Any exception or nack returned will result in a complete rollback of all parts of the `onCommit` and `onValidate` (the transaction also covers read commands) methods.
...
implement Rx3ContextValidatingEventHandler interface, as shown below:
