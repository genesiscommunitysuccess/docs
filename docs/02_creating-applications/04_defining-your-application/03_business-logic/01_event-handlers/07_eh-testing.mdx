...
For more information about `AbstractGenesisTestSupport`, see the [testing docs](/managing-applications/test/integration-testing/).

Once you have set up your configuration, we can start writing tests against our Event Handler.

### Writing tests

Let's write some tests for this simple event handler defined below. If you are looking for Java event then ignore onValidate method for now
...
We may also want to test a negative case of when we expect to receive an error as a response.
You need to modify the previous example Event Handler and add onValidate block:
...
To test that the Event Handler authorisation works correctly, we need to do some setting up.
...
For Java event example refer this document [Authorization API](/reference/developer/api/authorisation-api)
...
Third, we need to specify the auth cache override in the `GenesisTestConfig`:
...
Fourth, in our test set-up, let's authorise one user to be able to insert trades and another who is not.
