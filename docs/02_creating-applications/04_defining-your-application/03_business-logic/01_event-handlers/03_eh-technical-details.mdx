...
```java
    ...
    import org.jetbrains.annotations.Nullable;
    @Module
    public class EventCounterParty implements Rx3EventHandler<Counterparty, EventReply> {private final RxEntityDb entityDb;@Inject
    ...
    public class ContextEvent implements Rx3ContextValidatingEventHandler<Company, EventReply, String> {
        ...
        public Single<ValidationResult<EventReply, String>> onValidate(@NotNull Event<Company> event) {
            ...
            if(company.getCompanyName().equals("MY_COMPANY")) {
                return Single.just(validationResult(ack(this).blockingGet(),"Best company in the world"));
            } else {
                return Single.just(validationResult(ack(this).blockingGet()));
            }
        }
    }
```
