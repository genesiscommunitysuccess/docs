...
```java
    ...
    @Module
    public class EventCounterParty2 implements Rx3ValidatingEventHandler<Counterparty, EventReply> {
        ...
        public Single<EventReply> onValidate(@NotNull Event<Counterparty> event) {
            ...
            if(counterparty.getName().isEmpty()) {
                List<GenesisError> genesisErrorList = new ArrayList<>();
                genesisErrorList.add(new StandardError("Standard Error", "Counterparty should have a name"));
                return Single.just(new EventReply.EventNack(Collections.emptyList(), genesisErrorList));
            }
            return Single.just(new EventReply.EventAck());
        }
    }
...
```java
    ...
    public Single<EventReply> onValidate(@NotNull Event<Counterparty> event) {
        ...
        if(!counterparty.getName().equals("MY_COMPANY")) {
            List<GenesisError> genesisErrorList = new ArrayList<>();
            genesisErrorList.add(new StandardError("Standard Error", "We don't accept your company"));
            return Single.just(new EventReply.EventNack(Collections.emptyList(), genesisErrorList));
        }
        return Single.just(new EventReply.EventAck());
    }
    ...
    public class ContextEvent implements Rx3ContextValidatingEventHandler<Company, EventReply, String> {
        ...
        public Single<EventReply> onCommit(@NotNull Event<Company> event, @Nullable String context) {
            ...
            if (context != null) {
                ...
                parsedContext = "Missing context";
            }
            Company company = event.getDetails();
            entityDb.insert(company);

            return Single.just(new EventReply.EventAck(List.of(Map.of("VALUE",parsedContext))));
        }
        ...
        public Single<ValidationResult<EventReply, String>> onValidate(@NotNull Event<Company> event) {
            Company company = event.getDetails();
            if(company.getName().equals("MY_COMPANY")) {
                return Single.just(validationResult(EventReply.EventAck(), "Best company in the world"))
            } else {
                return Single.just(validationResult(EventReply.EventAck()))
            }
        }
    }
```
