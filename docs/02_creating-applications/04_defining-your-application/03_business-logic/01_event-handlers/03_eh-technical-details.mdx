...
Here is a simple example of an Event Handler file. It defines an Event Handler of type `<Counterparty>`, which inserts a counterparty into the database, using the [entityDb](/reference/developer/api/database/how-to/interface/entity-db/).
...
- an input type
- an `onCommit` block, which is where you specify the actions the Event Handler is to perform
The `onCommit` block must return either an `ack()` or `nack(...)`.

Now let's build on our simple example and look at some of the most useful options you can add.

## Adding useful options

### Name
Every `eventHandler` in your **.kts** must have a unique name. If you do not provide one, it will be allocated automatically. In this case, the Event Handler will automatically be named EVENT_COUNTERPARTY.

You can define a name for the `eventHandler`. Note that the prefix `EVENT_` is automatically added to this name. 
