...
Fields are the individual elements which make up any given table in a system's database, and you can think of them as ultimately holding your data.

When defining the data model, we define all of the individual fields needed to make up all the tables in your application.
...
<Tabs defaultValue="whereToDefine" 
        values={[{ label: 'Technical details', value: 'techDetails', }, 
                 { label: 'Where to define', value: 'whereToDefine', }, 
...
Fields are defined in `<application-name>-fields-dictionary.kts`. 

For example for the `position` application that file name would be `position-fields-dictionary.kts`
...
Genesis is modular, so if you have a dependency on another module (e.g. AUTH) your project will naturally inherit all of the fields and have them available in the project. 

As such we need to ensure we either re-use fields available in dependent modules, else define unique field names between all modules and the project itself

### Scripting

GPAL is a scripting language which gives engineers plenty of advantages especially when it comes to repetition. 
For example tasks where needing to create large number of table fields, say `ALGO_PARAM_1` through to `ALGO_PARAM_50`, we can introduce for loops:
