...
#### Extra notes on max size

When using a SQL DB and wanting a field's max size to be the same as what the underlying SQL DB supports, you can use the helper function `dbMaxSize()`.
...
```
Optionally you can also specify a target max size. This is useful for when you want to limit a field's size to a particular length but want to be flexible enough to support different SQL DBs.
For example you could define a field like so:
```kotlin
field(name = "DESCRIPTION", type = STRING, maxSize = dbMaxSize(target = 9000))
```
If using Postgres this would set the field's max size to 9000 because Postgres can support up to 65535. But if using MS SQL then this field's max size would be set to 8000 because that is the max supported size for MS SQL.
