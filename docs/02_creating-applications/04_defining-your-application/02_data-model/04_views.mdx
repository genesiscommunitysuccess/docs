...
When we set up our data model, it will imply relationships between tables. For example, a TRADE has a COUNTERPARTY_ID and an INSTRUMENT_ID and so link to the COUNTERPARTY and INSTRUMENTS tables.

Views allow us to join related tables for a holistic single view.

They are a lot more powerful than this in practice, and underpin many Genesis components which need to read data from the database either in real-time or ad-hoc querys.
...
Views are defined in `<application-name>-view-dictionary.kts`. 

For example for the `position` application that file name would be `position-view-dictionary.kts`
...
Joins allow us to bring other related tables into a view so we can serve up their fields in a single query row.

For example you might want to join a TRADE to an INSTRUMENT in order to pull in INSTRUMENT fields such as it's NAME or CURRENCY to serve up to a grid.
...
You can define multiple joins onto any given table, you can also add nested joins (join to a joined table), and so on...

The following joins POSITION onto ALT_INSTRUMENT_ID on a 2 field key. One of the join values (ALTERNATE_TYPE) uses a literal value as opposed to stemming from a field in POSITION.

We also see a nested join, from ALT_INSTRUMENT_ID onto INSTRUMENT_L1_PRICE
...
:::warning
views with one-to-many joins cannot be used with certain Genesis components, e.g. dataservers.
:::
...
We can add all the fields from a given table to a view by using the allFields accessor
...
We can also override the name of a field by using various operators

- withAlias - give the field an alternative name on the view
- withPrefix - add a prefix to the standard field name. Handy if we have a clash (e.g. COUNTERPARTY and INSTRUMENT both have a NAME field)
...
Derived fields are used to server up fields which are not part of the tables which make up a given view. Their values are typically derived from fields in the tables.
...
Available join types are INNER and OUTER. The default if OUTER where not supplied
...
These are typically used in requestServer queries:
...
So for the above, if we had a requestReply using the view, it would make ALTERNATE_TYPE available as a field input parameter.
