...
* `pal-dataserver` : Add new dataserver update processing mode ? Which update

### Fixes

* `xml-dataserver` : Fixed a race condition when processing a modify event which matches a where clause, where the original insert event occurred very soon before, but did not match the where clause ? Is this correct?
