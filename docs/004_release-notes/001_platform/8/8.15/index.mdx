...
* Polished resizable modal behavior 
* Added initial dimensions properties 
* Support extended design tokens from JSON file 
* Rapid Modal and Dialog Optimisation 
* Support sub-one hour and multi-hour scheduling
* Scheduler support interval and selected hours
* Added new design tokens for form elements

#### Fixes

* Regression with modal styling and layout 
* Fixed some PBC Reporting issues 
* Fix dialog type consistency in attributes/styles 
* Fix z-index in tabs
* Add default design tokens values for missing JSON values
* Change default design tokens to a TypeScript file 
* Replace hardcoded light gray text color in multiselect 
* Emit file-selected event from file-upload 

### Foundation forms

#### Features

* Support default approval message configuration
* Automatic approval message workflow support 
* Add UI schema config to conditionally hide array list delete button 

#### Fixes

* Pass response in submit-success 
* Include nested elements in validateFn 
* Handle datetime/number filter by default + styling
* Correct jsonSchema for filters in req-rep 
* Set autocomplete to off for text/number renderer
* Adding margin left to array list delete button 
...
* Add control panel to delete saved configs 
* Foundation header styling and configuration options 
* Add nested nav permissions and layout item indicators 
...
* Add support for factories for usage in frameworks such as React
* Adding reset to default functionality 

#### Fixes

* Defend against bad configuration causing runtime errors 

### Grid pro

#### Features

* Upgrade AG Grid from v29.2.0 to v34.2.0 with compatibility layer
* Add row-count status bar for infinite scroll
* ReqRep polling support for SSMR 
* Upgrade ag-grid to v35 
* Enable multi column filter for string and enum fields
* Separate filters generation for ssrm 
* Consider null as blank on filter 
* Add RowSelectionModule support 
* Handle values and criteria for setFilter

#### Fixes

* Merge existing grid options and column definitions 
* Update selected row background color 
* Enhance datasource metadata and grid initialization 
* Correct criteria reset in SSMR 
* Update originalCriteriaMatch and apply filters 
* Restore event handlers 
* Fix race condition in datasource reconnection test
* Hide loading overlay after rowData change 
* Correct filters for blanks 
* Correct filters for notBlanks 
* Prevent memory leaks by cleaning up observers and subscriptions
* Respect explicit filterParams values in agSetColumnFilter 
* Export missing datasource types 

### Foundation comms

#### Features

* Add support for `>=` and `<=` in criteriaFiltersToFields
* Add HTTP error handling and session refresh for 401 errors
* Add metadata caching to HTTP connect 
* Add caching for getAvailableResources 

#### Fixes

* Remove unnecessary kv storage availability check 
* Export HttpConnectConfig types for HTTP polling configuration 
* Support REQUEST_SERVER messages in HTTP mode 
* Support per-resource polling intervals via POLLING_INTERVAL_MAP 

### Foundation Entity Management

#### Features

* Add foundation-filters
* Ability to set error notification config 

#### Fixes

* Refresh customEvent form data after update 
* Export list custom element 
* Tweak lifecycle to handle layout events
* Clear error column field after success submit 
...
* ESLint & Stylelint builder upgrade to ESLint 9 with concurrency support 

#### Fixes

* Handle time filter properly 
* Magic number warnings and set to errors for consistency
