...
* Save Document API should allow file type to be provided

###### Dependency Changes

* `deps` : add JFrog dev-repo to Gradle configs to mitigate Maven Central throttling 
...
* `fix` : Email API should allow optional template reference parameter

###### Dependency Changes

* build(deps): add JFrog dev-repo to Gradle configs to mitigate Maven Central throttling

#### 8.15.5

###### Fixes

* `fix-shared` : Replace per-record deletion with batched deleteAll in fix session reset with 10000 batch size
