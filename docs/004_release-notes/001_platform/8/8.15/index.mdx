...
:::tip
This release contains [breaking changes](#breaking-changes-guidance). Further details are listed at the end of this page, along with instructions for migrating from previous releases.
:::
...
* `deploy-gradle-plugin` : Remove legacy tasks; and move docker task to `exec-gradle-plugin`, see [here](#deploy-plugin-changes) for details
...
* `genesis-resource-daemon` : Backport LDAP login mode for the management console
* `genesis-router` : Sysdef-overridable router and MCP ports

###### Fixes

* `dbmon` : Compare against the comparator date in Expr.dateIsAfter

#### 8.15.36

###### Features

* `genesis-pal-consolidator` : Add excludedEmitters support

###### Fixes

* `criteria` : Parse date literals the same way in memory as in SQL
* `genesis-clustersupport` : Extend transient-I/O retry to the explicit-pin path 

#### 8.15.35

###### Features

* `clustersupport` : Consul-backed gFlake NodeId allocation, wiring & reclamation 
* `genesis-db` : Opt-in cached point reads inside transactions 
* `genesis-pal-dataserver` : Report lmdb space usage as metrics 

###### Fixes

* `genesis-clustersupport` : Scope ConsulClientProvider as a singleton 
* `genesis-db` : Report an absent field as null rather than dereferencing it 
* `genesis-db` : Rethrow InterruptedException during SQLEngine dictionary retrieval 
* `genesis-environment` : Build FDB indexes from record-aligned ranges instead of stitched fragments
* `genesis-environment` : Harden VerifyFDBIndexes and the record-aligned FDB remap scan
* `genesis-environment` : Resume an FDB index build after the transaction-size guard stops a batch
* `genesis-pal-dataserver` : Guard the lmdb env lifecycle instead of one call site 
* `genesis-pal-dataserver` : Join lmdb background loops before closing the env 
* `genesis-pal-dataserver` : Read the lmdb space usage in a read transaction
* `server` : Convert DateTime and enum values on every view DbRecord path 

#### 8.15.34

###### Features

* `genesis-resource-daemon` : Authenticated management API and browser console 

###### Fixes

* `genesis-environment` : Make LogLevel work over WebSocket and from the console
* `genesis-process` : Nullable Any-typed fields lose their null option in generated JSON schema 

#### 8.15.33

###### Breaking changes

* `genesis-db` : Hide vararg overload from entity-DB getAllAsList 

###### Fixes

* `genesis-criteria` : Allow nested binary expressions in evaluator result/rule expressions
* `genesis-db` : Backport Bulk upsertAll order/count fix to 8.15.x 
* `genesis-db` : Expose no-arg getBulk()/count() to GPAL scripts

#### 8.15.32

###### Dependency changes

* Bump org.jline:jline from 3.30.6 to 4.0.7 

#### 8.15.31

###### Features

* `genesis-columnar` : SCDB columnar wire encoding for dataserver & request-server
* `genesis-pal-consolidator` : Add opt-in eventual-consistency mode (recompute-on-trigger) to fix out-of-order updates under high throughput

###### Fixes

* Install custom Nginx version via dnf

#### 8.15.30

###### Fixes

* Add genesisType for BigDecimal nullable and non-nullable case
* Update installed toolsets in docker images 
...
* `genesis-pal-datapipeline` : add ability for dbBatchQuery source to start polling from latest

#### 8.15.28

###### Fixes

* `clustersupport` : route pipeline event sink through MessageService with UNAVAILABLE retry and auto-failover
* `genesis-session` : pre-generate Chronicle value flyweight to avoid startup OOM 

#### 8.15.27

###### Fixes

* `daemon` : Fix minor resource leak in daemon script if an exception occurs

#### 8.15.26

###### Fixes 

* `consul` : sessionJob starts unconditionally and monitor checks always register as critical
* `consul` : shut down replaced client to stop Netty reconnect loop retrying dead node

#### 8.15.25

###### Features

* `genesis-router` : Add httpCompression option to router definition

#### 8.15.24

###### Fixes

* `genesis-clustersupport` : Backport Consul failover fixes to 8.15.x

#### 8.15.23

###### Features

* `coreproduct` : Increase maxSize to 256 for SYSTEM_KEY field
* `genesis-process` : Add system definition flag to explicitly disable tracing 

###### Performance improvements

* `genesis-db` : Optimize DataTypeConverter resolution

###### Fixes

* `genesis-pal-datapipeline` : Ensure buildIndex filter is applied from the first poll when no persistence entry exists

###### Dependency changes

* Mirror Maven Central through JFrog and drop unauthenticated spring-plugins repo 

#### 8.15.22

###### Fixes

* `genesis-pal-streamerclient` : Revert "Resource-based service discovery for streamer clients with compact process support" due to resource name clashing issues

###### Dependency Changes 

* Add JFrog dev-repo to Gradle configs to mitigate Maven Central throttling
* Point Maven wrapper distributionUrl to Apache archive to mitigate Maven Central throttling 
* Use JFrog dev-repo first for Maven submodules to mitigate Maven Central throttling

#### 8.15.21

###### Fixes

* `genesis-environment` : SendIt BigDecimal Precision and Rounding Issues for FDB Migrations
* `genesis-router` : preserve MORE_ROWS flag in HTTP dataserver polling response

#### 8.15.20

###### Fixes

* `genesis-dbmon` : support -l flag in search/qsearch when criteriaMode is enabled
* `genesis-environment` : update DumpTable to use commons-collections4

#### 8.15.19

###### Fixes

* `genesis-dataserver2` : Add back kryo-serializers dataserver2 dependency for backwards compatibility

#### 8.15.18

###### Fixes

* `genesis-dataserver2` : add Joda-Time Kryo serializers to remove kryo-serializers dependency

#### 8.15.17

###### Features

* `genesis-pal-requestserver` : expose Request.Details to custom reqrep handlers

###### Fixes

* `genesis-config` : Replace shiro with javax.crypto and add tests for backwards compatibility

#### 8.15.16

###### Fixes

* `gradle` : Allow skipping jars by prefix in classpath file for dynamic deploy

#### 8.15.15

###### Fixes

* `genesis-clustersupport` : replay current snapshot on addServiceListener to avoid waitForDependencies hang

#### 8.15.14

###### Dependency changes

* Bump jackson 2.21.3, log4j 2.25.4, aws-jdbc 3.3.0, debezium 3.5.0, bcprov 1.84, plexus-utils 3.6.1, camel 4.18.2

#### 8.15.13

###### Features

* `coreproduct` : Make MANIFEST_SKIP_PREFIXES configurable via env var 
* `genesis-db` : Configurable IO dispatcher for SyncEntityDb

#### 8.15.12

###### Fixes

* `coreproduct` : Skip maven-artifact and plexus-utils MANIFEST refs in getDependenciesForJar

#### 8.15.11

###### Breaking changes

* `genesis-router` : Sanitise output strings for XSS attacks 

###### Dependency changes

* Make opentelemetry-javaagent compileOnly in genesis-tracing
* Bump Javalin from 6.7.0 to 7.2.0, Jetty from 11.0.25 to 12.1.8 and MCP SDK from 0.10.0 to 1.1.1
* Bump io.opentelemetry.javaagent:opentelemetry-javaagent from 2.25.0 to 2.27.0(deps): bump io.opentelemetry.javaagent:opentelemetry-javaagent from 2.25.0 to 2.27.0
* Bump kafkaClientVersion from 4.1.1 to 4.1.2

###### Features

* `gradle` : Add packagingStrategy property to ExecExtension for app distribution folder structure

###### Fixes

* Exclude plexus-utils from distribution zip
...
* `genesis-db` : Optimistic concurrency strict upsertAll with new record throws exception when timestamp not provided 

#### 8.15.8

###### Breaking changes

* `coreproduct` : Remove MigrateDictionaryToGenesis scripts broken by dta dependencies removal in 8.15.0 

###### Fixes

* `genesis-pal-dataserver` : Return defensive copy from MultiRowEntityToRowMap.recordsForEntity to prevent concurrent iteration on live LongHashSet 

#### 8.15.7

###### Fixes

* `genesis-eventhandler` : Prevent duplicate event handler registration with ClassUtils.getAllInterfaces 
* Avoid exposure of internal details through API and expose them in LOG instead

#### 8.15.6

###### Features

* `genesis-db` : Add delimiter-aware ContainsWordsStartingWithIgnoreCase

###### Fixes

* `genesis-config` : Script cache invalidation targets wrong file in attemptToRecover
* `genesis-eventhandler` : Interface resolution misses inherited interfaces causing type lookup failures 
* `genesis-pal-dataserver` : Check range filter in processOutOfSyncUpdate to remove out-of-range records 

#### 8.15.5

###### Fixes

* `genesis-db` : Translate SQL select aliases correctly in SqlStatementTranslator
* `genesis-db` : Reset oracle sequence values and continue setsequence batch on row errors 

#### 8.15.4

###### Features

* `docker` : Optimise generated Dockerfile build 

###### Performance improvements

* `genesis-pal-dataserver` : Batch enrichment DB lookups to reduce N round-trips to 1
* `genesis-pal-dataserver` : Parallelize client fan-out in DataserverUpdateProcessor 
* `genesis-pal-dataserver` : Replace runBlocking with non-blocking dispatch in ConnectionManager 

###### Fixes

* `genesis-commons` : DST-safe week boundary in dateTimeIsInRange tests
* `genesis-criteria` : `dateTimeIsInRange` throws exception when ran at midnight for TimeRange HOUR and TimeContext previous
* `genesis-pal-dataserver` : Fix intermittent async DataServer test failures caused by timestamp collisions, thread-safety race and stale ClientIndex overwrites
* `genesis-pal-dataserver` : Fix auth update silent data loss and batch emissions 
* `genesis-pal-dataserver` : Fix enriched row state not cleaned up on row deletion 
* `remap` : Add FDB transaction limit checks whilst performing batch write operations to prevent blocker failures whislt executing remap 

#### 8.15.3

###### Performance improvements

* `genesis-pal-execution` : Reduce memory footprint of backward join cache by optimising backward joins implementation and add special case for Identity branches 

###### Fixes

* `exec-gradle-plugin` : Ensure AppDistZip and AppDistTarGz files are compatible with gradle config cache 
* `exec-gradle-plugin` : Replace stock processes with site-specific processes per groupId instead of merging 
* `genesis-db` : Correct threading in jms update queue implementation 
* `genesis-environment` : Improve remap handling in complex situations 
...
The deploy plugin contained tasks that were designed to deploy genesis applications locally, to wsl and over ssh.
With the Genesis IntelliJ plugin and Genesis Start, these have become redundant and have been removed.
This plugin also provides install tasks, that were initially designed to install genesis locally, but that are
currently used to generate deployable artifacts.
For more information on how to generate deployable artifacts going forward, please see [here](/build-deploy-operate/build/build-tools/)
