...
Use only the [methods](/develop/server-capabilities/integrations/custom-endpoints/#request-methods) exposed as public in the interface (get, put, post, delete). 
...
Any new implementations of Data Pipeline must only use the [standard http web calls](/develop/server-capabilities/integrations/custom-endpoints/#request-methods). Check the revised [reference documentation for Data Pipeline](/develop/server-capabilities/integrations/data-pipelines/) for full details.
:::
