...
* Add properties `queryUpdateMessagesReceived` and `lastMoreRowsValueReceived` to `QueryView` object to enable creation of complex tests 
* Implement missing functionality in QueryView to handle all potential use cases 
* Add flag to create compact toString method on entities 
* Add support for dumpit csv and csv.gz files 
...
* Generate all ENUM values inside enum reference objects for completeness 
...
* Catch and accumulate unsupported formula errors while processing a worksheet 
* Fix side effect produced by changing excel naming strategy 
* Make the ExcelType returned from the sampling of empty cells belonging to a labelled column be STRING 
* Ensure we use LMDB as source of truth when the `oldRecord` information in the table update is not available 
* Do not refresh updates that have moved in or out of range 
* When db cache inserts are enabled, write inserts to the cache if existing cache entry value is null 
* Better log error message when SysDef evaluation fails 
* Handle multiple updates for same record in data server 
* Handle case when attempting to rename column that does not exist in dataset 
* Site specific plugin to do not modify binary files stored under data 
* Ensure route extension mechanism can handle dependency injection correctly 
...
* Update criteria conversion to support pure dates 
* Disabling days of the week in date picker 
* Datepicker adding bottom slot, hide weekends attribute, display inline attribute 
* Add DOT_ENV_FILE overrides for picking up extra .env files 
* Allowing a disabled legend via chart config 
* Create package 
* Dispatch logout event from header 
* Add foundation-storybook showcase 
* Add functional emitter 
* Enable filter persistance 
* Expose current tab index on tabs component 
...
* Using current date for rolling day 
* Fixing null date set in calendar
...
* Adding isNullOrWhitespace to web core exports 
