...
* `genesis-db` : Use Dispatchers.IO to handle database queries used in SqlViewExecutor to avoid deadlocks derived from exhausting the dbCoroutinesContext thread pool 
* `genesis-db` : GetRange on a View hangs indefinitely when called within a database transaction
* `genesis-db` : Make AccumulatingUpdateQueueListener.destroy() idempotent
* `genesis-db` : Native sql layer, release connection on failure to execute query
* `genesis-db` : Release semaphore permit when asDataSource().getConnection() fails to acquire a JDBC connection 

###### Dependency changes

* Bump testcontainers from 1.21.3 to 1.21.4
