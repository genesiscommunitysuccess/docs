...
We have had to move onto version 8.10.2 of Gradle as part of a security vulnerability. A dependent library of the Genesis Framework (Apache Camel) had a critical vulnerability and the fixed version uses a library called Jackson 2.18, which is incompatible with Gradle 8.4 (our previous version of gradle). Gradle have fixed in 8.10.2+ but they have not backported to 8.4 and as such projects taking Genesis 8.10 + of the Genesis Server Framework are required to follow these steps to update their project's gradle wrapper to 8.10.*.
