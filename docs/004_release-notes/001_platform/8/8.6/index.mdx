...
* `xml-dataserver` : Fixed a race condition that occurs when processing a modify event that immediately follows an insert that has failed on a `where` clause. Only relevant to applications that have legacy xml dataserver.
