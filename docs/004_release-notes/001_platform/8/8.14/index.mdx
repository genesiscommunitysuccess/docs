...
- fix(genesis-db): Use `Dispatchers.IO` to handle database queries used in `getByCriteria` and `SqlViewExecutor` to avoid deadlocks derived from exhausting the `dbCoroutinesContext` thread pool (GSF-7537) by @khouari1 in https://github.com/genesislcap/genesis-server/pull/5390

#### 8.14.11

###### Features

- feat: Allow passing of arbitrary datasource properties to Hikari using `sql.genesis` syntax (GSF-7536) by @khouari1 in https://github.com/genesislcap/genesis-server/pull/5388

###### Fixes

- fix(genesis-environment): Ensure field max size schema changes respect each individual table definition and preserve field nullability configuration when applying such changes in SQL Server (GSF-7535) by @khouari1 in https://github.com/genesislcap/genesis-server/pull/5381
