...
* `foundation-ui` : Dispatch explicit null event detail for blank rule 
...
* `client-app` : Update gsf to the latest version
...
To avoid these sort of configuration override conflicts we recommend using `processes.xml` system definition overrides documented [here](/develop/server-capabilities/runtime-configuration/system-definition).
