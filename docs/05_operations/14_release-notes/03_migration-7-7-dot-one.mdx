...
## Updating <module> in processes.xml defintions
...
This breaking change is due to the previous style leading to clashes in applications which have modules of the same name (e.g with a `<module>` value of `eventhandler`, it could pick either `<app_name>-eventhandler.jar` not `genesis-eventhandler.jar`) and this future proofs to avoid any such clashes going forward

Failing to make this change will lead to errors during `startProcess` or `startServer` similar to below:
...
Where the fix is to change <module> definition from `pal-requestserver` to `genesis-pal-requestserver`
