...
:::info
👎 Note this approach introduces a maintenance overhead when new versions of the plugin are released, and is not recommended.
:::

## Postgres (for local development)

Genesis full-stack applications require a DB to run. 

Genesis currently recommends that developers use Postgres for local development. It is free, performant and typically easy to set up.
...
:::note
👎 Note that artifacts will miss any scanning you may have set up in your artifactory if this approach is adopted.
:::

If you adopt this approach, you need to share the Genesis client-level user credentials with your. 
...
2. Decide if you wish to run typical Genesis processes/servion a single container or “compacted” processes . [See use of `--compactProcesses`](https://learn.genesis.global/docs/operations/commands/server-commands/#syntax-4) and add to your deployment scripts if desired.
...
4. Making log4j changes in the application to push all logs to stdout
5. Runtime data will be lost due to the nature of containers.
    1. Applications should not use container filesystem storage for any files stored by the application. Instead, use of a shared mounted drive, or else S3 (or similar) is recommended 
6. If you still use cgroup v1 in your container set-up and your containers believe they have access to all the CPUs and Memory of the container host, then you probably need to set **`-XX:ActiveProcessorCount={n}`**jvm flag where `{n}` should be set to the true CPU count the container has at runtime. This [can be applied to `JVM_OPTIONS` system definition item](https://learn.genesis.global/docs/server/configuring-runtime/system-definitions/#items-defined)  so that all processes use the flag on start-up.
...
Clustering 2 or more nodes, typically across availability zones, for resiliency is simple. hosts just need to be able to contact each other via the following ports, and to be known to each other by listing them in the system definition.

[Full clustering setup details here](https://learn.genesis.global/docs/operations/clustering/genesis/)

### Consul

Consul can also be set up and used in a Genesis application environment for HA. Our documentation provides details of how to [set up Consul](https://learn.genesis.global/docs/operations/clustering/consul/).

## nginx

Genesis installations usually use nginx as a lightweight webserver for:
...
Details on configuring it can be found in the [Genesis documentation here](https://learn.genesis.global/docs/operations/server-setup/config-management/#nginx-configuration)
...
:::info
Note that application development teams can configure these to use different port numbers. Make them aware of these settings so that you avoid clashes with internal policies or set-ups.
:::

## Optional supporting infrastructure

The infrastructure descibed here is optional, and can be used to meet specific requirements, such as HA setups.
...
The Genesis platform does not include a Load Balancer. We encourage use of your enterprise’s preferred LB. Usually, this is set up to point to the nginx server running on each Genesis host.
