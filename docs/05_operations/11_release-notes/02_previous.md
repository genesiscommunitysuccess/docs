...
- Add "customLoginAck" block to auth-preferences so we can further customise the login ack message
- Add new fields to ALL_APPROVAL_ALERTS and add new query for ALL_APPROVAL_ALERTS_AUDITS
- Added allowedClockSkewSeconds property to the OIDC verification configuration
- Added logout functionality for OIDC
- Allow configuration of default state for user after creation
...
- Enable sys def implicit receiver in SAML script
- Make user login case-insensitive

### Fixes
- Fix "loginAuthAck" implementation and add proper login test
...
- Fix ENTITY_VISIBILITY implementation to work as expected
- Fix broken auth tests after GSF validation upgrade
- Fix index reference in auth-consolidator.xml to match index name in APPROVAL table
- Only id_token is used so all others are optional
- Remove permissioning block from ALL_APPROVAL_ALERTS and ALL_APPROVAL_ALERTS_AUDITS
- Remove `selfServiceReset` from default auth-preferences.kts
- Rollback SecureTokenGenerator implementation to use the same approach before PR #125 was merged
- Update auth configuration to match pending approval logic in GSF

## Genesis 6.5.0 (GSF)

### Breaking changes
- Make username in scope in permissioning 'where' clause
- MS SQL fields can take max length
- Updated 'USER' table with unique indices for 'REFRESH_TOKEN' 
- Set minimal safe TLS version to 1.2:
- Revamp pending approval mechanism
- Enable JSON schema validation for event handlers in GSF

### Features
- Make username in scope in permissioning 'where' clause
- MS SQL fields can take max length
- Add simple "toValue" extension method to transform/project POJO-like classes between each other
- Added '--dumpSQL' feature to remap to allow export of DDL statements
- Added support to query data based on the TIMESTAMP field 
...
- Remove Service Creator functionality
- Deprecate `Env` helper for Sysdef files 
- Revamp pending approval mechanism 
- Running containers as non root users by 
- Several improvements to pending approval implementation 
- Allow access to self-service password reset without logging in 
- Enable JSON schema validation for event handlers in GSF 
- Enable case insensitive login on the router 
- Make auth map optional in permissioning GPAL 
- Rename backwards joins 
- Updated 'USER' table with unique indices for 'REFRESH_TOKEN' 

### Fixes

- Add args to main method so compiler recognises it as a valid main method 
- "toDbRecord" method in generated GPAL view entities now sets all view fields 
- Changes around h2 execution 
- Check alias store type is SQL to ensure compatibility when using FILE store 
- Correctly resolve dictionary when using custom schema 
- Criteria should be generated with table names 
- Disable resourceDaemons script endpoint by default 
- Do not delete and recreate sequences during Oracle remap 
- Ensure sequences on autoincrement fields are deleted when the relevant tables are 
- Fixed issue where the main docker image and the debug docker image would both be named "debug" 
- Fixed issue whereby codegen would fail when using formatter in fields 
- Fixes resource maps for Consul cluster mode 
- Make get/set AutoIncrement work for Oracle and MSSQL 
- Override "requiresAproval" flag as part of pending approval insert mechanism 
- RT will now wait for log file generation before tailing, up to 20 seconds 
- Reduce excessive logging in dataservers 
- Remap: avoid check remap lock flag, rewriting AliasStore and rewriting DictionaryStore if there are no changes to be made 
- Remove hard requirement to provide a REQUIRES_APPROVAL field as part of the json schema definition 
- Remove sequences from backed up tables to avoid exception on creation of new table 
- Skip RefreshTokenCleanUp if USER table does not exist
...
- Various fixes for SQL operations within custom schemas
- Allows auto-increment ids shared between more than one table to have their own sequence value
- Checking existing constraints before trying to rename them
- Cleaning the data after test execution
- Denial of service via bad dependency version
- In case of process unavailability the auth state is rolled back
- Initialise SysDef object during process bootstrap
- Fix killProcessRestarter
- Move datadump check inside the else block to avoid running it after we have cleared the input buffer
- Remap should not attempt to create a backup of tables that have been added to the schema when using Oracle
- Set minimal safe TLS version to 1.2
- Tactical fix for dataserver priming race condition
