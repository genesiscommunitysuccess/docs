...
| Genesis Site Specific Distribution | global.genesis.site-specific.distribution | Creates the site-specific distribution zip file. presently, this is based on default configuration within the plugin, the same as the Genesis Distribution plugin. | Yes, if project type is APPLICATION |
| Genesis Exec | global.genesis.exec | Creates Gradle tasks used by the Genesis IntelliJ plugin | Yes |
