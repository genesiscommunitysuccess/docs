...
| Genesis Site Specific Distribution | global.genesis.site-specific.distribution | Creates the site specific distribution zip file. At the moment it does this based on default configuration within the plugin, the same as the Genesis Distribution plugin. | Yes if project type is APPLICATION |
