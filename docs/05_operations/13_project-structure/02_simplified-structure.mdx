...
Now let's break down where exactly you put your files in the Simplified structure compared to the Legacy structure. For this example, let’s have our product named "position".
...
The internal modules are located in the root project’s (server) build folder "build/internal-modules". Developers should not amend the files inside those modules.
