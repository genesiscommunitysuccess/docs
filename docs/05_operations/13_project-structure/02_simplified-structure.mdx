...
| -site-specific | Removed. [Application](../settings-plugin/#project-type) projects will have a site-specific distribution file created which will be located in the root project's (server) build folder, **server/build/distributions**. Non-project config and script files will be included in the distribution. All folders and files in the main module's **src/main/genesis** folder, except cfg and scripts, will be also be included. |
