...
- standalone - outside an existing project, such as when scaffolding a new project
- local - within an existing project, such as when executing builds within the project
...
Once you have bootstrapped NPM dependencies, you will be able to call Genx CLI in the **package.json** scripts section in this way:
...
Once this is in place, if you execute `npm run build` or `npm run dev`, it will launch Genx CLI installed in your project locally.
...
Use the `-s` or `--seed` parameter to specify the local seed directory, a remote seed or one of the pre-defined seeds provided by Genesis.
...
This command is useful when used locally in your project. Simply add the following to the list of scripts in your **package.json**:
...
The `build` command creates a production bundle for deployment.
...
The `serve` command enables you to preview a production bundle locally. It starts a static HTTP server in the `dist` folder.
...
The `upgrade` command updates the Foundation UI NPM module dependency versions, regardless of the range defined in your **package.json**. It can be plugged in into CI jobs for automated upgrade workflows.
