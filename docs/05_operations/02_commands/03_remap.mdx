...
|          | --skipTableDump  <!-- remap --skipTableDump -->      | no        | Skips the back-up of old data and runs much quicker; **use with caution** - you will not be able to [roll back](../../05_operations/02_commands/03_remap.mdx#rollback) unless you have specifically made a full back-up | No                 |
...
Renaming fields is unsupported by `remap`, as there is no way to identify individual fields (indices and tables are identified by an "id"). For this purpose, we have the [rename fields script](./01_server-commands.mdx#renamefields-script).
...
A common issue with type conversions is a String to Enum conversion where not all the data matches the allowed list of Enum values. Often, this mismatch is just a case of needing to capitalise the non-matching value, or add an underscore, or both. Where this is the case, you can run the `FixEnumValues` script, or add an installHook that calls it for you. See the section in [server commands](./01_server-commands.mdx#fixenumvalues)).
