...
**Note: The following services do **not** use the new service-discovery mechanism:
...
A new optional Reporting component is now available, which enables users to create report specifications, run them, or save them for later use.
...
To make data available to users to create reports on, entries need to be inserted into the `REPORT_DATASOURCES` table, which determines which data resources can be reported on. Typically, you’ll add pre-existing Data Servers that you need reports on, but you can add new sources by simply creating new Data Servers (using the existing data mechanism).

The Report Server adds the following Meta Data Services.
