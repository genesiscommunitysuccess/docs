...
### What are the main services, features and capabilities?
...
- **Tables (entities)** - model application data as tables with attributes.
- **Views** - Join entities together to form views.

#### Core Microservices
These microservices provide the foundational building blocks for Genesis applications. Some of the key microservices are:

- **Event Handler** – Manages data creation, updates, and business logic execution (CQRS command service).  
- **Data Server** – Handles real-time queries, caching, and live data updates to all subscribers.  
...
These services enable seamless interaction with external systems. Some of the key integration services are:
...
Genesis provides a library of **prebuilt UI components** for financial applications. Some of the key UI components are:
...
Genesis provides a set of reusable components which can be used in any application. Some of the key full-stack components are:
