...
There are option when it comes to file storage. File storage is easy to configure in business components such as [Document Management](/develop/business-components/doc-management/doc-management-introduction/) and where using integration capabilities such as [Data Pipelines](/develop/server-capabilities/integrations/data-pipelines/)

- **Local storage** : useful for many application's however this can cause issues in clustered or container setups unless the filesystem is mounted across all servers.
- **Cloud storage** : managed cloud file storage services, such as AWS S3, are sometimes preferred in HA and container setups.
...
Genesis application environment servers provide a [healthcheck endpoint](#breaks), which the Load Balancer can use to determine server health.
