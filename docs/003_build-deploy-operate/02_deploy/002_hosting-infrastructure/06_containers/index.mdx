...
- `genesis-system-definition.kts` values - To deploy a Genesis App in a Docker container, you need to ensure the item `DEPLOYED_PRODUCT` is present in your `genesis-system-definition.kts`](/develop/server-capabilities/runtime-configuration/system-definition/), with the value set to the name of your app.
