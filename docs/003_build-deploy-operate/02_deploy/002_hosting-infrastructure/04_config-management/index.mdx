...
sidebar_position: 4
id: bdo-deploy-config-management
...
* Environmental override configuration (which requires changes to [processes.xml](/develop/server-capabilities/runtime-configuration/processes/))

### Dependencies

The packages that are needed to run a Genesis application are covered in more detail in [host preparation](/build-deploy-operate/deploy/hosting-infrastructure/host-preparation/).
...
For a manual set-up of nginx, we recommend using this nginx.conf file:

<!-- This code below is manually kept in line with https://github.com/genesislcap/genesis-server/blob/master/genesis-subcore/deploy-gradle-plugin/src/main/resources/docker/nginx.conf --> 
...
Each process within the application can be instructed to read another file to override the main configuration file, [system-definitions](/develop/server-capabilities/runtime-configuration/system-definition/).

The override files can be placed anywhere that is readable to the Genesis application's run user. Override files can be 1-to-1 with processes or re-used. Their location is specified on a per-process basis in the [processes.xml](/develop/server-capabilities/runtime-configuration/processes/) config file (which is part of the site-specific directory contents).
...
Note, override files are not able to perform environment substitutions in the way that system-definitions can. The override file must be a .kts file (Kotlin script), so it is _executed_, whereas the properties file is only read. See [clusters/Environment variables](/build-deploy-operate/deploy/hosting-infrastructure/virtual-machines/#environment-variables).
