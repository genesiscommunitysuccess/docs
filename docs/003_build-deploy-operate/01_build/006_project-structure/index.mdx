...
Contains Genesis DSL files used to configure most of the server capabilities. Any which do not generate DAOs (as they refer to DAOs) should live in this directory. PAL definitions (Data Server, Event Handler, and so on) can also be defined in **compiled Kotlin classes** under `src/main/kotlin` (in a package that is scanned by the process), using the same DSL; see [GPAL Standards](/develop/server-capabilities/gpal-standards/#script-and-compiled-definitions).
