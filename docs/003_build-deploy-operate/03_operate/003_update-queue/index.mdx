...
The `MqLayer` configuration item supports the following values:
...
- `AERON` - a high-performance messaging system.
- `GenerationalNano` - a FoundationDB-only queue and the fallback when `MqLayer` is unset in the legacy database manager path.
- `Disabled` - a no-op queue used by scripts and `DbMon`.
...
| `ZeroMQProxyUnicastRelayExcludedTopics` | String based comma-separated list of topics to be excluded from Unicast relay retransmission. Useful to avoid unnecessary data transfer between `GENESIS_CLUSTER` services for topics that do not need real-time processing. Example: `"TRADE_AUDIT,ORDER_AUDIT"`  | `""` |
