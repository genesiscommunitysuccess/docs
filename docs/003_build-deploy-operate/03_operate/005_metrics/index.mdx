...
When the hierarchical structure is used, the metric ID is built of the following components separated by dots `.`:
1. `genesis`
...
For example: `genesis.genesis.genesis_router.tam_prod1.active_connections`

#### Dimensional metric structure

When the dimensional metric structure is used, the metric ID begins with the metric name and all tag keys and values are joined.

For example: `active_connections{hostName=tam_prod1.processName=genesis_router.groupName=genesis}`

#### Settings

Genesis supports the [Pickle protocol](https://graphite.readthedocs.io/en/stable/feeding-carbon.html#the-pickle-protocol).

You must define both `MetricsGraphiteURL` and `MetricsGraphitePort` in your system definition file. There are no default values for these properties; if either is missing, the application throws a `MetricsConfigurationException`.

The `MetricsStructureType` property is optional and defaults to `hierarchical`.
