...
Install hooks are scripts which will run automatically the next time `genesisInstall` is run in an environment. They are typically operations which should run only once for a given environment, as part of an application version upgrade. If any fail, an error will be provided and it will run again on the next `genesisInstall`.
...
Any environment running an application needs to store state of running any install hooks, so that the next run of `genesisInstall` knows whether to run them or not.

The system definition property `HookStateStore` defines how the environment determines if an installHook has already run:

- `HookStateStore="FILE"` : The install hook's log file output is used to determine. Logs are stored in  `GENESIS_HOME/runtime/installHooks`. The hook state is considered as successful if the last line of the latest log file reads "SUCCESS".

- `HookStateStore="DB"` : The install hook's statuses are stored in the application's database in a table called `SYSTEM`. The hook key is stored in the `SYSTEM_KEY` column and the status is stored in the `SYSTEM_VALUE` column. This is read to determine if any given install hook script needs to run.

#### Running an install hook every time

On rare occasions you might need an install hook which runs every time a genesisInstall is run. i.e irrespective of their previous run status. To do so use one of the following options:
...
#### `genesisInstallHooks`
The `genesisInstallHooks` command will specifically run a (set of) install hook scripts. This is needed when:

1. You need to run just an install hooks without running genesisInstall.
1. You need to run `PRE_INSTALL` or `POST_REMAP` install hooks.
    1. `PRE_INSTALL` hooks are used when you need the script functionality to run **prior** to `genesisInstall` running. Scripts must be placed in the `/scripts/installHooks/preInstall/` directory.
    1. `POST_REMAP` hooks are used when you need the script functionality to run **after** a `remap` has been completed. Scripts must be placed in the `/scripts/installHooks/postRemap/` directory.
...
| --init             | false   |                | `genesisInstallHooks --init=true` marks all installHooks as completed without running any of them. Typically used on a brand new environment. If you the HookStateStore is set to DB and this is a new environment, you'll need to run remap before running with this flag. |
...
- Each install hook must contain a priority number + `_` at the beginning of the file name. This number should be unique. For example: **1_**migrateLogFiles.sh, **2_**migrateDictionary.sh.
