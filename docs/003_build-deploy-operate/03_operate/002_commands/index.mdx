...
`finally` and `batchFinally` clauses: It is run for every record that is purged and used to add some extra functionality if needed, including cascading the purge, see below for details.

### Cascade purges

In some data sets, it is necessary to cascade the purging and delete related records:
...
The main difference is that `batchFinally` is processed in batches, which means that any cascade operation should run more efficiently.
This could be a consideration on larger data sets.
...
`formatMode` corresponds to DumpIt argument.
The `batchSize` property represents purgers batch size. Larger batch sizes should be able to use more efficient database operations to purge, but will require more memory.
