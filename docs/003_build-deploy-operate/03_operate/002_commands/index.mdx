...
You can utilize the system definition property `HookStateStore`, to set it to either `FILE` or `DB`
...
If you need to run some install hooks all the time, i.e irrespective of their previous run status, you can achieve that with following two options
...
| --init             | false |              | `genesisInstallHooks --init=true` Does database dry run                                                    |
