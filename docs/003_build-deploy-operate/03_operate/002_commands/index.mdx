...
This command lets you inspect the system definition values that a given host or environment is picking up. This allows you to confirm what value a process will use at runtime, which is useful for diagnosing unexpected behaviour in environments where configuration can vary by host.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| `PROPERTY_NAME` | yes | The name of the system definition property to inspect | none | none |

###### Examples

To inspect the value of the `DbHost` property:

```bash
readSysDef DbHost
```

To inspect the value of the `MqHost` property:
