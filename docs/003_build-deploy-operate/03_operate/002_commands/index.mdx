...
If more complicated purging logic is required, custom purgers can help.
These allow developers to return a `Collection` or `Flow` of entities as a purger instruction.
For example, if we wanted to remove all users without a corresponding user session:
