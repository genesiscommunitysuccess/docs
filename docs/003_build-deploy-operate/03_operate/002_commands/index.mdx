...
The `genesisInstall` command installs or updates the application schema, system definitions, and other configurations in the database.

### Blocking `genesisInstall` in containerised environments

To prevent accidental execution of `genesisInstall` in containerised environments (such as Docker), you can block the command using the `BlockGenesisInstall` system definition property.

When you generate a Docker runtime image using the Genesis Gradle plugin, `GENESIS_SYSDEF_BlockGenesisInstall` is automatically set to `true` in the environment.

If you attempt to run `genesisInstall` when `BlockGenesisInstall` is set to `true`, the command prints a warning and exits cleanly without performing any installation steps.

To apply changes in environments where `genesisInstall` is blocked, you must:
- Use environment variables for system definition updates.
- Rebuild and redeploy a new container image for configuration or GPAL changes.

You can configure this setting using the `BlockGenesisInstall` property in your `genesis-system-definition.kts` file, or override it using the `GENESIS_SYSDEF_BlockGenesisInstall` environment variable.

###### Syntax

| System Definition Key | Environment Variable Override | Type | Description | Default |
|---|---|---|---|---|
| `BlockGenesisInstall` | `GENESIS_SYSDEF_BlockGenesisInstall` | `boolean` | When set to `true`, blocks the execution of `genesisInstall`. | `false` (automatically set to `true` in Docker images) |

###### Examples

To temporarily allow `genesisInstall` to run in an environment where it is blocked:
...
The `readSysDef` command displays the system definition property values that your host or environment is currently using. This is useful for verifying the active configuration, as it prints each property name and its resolved value, including any environment-variable overrides.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| `[PROPERTIES]` | yes | One or more space-separated system definition property names to resolve | none | none |

###### Examples

To resolve a single property, such as `DbHost`:

```bash
readSysDef DbHost
```

To resolve multiple properties, such as `DbHost`, `DbMode`, and `MqLayer`:

```bash
readSysDef DbHost DbMode MqLayer
```
