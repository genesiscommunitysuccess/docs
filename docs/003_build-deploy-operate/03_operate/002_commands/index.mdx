...
- you need to run `PRE_INSTALL` or `POST_REMAP` install hooks
    - `PRE_INSTALL` hooks are used when you need the script functionality to run **prior** to running `genesisInstall`. Scripts must be placed in the `/scripts/installHooks/preInstall/` directory.
    - `POST_REMAP` hooks are used when you need the script functionality to run **after** a `remap` has been completed. Scripts must be placed in the `/scripts/installHooks/postRemap/` directory.
...
`filterDays`: this method enables to purge data based on calendar date.
You need to provide:
- max age of record
- and optional LONG or DATETIME field of table you want to purge and if not specified TIMESTAMP field of table is used
...
For this, we provide `finally` and `batchFinally` clauses.

When you use one of these clauses, you can add extra functionality for every record that is purged, such as the logic for purging related records:
...
Whereas `finally` deletes records individually as the process runs, the `batchFinally` clause processes the purge in batches and you can specify `batchSize` in the `config` (see below). This enables you to purge more efficiently when you have large datasets, as it results in fewer database operations.

For example, if you use the `finally` clause and you delete 8,000 records, then there will be 8,000 database operations. If you use the `batchFinally` clause on the same dataset with `batchSize` set to 1,000, there will be 8 database operations.
...
```
These config values affect how purgers behave. By default, records are written to a log file in `backupLocation`.

Alternatively, records can be written in a [DumpIt](#dumpit) compatible `.csv` or `.csv.gz` file.

- `formatMode` corresponds to the DumpIt argument.
- `batchSize` represents the batch size of each purge (which results in a single database operation for each batch). Larger batch sizes should be able to use more efficient database operations to purge, but they do require more memory.
