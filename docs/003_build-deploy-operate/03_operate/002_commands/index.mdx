...
* `UNKNOWN` on initialization of a new node, else where there are connectivity issues to other nodes in the cluster
...
* `UNKNOWN` on initialization of a new node, else where there are connectivity issues to the Consul cluster
* `Consul`
...
* `STANDBY` the process is in standby mode; it will take it over if the node becomes Primary (when operating in a regular cluster), else this process is elected the primary singleton process (when operating in a Consul cluster)
