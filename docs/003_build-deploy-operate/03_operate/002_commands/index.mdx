...
The ORDER USER table will be appended to the files that were created in the `tas` directory.
...
1. Navigate to the `{appName}\server\jvm\{appName}-config\src\main\resources\scripts\installHooks` directory.
...
`genesisInstall` looks at all the directories (apart from runtime and generated), all the modules, and all files in the `cfg` directory. It copies the config files from the `cfg` directory into the `generated` directory.
...
Following this, when you start any process, the `startProcess` command reads from the `cfg` directory in the `generated` directory.
...
Install hooks run automatically as part of `genesisInstall`. You can specify the relevant scripts as part of `genesisInstall` then add those scripts to the file  `{applicationName}_config/resources/scripts/installHooks`.
...
On the server, these must be located in the `GENESIS_HOME/{applicationName}/scripts/installHooks` directory. Logs are located in `GENESIS_HOME/runtime/installHooks`.

#### Status of install hook run

To control where the status of the run is stored, use the system definition property `HookStateStore`:

- `HookStateStore="FILE"` places the log file of the hook under **GENESIS_HOME/runtime/installHooks**. The hook state is considered as successful if the last line of the log says "SUCCESS".

- `HookStateStore="DB"` adds the details to a database table called `SYSTEM`; the hook key is stored in the `SYSTEM_KEY` column. The status is stored in the `SYSTEM_VALUE` column.

If you need to run some install hooks all the time, i.e irrespective of their previous run status, use one of the following options:

- Add `_persist` string as part of the install hook name.
- Use the `genesisInstallHooks` command with the `repeatHooks` option, for example: `genesisInstallHooks --repeatHooks=testApp1,testApp2`

#### `genesisInstallHooks`
You can use this command to perform an install hook run at any time.

| Argument           | Mandatory | Options      | Description                                                                                               |
|--------------------|-----------|--------------|-----------------------------------------------------------------------------------------------------------|
| --hookType         | false | POST_INSTALL | This is the default value for hook type, which runs the install hooks in the `/scripts/installHooks/` directory  |
|                    |         | PRE_INSTALL  | `genesisInstallHooks --hookType=PRE_INSTALL` runs install hooks in the `/scripts/installHooks/preInstall/` directory |
|                    |         | POST_REMAP   | `genesisInstallHooks --hookType=POST_REMAP` runs install hooks in the `/scripts/installHooks/postRemap/` directory |
| --init             | false |              | `genesisInstallHooks --init=true` performs a database dry run                                                    |
| --repeatHooks      | false |              | `genesisInstallHooks --repeatHooks=testApp1,testApp2` repeats the install hooks for a specified comma-separated product list |

Install hook file-name conventions:
- Genesis only uses shell script for install hooks. Within the shell script, you can call a Python script, a Kotlin script or whatever is necessary.
- The install hook name must be unique within all the relevant directories for install hooks: `installHooks/`, `installHooks/preInstall/` and `installHooks/postRemap/`.
- Each install hook must contain a priority number at the beginning of the file name. This number should be unique. For example: **1_migrateLogFiles.sh**, **2_migrateDictionary.sh**.
