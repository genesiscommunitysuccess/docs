...
***Where do we store the status of install hook run***

You can use the system definition property `HookStateStore`, to set it to either `FILE` or `DB`.

If `HookStateStore="FILE"` : We refer the log file of the hook located under **GENESIS_HOME/runtime/installHooks**. The hook state is considered as success, if the last line of the log says "SUCCESS".

If `HookStateStore="DB"` : We refer the database table called `SYSTEM`, hook key and status are stored under `SYSTEM_KEY` and `SYSTEM_VALUE` columns respectively

If you need to run some install hooks all the time, i.e irrespective of their previous run status, you can achieve that with one of these options
- Add `_persist` string as part of the install hook name
- Use `genesisInstallHooks` command with `repeatHooks` option ex: `genesisInstallHooks --repeatHooks=testApp1,testApp2`

Along with running the install hooks as part of genesisInstall, we give user flexibility to run them at different genesis-stages by using command called as `genesisInstallHooks`.

#### `genesisInstallHooks`

| Argument           | Mandatory | Options      | Description                                                                                               |
|--------------------|-----------|--------------|-----------------------------------------------------------------------------------------------------------|
| --hookType         | false | POST_INSTALL | This is the default value for hook type, which considers the install hooks under `/scripts/installHooks/` folder are run |
|                    |         | PRE_INSTALL  | `genesisInstallHooks --hookType=PRE_INSTALL` : considers install hooks under `/scripts/installHooks/preInstall/` folder |
|                    |         | POST_REMAP   | `genesisInstallHooks --hookType=POST_REMAP` : consider install hooks under `/scripts/installHooks/postRemap/` folder |
| --init             | false |              | `genesisInstallHooks --init=true` performs a database dry run                                                    |
| --repeatHooks      | false |              | `genesisInstallHooks --repeatHooks=testApp1,testApp2` repeats the install hooks for specified comma seperated product list |
...
- The install hook name must be unique among all the folders of install hooks i.e. `installHooks/`, `installHooks/preInstall/` and `installHooks/postRemap/`.
...
To use this tool, you must have a `*-purger.kts` file in the application's `cfg`` folder. Invoking the **PurgeTables** command will pick up all files that are found in the `$GENESIS_HOME/generated/cfg/` directory.
