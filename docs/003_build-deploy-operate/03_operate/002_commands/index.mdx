...
The `DropTable` command removes all rows from the specified tables. You must specify at least one table; the command does not support dropping all tables without explicit names. The command prompts you for confirmation before dropping each specified table.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| `-t` | yes | Space-separated list of table names to have their rows removed | none | none |

###### Examples

The following example removes all rows from three separate tables:
...
### Preventing execution with BlockGenesisInstall

You can prevent the execution of `genesisInstall` by setting the system definition property `BlockGenesisInstall` to `true`. This is useful in containerised environments (such as Docker) where running `genesisInstall` directly is discouraged.

When `BlockGenesisInstall` is enabled:
- Running `genesisInstall` prints a warning and exits cleanly without performing any installation steps.
- The Genesis Gradle plugin automatically sets the environment variable `GENESIS_SYSDEF_BlockGenesisInstall=true` in generated Docker runtime images to prevent accidental execution.

If you need to override this block in a containerised environment, you can set the environment variable `GENESIS_SYSDEF_BlockGenesisInstall` to `false`.

###### Examples

To temporarily allow `genesisInstall` to run in an environment where it is blocked, set the environment variable before running the command:
