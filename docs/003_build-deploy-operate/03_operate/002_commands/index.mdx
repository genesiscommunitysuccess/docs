...
<!-- INSERT AFTER: ## ReconcileDatabaseSync -->

## `readSysDef`

Use the `readSysDef` command to quickly find the system definition (`sysdef`) values that a given host or environment is picking up and using in a process. This command prints each specified property name and its resolved value, including any environment-variable overrides.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| `PROPERTIES` | yes | Space-separated list of system definition property names to resolve | none | none |

###### Examples

To read the resolved values for `DbHost`, `DbMode`, and `MqLayer`:
