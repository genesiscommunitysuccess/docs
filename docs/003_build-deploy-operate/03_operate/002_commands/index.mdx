...
`finally` is a filter for implementing [cascade purges](#cascade-purges).

`batchFinally` enables you to process records in batches when you purge. You can specify the `batchSize` in the `config`.<br></br> This is useful when you are purging large datasets - you can cut down the number of database operations, which speeds up the process. However, this uses more memory. `batchFinally` can also be used for [cascade purges](#cascade-purges).

### Config

You can supply `config` values to control the behaviour of the purge. For example:
...
The example above us`config` settings:
...
<br></br>For example, if you use the `batchFinally` clause with `batchSize` set to 1,000 and you delete 8,000 records, there will be 8 database operations. By contrast, an unbatched operation using the `finally` filter would have a database operation for every single delete.
