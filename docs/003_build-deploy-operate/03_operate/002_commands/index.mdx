...
This command displays the system definition values that are currently active for your host or environment. It is useful for verifying which system definition values a process is picking up, especially when values are overridden by environment variables.

###### Syntax

The `readSysDef` command has no parameters.
