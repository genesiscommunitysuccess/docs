...
This command displays the system definition values currently active for the host or environment. It helps verify which system definition properties a process resolves and uses at runtime.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| `PROPERTY_NAME` | no | The specific system definition property to retrieve. | none | displays all system definition properties |

###### Examples

To display all active system definition properties:

```bash
readSysDef
```

To display the value of a specific system definition property (for example, `DbHost`):
