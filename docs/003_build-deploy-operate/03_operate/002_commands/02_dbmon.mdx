...
The [`showTables`](#dbmon-commands) and [`showViews`](#dbmon-commands) commands display an alphabetic list of available tables or views. For example, here we have used the command `showTables`:
...
[Timestamps](/develop/server-capabilities/data-access-apis/#timestamps), are made of a timestamp in milliseconds, the sequence id and the node id: for example,
...
| listAll                              | `<key_name> <num_key_fields> <max_records>` |  requires the [table or view context](#finding-and-viewing-information) and the record context; lists all the records from the table or view, which have the same value as the context record for the first fields of the key (if all the fields for the key are used, only one record is returned)  |
...
<!-- Auto-generated from genesislcap/genesis-server#0 -->
<!-- INSERT AFTER: ## KillServer -->

## `readSysDef`

This command displays the resolved system definition (sysdef) values that the current host or environment uses for its processes. It is useful for verifying active configuration values when overrides, such as environment variables, are in use.

###### Syntax

| Argument | Mandatory | Description | Restricted values | Default |
|---|---|---|---|---|
| [PROPERTY_NAME] | no | The specific system definition property to retrieve. | none | displays all properties |

###### Examples

To display all resolved system definition properties:

```bash
readSysDef
```

To display the value of a specific system definition property:
