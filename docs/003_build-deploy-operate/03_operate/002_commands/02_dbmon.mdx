...
To delete a record, use the `delete` command. This command deletes only the currently selected record (`Context.currentRecord`).

If you run `delete` when you have only selected a table (and no specific record has been loaded or searched), the command fails with the error `Record does not exist in the database`.

If you have performed a search, the current record is set to the last record matched and printed. Running `delete` immediately after a search deletes that last search result.
