...
This new feature is consistent across our different modules, such as authorisation, Data Server, and REQUEST SERVER.

The new `filter` clause replaces the old `where` clause. The old `where` clause is now deprecated. Use `filter` from now on.
...
We have made rxSources non-functional interfaces, standardised our http source syntax and added utilities. 
...
we have added type safe onOperationError handling to data pipelines. Error handling for Data Pipeline has been redesigned. 

**Are you affected?**

If your application uses Data Pipelines and has error handlers in them, you are affected.

:::info
**Medium fix**
Change your error handling based on the [new handlers](/server/data-pipelines/implementations/#handlers).
:::
