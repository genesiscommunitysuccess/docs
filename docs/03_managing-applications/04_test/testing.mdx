...
In all other regards, the database tests are a normal JUnit test. If you need additioal components, you will need to construct them or mock them. You won’t be able to specify a `GENESIS_HOME` folder for additional configuration.
...
Any path provided for `genesisHome` and `initialDataFile` should be an absolute location.
