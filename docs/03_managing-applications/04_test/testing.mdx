...
### Before you start
Before you start, make sure your server is running. Then run mon to check that your particular component’s process is running. For example, if you want to test your request server, check that the _application_**REQUEST_SERVER** process is running.
For any testing, you need to know the IP address of name of your server, plus user name and password that will enable you to login and authenticate yourself.
...
Enter the IP address of our sever.
Log in with your user name and password
Click on the RESOURCES tab.
Filter the type to show only request servers.
For example:
To follow
...
For the url, you need to supply your server instance, followed by event-login-auth. For example:
https://test-mynewapp/event-login-auth
Insert the message below (substituting your correct user name and password) in the main body. 
...
```
At the top left of the Insomnia window, click on the arrow at the end of the query name to set the header. Insert a SOURCE_REF in the header. This can be any string that identifies you (in effect).
...
This section examines two types of test that you can develop:

* database test
* service test

For both types, you will need to start with a dictionary. This section will guide you through using GPAL dictionaries in Genesis tests.
...
- Database tests - use these types for testing classes that require database access.
- Test support tests - use these types of tests for testing a service

All dictionaries types work with both types of test.
...
In this test, we define two fields and a table that uses these. We make sure there are no records in the `USER` table. Please note that since we are creating a stand-alone dictionary, we can start with id 1.
...
Paths provided for `genesisHome` and `initialDataFile` should be absolute location.
...
There are two functions for sending messages to a service; one uses RxJava2 Single, the other uses Kotlin coroutines. Whichever one you use, shouldn’t make a whole lot of difference in your test. The method `sendMessage(…)` will return a `Single`, this will require a call to `blockingGet()` for every message you’re interested in. `sendMessageAsync` will require you to wrap your test in a `runBlocking { … }` block.
...
##### Type safe Req Rep tests

Below is an example of writing a type safe req rep test using a `RequestReplyWorkflow`. `RequestReplyWorkflow` requires two type parameters. The first is the inbound class and the second is the outbound class.
...
You can override system definition properties in your test class by overriding the `systemDefintion()` function.
