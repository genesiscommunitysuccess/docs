...
3. For the url, you need to supply your server instance, followed by event-login-auth. For example:
https://test-mynewapp/event-login-auth
4. Insert the message below (substituting your correct user name and password) in the main body. 
...
```
5. At the top left of the Insomnia window, click on the arrow at the end of the query name to set the header. Insert a SOURCE_REF in the header. This can be any string that identifies you (in effect). In the example below, we have set SOURCE_REF to BAUDOIN1 (for no particular reason).
![](/img/insomnia-header.png)
