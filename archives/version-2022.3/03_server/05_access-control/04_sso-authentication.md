...
```
Note that the 'loginEndpoint' is the URL to which the front end is re-directed after the full SAML workflow has been completed and an SSO_TOKEN has been issued. 

If this URL is a redirect, the SSO_TOKEN query parameter could be lost. 

Additionally, if the web server is routing via scripts, navigating to this URL could throw a **404 Not Found** error. The remedy in this case is to add an override for 404 errors to redirect back to your application's logon screen. 
